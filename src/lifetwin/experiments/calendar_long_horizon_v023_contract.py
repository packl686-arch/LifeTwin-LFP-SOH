"""Authenticated V2.8 boundary over the generic V0.20 lifecycle core."""
...
class V028ContractError(V024ContractError):
    """Raised when the V2.8 identity weakens the authenticated base."""


def _adapt_config_json(
    base: V024ContractView,
    design: ValidatedV028Design,
) -> str:
    config = base.protocol.config()
    config["protocol_id"] = design.protocol_id
    partitions = config.get("design_partitions")
    if not isinstance(partitions, dict):
        raise V028ContractError("Inherited design_partitions is invalid")
    ...
    if not isinstance(restored_partitions, dict):
        raise V028ContractError("Adapted design_partitions is invalid")
    restored_partitions["seed_roots"] = dict(base.protocol.seed_roots)
    if restored != base.protocol.config():
        raise V028ContractError("V2.8 changed an inherited scientific field")
    return json.dumps(
        config,
        ensure_ascii=True,
        allow_nan=False,
...
def load_v028_contract_view(
    amendment_path: str | Path = DEFAULT_V028_AMENDMENT_PATH,
) -> V024ContractView:
    """Load the exact V2.8 identity without deriving or consuming a seed."""

    design = load_v028_design(amendment_path)
    base = load_v027_contract_view()
    if design.raw["base_contract"]["fixed_core_commit"] != V028_FIXED_CORE_COMMIT:
        raise V028ContractError("V2.8 fixed-core ancestry changed")
    if set(design.seed_roots.values()).intersection(
        root for _, root in base.protocol.seed_roots
    ):
        raise V028ContractError("V2.8 roots collide with V2.7")
...
__all__ = ["V028ContractError", "load_v028_contract_view"]
