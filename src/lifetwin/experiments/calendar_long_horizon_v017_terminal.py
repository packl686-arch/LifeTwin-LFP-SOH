"""Fail-closed V2.2 pre-prediction terminal publication.

The writer emits exactly the three terminal artifacts frozen by the V2.2
...
V022_AMENDMENT_BYTE_SHA256 = load_v022_design().config_byte_sha256
_PROJECT_ROOT = Path(__file__).resolve().parents[3]


class V017TerminationError(ValueError):
    """Raised when terminal evidence is invalid, unsafe, or conflicting."""


class TerminalReason(str, Enum):
    """The exact reason-code registry frozen by the V2.2 amendment."""
...
_INTEGRITY_MESSAGES: Mapping[TerminalReason, str] = {
    ...
    TerminalReason.INTEGRITY_PATH_ISOLATION_FAILURE: (
        "The frozen V2.2 path-isolation contract failed."
    ),
...
class V017TerminalInconclusive(RuntimeError):
...
class V017IntegrityError(RuntimeError):
...
class V017Interrupted(RuntimeError):
...
class V017ConservativeVoid(RuntimeError):
...
def _v022_training_reason(error: BaseException) -> TerminalReason | None:
    """Recognize only the shared nominal calibration signal."""

    if type(error) is not V022CalibrationTerminalInconclusive:
        return None
...
def classify_terminal_exception(error: BaseException) -> TerminalClassification:
    ...
    if isinstance(error, V017IntegrityError):
    ...
    if isinstance(error, V017ConservativeVoid):
        return _classification(
            error=error,
            disposition=TerminalDisposition.INTEGRITY_FAILURE,
    ...
    if isinstance(error, V017TerminalInconclusive):
        return _classification(
            error=error,
    ...
    training_reason = _v022_training_reason(error)
    ...
    if isinstance(error, V017Interrupted):
        return _classification(
            error=error,
...
def _canonical_json_bytes(payload: Mapping[str, Any]) -> bytes:
    try:
        ...
        raise V017TerminationError("Terminal JSON is not canonicalizable") from exc
    return f"{text}\n".encode("ascii")
...
def sanitized_structural_traceback(
    ...
    *,
    ...
    if (
        ...
        raise V017TerminationError("Structural traceback sanitization failed")
    return raw
...
class TerminalContext:
    ...
    def __post_init__(self) -> None:
        if self.protocol_id != V022_PROTOCOL_ID:
            raise ValueError("Terminal protocol_id is not the validated V2.2 ID")
        if self.config_byte_sha256 != V022_AMENDMENT_BYTE_SHA256:
            raise ValueError("Terminal config hash is not the validated V2.2 amendment")
        if (
...
def _validate_exact_keys(
    ...
    *,
    ...
    if not isinstance(value, Mapping) or set(value) != keys:
        raise V017TerminationError(f"{context} keys are not exact")
    return value


def _validate_identity(payload: Mapping[str, Any], *, context: str) -> None:
    if (
        payload["protocol_id"] != V022_PROTOCOL_ID
        or payload["config_byte_sha256"] != V022_AMENDMENT_BYTE_SHA256
        ...
        raise V017TerminationError(f"{context} identity is invalid")
...
def _expected_classification(
    ...
    raise V017TerminationError("Disposition and reason classification conflict")
...
def _validate_structural_traceback(
    ...
    *,
    ...
    if (
        ...
        raise V017TerminationError("Structural traceback identity is invalid")
    for value in trace["frames"]:
        ...
        if (
            ...
            raise V017TerminationError("Structural traceback frame is invalid")
    ...
    if (
        ...
        raise V017TerminationError("Structural traceback contains unsafe text")
...
def validate_terminal_attempt_record(payload: Mapping[str, Any]) -> None:
    ...
    if not isinstance(record["git_dirty"], bool):
        raise V017TerminationError("Terminal attempt git_dirty is invalid")
    for phase_name in ("phase", "last_completed_phase"):
        if (
            ...
            raise V017TerminationError("Terminal attempt phase is invalid")
    try:
        reason = TerminalReason(record["reason_code"])
        mode = ClassificationMode(record["classification_mode"])
    except (TypeError, ValueError) as exc:
        raise V017TerminationError("Terminal classification code is invalid") from exc
    ...
    if (
        ...
        raise V017TerminationError("Terminal disposition is invalid")
    ...
    if (
        ...
        raise V017TerminationError("Terminal truth exposure is invalid")
    ...
    if (
        ...
        raise V017TerminationError("Sanitized stderr metadata is invalid")
...
def _validate_preterminal_file_record(value: object) -> Mapping[str, Any]:
    ...
    if (
        ...
        raise V017TerminationError("Preterminal artifact record is invalid")
    return record
...
def _validate_terminal_file_record(
    ...
    *,
    ...
    if (
        ...
        raise V017TerminationError(f"{expected_path} file record is invalid")
...
def validate_terminal_artifact_manifest(payload: Mapping[str, Any]) -> None:
    ...
    if not isinstance(files, list) or len(files) != 2:
        raise V017TerminationError("Terminal file registry is invalid")
    expected = (
    ...
    if not isinstance(preterminal, list) or not preterminal:
        raise V017TerminationError("Preterminal artifact registry is invalid")
    ...
    if (
        ...
        raise V017TerminationError("Preterminal artifact registry is incomplete")
    ...
    for key, value in commitments.items():
        if key == "prediction_commitment_byte_sha256":
            if value is not None:
                raise V017TerminationError("Prediction commitment is forbidden")
        elif value is not None and (
            not isinstance(value, str) or _SHA256.fullmatch(value) is None
        ):
            raise V017TerminationError("Preterminal commitment is invalid")
    ...
    for commitment_name, path in commitment_paths.items():
        ...
        if committed is not None and observed is None:
            raise V017TerminationError("Committed preterminal artifact is absent")
        if committed is not None and observed["byte_sha256"] != committed:
            raise V017TerminationError("Preterminal artifact commitment hash differs")
    ...
    if policy != {
        ...
        raise V017TerminationError("Terminal artifact policy is false")
    if (
        ...
        raise V017TerminationError("Terminal manifest boundary is invalid")
...
def _parse_ledger_prefix(raw: bytes, context: TerminalContext) -> LedgerSnapshot:
    if not raw or not raw.endswith(b"\n"):
        raise V017TerminationError("Preterminal ledger is empty or truncated")
    try:
        ...
        raise V017TerminationError(
            "Preterminal ledger failed the strict canonical V2.2 parser"
        ) from exc
    try:
        progress = states[context.attempt_id]
    except KeyError:
        raise V017TerminationError("Attempt is absent from the preterminal ledger")
    if (
        ...
        raise V017IntegrityError(
            TerminalReason.INTEGRITY_CONFIG_OR_SOURCE_HASH_MISMATCH
        )
    opened = set(progress.opened_truth_files)
    if opened & _HELD_OUT_TRUTH:
        raise V017IntegrityError(TerminalReason.INTEGRITY_FORBIDDEN_TRUTH_ACCESS)
    if opened - _DEVELOPMENT_TRUTH:
        raise V017IntegrityError(TerminalReason.INTEGRITY_INFORMATION_LEAK)
    ...
    if progress.truth_commitments_byte_sha256 != context.truth_commitments_byte_sha256:
        raise V017IntegrityError(TerminalReason.INTEGRITY_MISSING_COMMITMENT)
    ...
    if (
        ...
        raise V017IntegrityError(TerminalReason.INTEGRITY_ARTIFACT_HASH_MISMATCH)
...
def _build_attempt_record(
    *,
    ...
    try:
        ...
        raise V017TerminationError("Sanitized structural traceback is invalid") from exc
...
def build_termination_manifest(
    *,
    ...
    """Compatibility alias for the exact V2.2 artifact-manifest builder."""
...
def _validate_terminal_ledger_record(
    ...
    *,
    ...
    if record != expected:
        raise V017TerminationError("Terminal ledger commitment conflicts")
    ...
    *,
    ...
    if current == expected_prefix:
        ...
        if path.read_bytes() != expected_prefix + record_raw:
            raise V017TerminationError("Terminal ledger append was not exact")
        return True
    if not current.startswith(expected_prefix):
        raise V017TerminationError("Live ledger prefix changed after snapshot")
    suffix = current[len(expected_prefix) :]
    if not suffix.endswith(b"\n") or len(suffix.splitlines()) != 1:
        raise V017TerminationError("Terminal ledger has later or malformed records")
...
def _canonical_mapping_from_bytes(
    ...
    *,
    ...
    try:
        ...
        raise V017TerminationError(f"Existing {context} is invalid") from exc
    if not isinstance(value, Mapping):
        raise V017TerminationError(f"Existing {context} is not an object")
    if _canonical_json_bytes(value) != raw:
        raise V017TerminationError(f"Existing {context} is noncanonical")
    return value
...
def _resolve_publication_roots(
    *,
    ...
    def physical_root(raw: str | Path, *, context: str) -> Path:
        ...
        for candidate in (lexical, *lexical.parents):
            if os.path.lexists(candidate) and _is_reparse(candidate):
                raise V017TerminationError(f"{context} root traverses a reparse point")
        resolved = lexical.resolve()
        if not resolved.is_dir():
            raise V017TerminationError("Terminal publication roots must exist")
    ...
    if (
        ...
        raise V017TerminationError("Termination and label-free roots must be disjoint")
    ...
    *,
    ...
    with _publication_lock(root), locked_ledger(ledger):
        ...
        if any(
            ...
            raise V017TerminationError("Terminal registry contains unsafe entries")
        ...
        for name, path in observed_paths.items():
            if path.read_bytes() != expected_bytes[name]:
                raise V017TerminationError(f"Terminal artifact conflicts: {name}")
...
def _environment_guarded_error(
    *,
    ...
    try:
        ...
        return V017IntegrityError(TerminalReason.INTEGRITY_ENVIRONMENT_MISMATCH)
    if (
...
def publish_terminal(
    *,
    ...
    """Publish any typed/default V2.2 pre-prediction terminal disposition."""
...
__all__ = [
    ...
    "TerminalDisposition",
    "TerminalReason",
    "V017ConservativeVoid",
    "V017IntegrityError",
    "V017Interrupted",
    "V017TerminalInconclusive",
    "V017TerminationError",
