"""One-shot, post-freeze generation for the V2.1 synthetic protocol.

The public lifecycle has two explicit capabilities.  The first performs and
commits the complete RNG-free V2.1/V2 coordinate audit.  The second verifies
that exact commitment before it can consume the first V2.1 seed.  Neither
...
class V021GenerationError(ValueError):
    """Raised when the V2.1 generation lifecycle cannot advance exactly."""


@dataclass(frozen=True)
class WrittenV021GenerationArtifacts:
    """Metadata for one committed V2.1 generation bundle."""
...
def _is_reparse_entry(path: Path) -> bool:
    try:
        ...
        raise V021GenerationError(f"Cannot inspect formal path: {path}") from exc
...
def _physical_root_identity(
    ...
    *,
    ...
    try:
        ...
        raise V021GenerationError(f"Cannot bind {context} root identity") from exc
    if resolved != path:
        raise V021GenerationError(f"{context} root changed during resolution")
...
def _verify_physical_root_identity(
    ...
    *,
    ...
    if observed != identity:
        raise V021GenerationError(f"{context} directory identity changed")
...
def _require_exact_direct_files(
    ...
    *,
    ...
    if not root.is_dir() or _is_reparse_entry(root):
        raise V021GenerationError(f"{context} root is not a physical directory")
...
def _write_prepared_generation_artifacts_resumable(
    ...
    *,
    ...
    if (
        ...
        raise V021GenerationError("Prepared artifact membership or order changed")
...
def _decode_canonical_plan_commitment(raw: bytes) -> Mapping[str, object]:
    ...
    if not isinstance(decoded, Mapping):
        raise V021GenerationError("Generation-plan commitment must be an object")
    canonical = (
        json.dumps(
            decoded,
            ensure_ascii=True,
    ...
    if raw != canonical:
        raise V021GenerationError("Generation-plan commitment bytes are not canonical")
    return decoded
...
def _formal_environment() -> object:
    ...
    if (
        getattr(environment, "protocol_id", None) != V021_PROTOCOL_ID
        or getattr(environment, "git_dirty", None) is not False
    ):
        raise V021GenerationError("Formal V2.1 environment identity is invalid")
    return environment


def _validate_contract_view(view: V021ContractView) -> None:
    if type(view) is not V021ContractView:
        raise V021GenerationError("V2.1 contract view has an invalid type")
    if (
        ...
        raise V021GenerationError("V2.1 protocol and artifact contracts disagree")
    if view.design_status != _IMPLEMENTABLE_STATUS:
        raise V021GenerationError("V2.1 implementation has not been immutably frozen")
...
def _validate_attempt_identities(
    ...
    *,
    ...
    if not states:
        raise V021GenerationError("Formal exposure ledger contains no attempt")
...
def _select_generation_candidate(
    ...
    *,
    ...
    if len(candidates) != 1:
        raise V021GenerationError(
            "Exactly one plan-committed attempt may begin V2.1 generation"
        )
    if any(
        state.identity.attempt_id != candidates[0].identity.attempt_id
        and not state.terminal_failed
...
def commit_frozen_v021_generation_plan(
    *,
    label_free_root: str | Path,
) -> str:
    ...
    if getattr(environment, "config_byte_sha256", None) != (
        ...
        raise V021GenerationError(
            "Formal environment and V2.1 amendment commitments disagree"
        )
    ...
    except (OSError, V021FirewallError) as exc:
        raise V021GenerationError(
    ...
    append_formal_exposure_event(
        ...
        truth_commitments_byte_sha256=None,
        prediction_commitment_byte_sha256=None,
        message="Started the exhaustive RNG-free V2.1/V2 coordinate audit.",
    )
    try:
        commitment = audit_formal_v021_generation_plan(view)
...
def generate_frozen_v021_artifacts(
    *,
    label_free_root: str | Path,
    sealed_truth_root: str | Path,
) -> WrittenV021GenerationArtifacts:
    """Generate fresh V2.1 rows only after the committed collision audit."""
    ...
    try:
        ...
        raise V021GenerationError(
            "Formal exposure ledger failed before V2.1 generation"
        ) from exc
    identity = _identity(progress)
    ...
    try:
        ...
        _append_failure(
            ...
            view=view,
            phase="truth_committed",
            message="V2.1 generation did not reach its truth commitment.",
        )
        raise
    ...
    append_formal_exposure_event(
        ...
        prediction_commitment_byte_sha256=None,
        message="All nine fresh V2.1 sealed files were committed before truth access.",
    )
    return WrittenV021GenerationArtifacts(
