"""V2.4 calibration-pool split without changing the frozen V0.15 code.

The V2.4 rule separates the rows used for probability calibration from the
...
V024_MINIMUM_ELIGIBLE_FRACTION = 0.95
V024_MINIMUM_ELIGIBLE_COUNT = math.ceil(
    V024_MINIMUM_ELIGIBLE_FRACTION * CALIBRATION_COUNT
)
...
class V024CalibrationError(ValueError):
    """Raised when V2.4 calibration inputs violate their typed contract."""


@dataclass(frozen=True)
class V024CommittedMaskRow:
...
@dataclass(frozen=True)
class V024PretruthMaskCommitment:
...
def _numeric_matrix(
    ...
    *,
    ...
    try:
        ...
        raise V024CalibrationError(f"{name} must be a numeric matrix") from exc
    if matrix.shape != (rows, columns):
        raise V024CalibrationError(
            f"{name} must have shape ({rows}, {columns}), got {matrix.shape}"
        )
    if require_finite and not np.isfinite(matrix).all():
        raise V024CalibrationError(f"{name} must contain only finite values")
    return matrix
...
def _numeric_vector(
    ...
    *,
    ...
    try:
        ...
        raise V024CalibrationError(f"{name} must be a numeric vector") from exc
    if vector.shape != (length,):
        raise V024CalibrationError(f"{name} must contain exactly {length} values")
...
def _strict_text_vector(
    ...
    *,
    ...
    if raw.shape != (CALIBRATION_COUNT,) or any(
        ...
        raise V024CalibrationError(f"{name} must contain exactly 900 valid strings")
    result = tuple(str(value) for value in raw)
    if require_unique and len(set(result)) != CALIBRATION_COUNT:
        raise V024CalibrationError(f"{name} must contain 900 unique values")
    return result
...
def _require_source_count(**row_aligned: object) -> None:
    ...
    if mismatches:
        raise V024CalibrationTerminalInconclusive(
            "CALIBRATION_SOURCE_COUNT_NOT_900",
            "V2.4 calibration source count must be 900 for every aligned "
...
def _derive_pretruth_evidence(
    *,
    ...
    if not isinstance(risk_state, RiskDevelopmentState):
        raise V024CalibrationError("risk_state must be a RiskDevelopmentState")
    ...
    except V024CalibrationError as exc:
        raise V024CalibrationTerminalInconclusive(
            "CALIBRATION_BASELINE_INCOMPLETE",
    ...
    committed_rows: list[V024CommittedMaskRow] = []
    ...
    if commitment.eligible_count < V024_MINIMUM_ELIGIBLE_COUNT:
        raise V024CalibrationTerminalInconclusive(
            "CALIBRATION_RISK_ELIGIBLE_BELOW_855",
            "V2.4 isotonic calibration requires at least "
            f"{V024_MINIMUM_ELIGIBLE_COUNT} eligible rows; observed "
...
def calibration_eligibility_mask_sha256_v024(
    commitment: V024PretruthMaskCommitment,
) -> str:
    """Return the digest of an already-derived immutable pretruth commitment."""

    if not isinstance(commitment, V024PretruthMaskCommitment):
        raise TypeError("commitment must be a V024PretruthMaskCommitment")
...
def fit_calibration_development_state_v024(
    *,
    ...
    latent_target_forecast_days: Sequence[Sequence[float]],
    latent_targets_pct: Sequence[Sequence[float]],
) -> tuple[CalibrationDevelopmentState, V024CalibrationAudit]:
    ...
    if not isinstance(pretruth_commitment, V024PretruthMaskCommitment):
        raise V024CalibrationError(
            "pretruth_commitment must be a V024PretruthMaskCommitment"
        )
    ...
    try:
        ...
        if (
            ...
            raise V024CalibrationTerminalInconclusive(
                ...
                "A V2.4 isotonic map has fewer than two score thresholds",
            )
    except V024CalibrationTerminalInconclusive:
        raise
    except V2ModelError as exc:
        raise V024CalibrationTerminalInconclusive(
            "CALIBRATION_ISOTONIC_FIT_UNDEFINED",
            "V2.4 isotonic fitting is undefined on the shared eligible pool",
        ) from exc
    ...
    try:
        ...
        raise V024CalibrationTerminalInconclusive(
            ...
            "V2.4 conformal calibration is undefined on the 900-row source",
        ) from exc
    if (
        ...
        raise V024CalibrationTerminalInconclusive(
            ...
            "V2.4 conformal state differs from the 900/811 rule",
        )
...
__all__ = [
    ...
    "V024CalibrationError",
    "V024CalibrationTerminalInconclusive",
    "V024CommittedMaskRow",
    "V024PretruthMaskCommitment",
    "V024_MINIMUM_ELIGIBLE_COUNT",
    "V024_MINIMUM_ELIGIBLE_FRACTION",
    "calibration_eligibility_mask_sha256_v024",
