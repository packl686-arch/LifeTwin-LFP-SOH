"""Fail-closed implementation and environment guard for the V2.2 attempt."""
...
class V022EnvironmentError(RuntimeError):
    """Raised when a formal process is not the attested V2.2 implementation."""
...
def _git(repo_root: Path, *arguments: str) -> str:
    try:
        ...
        raise V022EnvironmentError(f"Cannot verify git {' '.join(arguments)}") from exc
...
def _source_hashes(repo_root: Path) -> Mapping[str, str]:
    ...
    if any(not path.is_file() for path in paths):
        raise V022EnvironmentError("A V2.2 implementation source is not a file")
...
def _strict_json_object(path: Path, *, context: str) -> dict[str, object]:
    try:
        ...
        raise V022EnvironmentError(f"{context} is invalid JSON") from exc
    if not isinstance(decoded, dict):
        raise V022EnvironmentError(f"{context} must be a JSON object")
    return decoded
...
def _reject_duplicate_keys(
    ...
    for key, value in pairs:
        if key in result:
            raise V022EnvironmentError(f"Duplicate JSON key: {key}")
        result[key] = value
    return result


def _reject_json_constant(token: str) -> None:
    raise V022EnvironmentError(f"Nonfinite JSON constant: {token}")
...
def verify_formal_environment(
    ...
    if not (root / ".git").exists():
        raise V022EnvironmentError("Formal repository root has no .git directory")
    top_level = Path(_git(root, "rev-parse", "--show-toplevel")).resolve()
    if top_level != root:
        raise V022EnvironmentError("Formal repository root is not the git top level")
    git_commit = _git(root, "rev-parse", "HEAD")
    if _FULL_GIT_HASH.fullmatch(git_commit) is None:
        raise V022EnvironmentError("Git did not return a full commit hash")
    if _git(root, "status", "--porcelain=v1", "--untracked-files=all"):
        raise V022EnvironmentError("Formal implementation worktree is dirty")
    ...
    try:
        ...
        raise V022EnvironmentError("V2.2 amendment validation failed") from exc
    ...
    if sys.version_info[:3] != _EXPECTED_PYTHON:
        raise V022EnvironmentError("Formal Python runtime differs from CPython 3.12.13")
    ...
    return FormalEnvironmentIdentity(
        ...
        config_canonical_sha256=V022_AMENDMENT_SEMANTIC_SHA256,
        preregistration_byte_sha256=_PREREG_BYTE_SHA256,
        environment_lock_byte_sha256=_LOCK_BYTE_SHA256,
