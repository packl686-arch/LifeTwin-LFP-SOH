"""Committed-artifact scoring adapter for the frozen V2.2 protocol.
...
stochastic roots, and the V2.2 risk-calibration population.  Formal scoring
requires an IO-issued committed model capability and an envelope bound to
verified V2.2 prediction-commitment evidence; it never accepts a bare V2
decoded model state.
"""
...
from lifetwin.experiments.calendar_long_horizon_v017_analysis import (
    BOOTSTRAP_RESAMPLES,
...
from lifetwin.experiments.calendar_long_horizon_v017_io import (
    ...
    _require_prediction_commitment_evidence_v022,
)
from lifetwin.experiments.calendar_long_horizon_v017_pipeline import (
    V022PipelineError,
...
class V022ScoringError(ValueError):
    """Raised when committed V2.2 scoring cannot prove its boundaries."""
...
_ATTEMPT_ID = re.compile(r"^v022-[A-Za-z0-9][A-Za-z0-9._:-]{0,95}$")
...
_PREDICTION_ENVELOPE_DOMAIN = b"lifetwin-v022-scoring-prediction-envelope-v1\0"
...
if _FORMAL_ANALYSIS_COUNTS != _AnalysisCounts(
    ...
    raise RuntimeError("V2.2 formal analysis cardinalities drifted")
if (
    V022_EXPECTED_SEED_ROOTS["random_rankings"] != 202608090210
    or V022_EXPECTED_SEED_ROOTS["bootstrap"] != 202608090211
    or V022_EXPECTED_SEED_ROOTS["stress_permutations"] != 202608090212
):
    raise RuntimeError("V2.2 formal stochastic roots drifted")


@dataclass(frozen=True, slots=True)
class V022ScoringResult:
    """The ten-artifact V2.2 scoring registry in structured form."""
...
class V022PredictionCommitmentEnvelope:
    """Opaque scoring capability bound to verified V2.2 IO evidence."""
    ...
    def __init__(
        ...
        *,
        ...
        config_sha256: str,
        attempt_id: str,
        evidence: V022PredictionCommitmentEvidence,
        ...
        if (
            ...
            or type(self) is not V022PredictionCommitmentEnvelope
        ):
            raise TypeError(
                "V022PredictionCommitmentEnvelope is issued only from an exact "
    ...
    def __setattr__(self, name: str, value: object) -> None:
        raise AttributeError("V2.2 prediction commitment envelopes are immutable")
...
def _sha256(value: object, *, context: str) -> str:
    if not isinstance(value, str) or _SHA256.fullmatch(value) is None:
        raise V022ScoringError(f"{context} must be a lowercase SHA-256")
    return value


def _component(value: object, *, context: str) -> str:
    if not isinstance(value, str) or _COMPONENT.fullmatch(value) is None:
        raise V022ScoringError(f"{context} must be a canonical component")
    return value


def _contract(view: V022ContractView | None = None) -> V022ContractView:
    contract = load_v022_contract_view() if view is None else view
    if type(contract) is not V022ContractView:
        raise V022ScoringError("The V2.2 contract view has an invalid type")
    if (
        contract.protocol.protocol_id != V022_PROTOCOL_ID
        or contract.artifacts.protocol_id != V022_PROTOCOL_ID
        or contract.protocol.config_sha256 != contract.artifacts.config_byte_sha256
    ):
        raise V022ScoringError("The active artifact contract is not exact V2.2")
    return contract
...
def _require_evidence_model_binding(
    evidence: V022PredictionCommitmentEvidence,
    model: V022CommittedModelStateEnvelope,
) -> None:
    entries = {
...
def _issue_prediction_commitment_envelope_v022(
    *,
    prediction_frames: Mapping[str, pd.DataFrame],
    model_state_envelope: V022CommittedModelStateEnvelope,
    prediction_commitment_evidence: V022PredictionCommitmentEvidence,
    formal: bool,
    contract_view: V022ContractView | None = None,
) -> V022PredictionCommitmentEnvelope:
    ...
    try:
        ...
        raise V022ScoringError(str(exc)) from exc
    if (
        model.protocol_id != V022_PROTOCOL_ID
        or model.config_sha256 != view.artifacts.config_byte_sha256
    ):
        raise V022ScoringError("Committed model identity differs from V2.2")
    try:
        evidence = _require_prediction_commitment_evidence_v022(
    ...
    return V022PredictionCommitmentEnvelope(
...
def _require_prediction_envelope(
    ...
    *,
    ...
    if (
        type(value) is not V022PredictionCommitmentEnvelope
        or value._issuer_key is not _PREDICTION_ENVELOPE_KEY
    ):
        raise V022ScoringError("An exact V022PredictionCommitmentEnvelope is required")
    envelope = value
    if (
        envelope.protocol_id != V022_PROTOCOL_ID
        or envelope.config_sha256 != view.artifacts.config_byte_sha256
        or envelope.attempt_id != model.attempt_id
    ):
        raise V022ScoringError("Prediction commitment provenance identity changed")
    try:
        evidence = _require_prediction_commitment_evidence_v022(
            envelope._evidence,
            require_ledger_committed=True,
    ...
    if (
        ...
        raise V022ScoringError("Prediction commitment IO evidence changed")
    _require_evidence_model_binding(evidence, model)
    commitment_hash = _sha256(
    ...
    if tuple(name for name, _, _, _ in metadata) != _PREDICTION_FRAME_FILENAMES:
        raise V022ScoringError("Prediction commitment artifact registry changed")
    ...
    if envelope.provenance_sha256 != expected:
        raise V022ScoringError("Prediction commitment envelope digest changed")
    return envelope
...
def _canonical_committed_equal(
    ...
    *,
    ...
    if canonical_csv_bytes(
        ...
        raise V022ScoringError(
            f"{filename} differs byte-for-byte from V2.2 recomputation"
        )


def validate_and_recompute_committed_predictions_v022(
    *,
    prediction_frames: Mapping[str, pd.DataFrame],
    model_state_envelope: V022CommittedModelStateEnvelope,
    prediction_commitment_envelope: V022PredictionCommitmentEnvelope,
    formal: bool,
    contract_view: V022ContractView | None = None,
    ...
    recomputed = recompute_label_free_pipeline_v022(
...
def _policy_comparison_v022(
    ...
    *,
    ...
    if len(random_rankings) < 1:
        raise V022ScoringError("Random-ranking table is empty")
    random_mean = float(
        pd.to_numeric(
    ...
    if not math.isfinite(random_mean) or random_mean <= 0.0:
        raise V022ScoringError("Random-ranking mean risk is unavailable")
...
def _risk_coverage_curves_v022(test: pd.DataFrame) -> list[dict[str, Any]]:
...
def _isotonic_calibration_diagnostics_v022(
...
def _bootstrap_summary_v022(
    ...
    *,
    ...
    if len(replicates) != expected_count:
        raise V022ScoringError("Bootstrap replicate count changed")
...
def canonicalize_score_frame_v022(
    frame: pd.DataFrame,
    filename: str,
    *,
    ...
    if "protocol_id" in result.columns and not result.empty:
        if set(result["protocol_id"].astype(str)) != {V022_PROTOCOL_ID}:
            raise V022ScoringError(f"{filename} protocol_id changed")
    return result


def canonical_score_csv_bytes_v022(
    frame: pd.DataFrame,
    filename: str,
    *,
    allow_empty: bool = False,
) -> bytes:
    ordered = canonicalize_score_frame_v022(
        frame,
        filename,
        allow_empty=allow_empty,
    )
...
def canonical_result_summary_bytes_v022(payload: Mapping[str, Any]) -> bytes:
    if set(payload) != SCORE_REPORT_KEYS:
        raise V022ScoringError("score_report.json keys changed")
    if payload.get("protocol_id") != V022_PROTOCOL_ID:
        raise V022ScoringError("score_report.json protocol_id changed")
...
def _json_artifact_bytes(
    ...
    *,
    ...
    if set(payload) != expected_keys:
        raise V022ScoringError(f"{filename} keys changed")
    if payload.get("protocol_id") != V022_PROTOCOL_ID:
        raise V022ScoringError(f"{filename} protocol_id changed")
    return canonical_json_bytes(payload)
...
def _payloads_without_manifest(result: V022ScoringResult) -> dict[str, bytes]:
    ...
    for filename, value in _artifacts_without_manifest(result).items():
        if filename.endswith(".csv"):
            if not isinstance(value, pd.DataFrame):
                raise V022ScoringError(f"{filename} must be a dataframe")
            payloads[filename] = canonical_score_csv_bytes_v022(
                value,
                filename,
                allow_empty=allow_empty,
            )
            ...
            payloads[filename] = canonical_result_summary_bytes_v022(value)
    if tuple(payloads) != REQUIRED_SCORE_ARTIFACTS[:-1]:
        raise V022ScoringError("V2.2 score artifact registry changed")
    return payloads
...
def _finalize_registry(
    ...
    *,
    model: V022CommittedModelStateEnvelope,
    prediction: V022PredictionCommitmentEnvelope,
    view: V022ContractView,
) -> V022ScoringResult:
    ...
    manifest = {
        "schema_version": "lifetwin.v022.scored_registry.v1",
        "protocol_id": V022_PROTOCOL_ID,
        "config_sha256": view.artifacts.config_byte_sha256,
        "attempt_id": model.attempt_id,
    ...
    if set(manifest) != _RUN_MANIFEST_KEYS:
        raise V022ScoringError("run_manifest.json keys changed")
    return replace(result, run_manifest=manifest)


def required_score_artifact_payloads_v022(
    result: V022ScoringResult,
) -> dict[str, bytes]:
    """Return all ten committed V2.2 score artifacts as canonical bytes."""

    if type(result) is not V022ScoringResult:
        raise V022ScoringError("An exact V022ScoringResult is required")
    manifest = result.run_manifest
    if not isinstance(manifest, Mapping) or set(manifest) != _RUN_MANIFEST_KEYS:
        raise V022ScoringError("run_manifest.json is not finalized")
    ...
    if (
        manifest.get("schema_version") != "lifetwin.v022.scored_registry.v1"
        or manifest.get("protocol_id") != V022_PROTOCOL_ID
        or manifest.get("config_sha256")
        != load_v022_contract_view().artifacts.config_byte_sha256
        ...
        raise V022ScoringError("run_manifest.json identity or registry changed")
    payloads = _payloads_without_manifest(result)
    artifacts = _artifacts_without_manifest(result)
    expected_records = [
        {
    ...
    if tuple(payloads) != REQUIRED_SCORE_ARTIFACTS:
        raise V022ScoringError("The complete V2.2 score registry changed")
    return payloads
...
def _terminal_result(
    ...
    *,
    ...
    if status_kind not in {"void", "inconclusive_not_success"}:
        raise V022ScoringError("Unknown terminal score status")
    unavailable = (
...
def _score_committed_artifacts_v022(
    *,
    prediction_frames: Mapping[str, pd.DataFrame],
    ...
    decoded, recomputed = validate_and_recompute_committed_predictions_v022(
    ...
    policy_test, test_reductions = _policy_comparison_v022(
        test,
        random_rankings,
        issue_count=TEST_ISSUE_COUNT,
        partition="test",
    ...
    policy_audit, _ = _policy_comparison_v022(
        audit,
        audit_random,
        issue_count=AUDIT_ISSUE_COUNT,
        partition="audit",
    ...
    bootstrap_summary = _bootstrap_summary_v022(
        bootstrap,
        issue_count=TEST_ISSUE_COUNT,
    ...
    risk_coverage_curves = _risk_coverage_curves_v022(test)
    calibration_audit = (
        model_state_envelope.validated_model_state.training_provenance.calibration_audit
    )
    isotonic_diagnostics = _isotonic_calibration_diagnostics_v022(
        trajectories,
        prediction_frames["risk_bundle.csv"],
        decoded,
    ...
    canonical = {
        name: canonicalize_score_frame_v022(frame, name)
        for name, frame in frames.items()
    }
    canonical_result_summary_bytes_v022(score_report)
    result = V022ScoringResult(
...
def score_committed_artifacts(
    *,
    ...
    """Run the formal V2.2 10k/5k/10k committed-artifact scorer."""
    ...
    try:
        return _score_committed_artifacts_v022(
            prediction_frames=prediction_frames,
...
score_committed_artifacts_v022 = score_committed_artifacts
required_score_artifact_payloads = required_score_artifact_payloads_v022
canonicalize_score_frame = canonicalize_score_frame_v022
canonical_score_csv_bytes = canonical_score_csv_bytes_v022
canonical_result_summary_bytes = canonical_result_summary_bytes_v022


def _run_stochastic_fixture_analyses_v022(
    trajectories: pd.DataFrame,
    *,
    issue_count: int,
    counts: _AnalysisCounts,
...
__all__ = [
    ...
    "canonical_result_summary_bytes_v022",
    "canonical_score_csv_bytes",
    "canonical_score_csv_bytes_v022",
    "canonicalize_score_frame",
    "canonicalize_score_frame_v022",
    "required_score_artifact_payloads",
    "required_score_artifact_payloads_v022",
    "score_committed_artifacts",
    "score_committed_artifacts_v022",
    "validate_and_recompute_committed_predictions_v022",
]
