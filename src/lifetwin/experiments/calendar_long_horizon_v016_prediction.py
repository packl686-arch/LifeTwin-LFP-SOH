"""Truth-incapable V2.1 formal fit and prediction entry points.

Public functions accept only sealed capabilities issued by V2.1 IO.  Their
...
class V021PredictionError(RuntimeError):
    """Raised when a sealed V2.1 fit or prediction capability is invalid."""


_FIT_SEAL = object()


class V021VerifiedFitResult:
    ...
    def __init__(
        ...
        *,
        ...
        if _seal is not _FIT_SEAL or type(self) is not V021VerifiedFitResult:
            raise TypeError(
                "Verified formal fit results are issued only by V2.1 prediction"
            )
        ...
        try:
            ...
            raise V021PredictionError(
                "Formal fit result is not a canonical V2.1 artifact"
            ) from exc
...
class V021PredictionWriteResult:
    """Metadata for three exclusively written prediction artifacts."""

    artifacts: tuple[ArtifactMetadata, ...]


def fit_verified_generation_bundle_v021(
    bundle: V021FreshGenerationBundle,
) -> V021VerifiedFitResult:
    """Run the six-worker formal optimizer only on verified fresh generation."""

    from lifetwin.experiments.calendar_long_horizon_v016_fit import (  # noqa: PLC0415
        V021FitError,
        _fit_structure_library_formal_from_verified_frames_v021,
    )
    from lifetwin.experiments.calendar_long_horizon_v016_io import (  # noqa: PLC0415
        V021IOError,
        _extract_fresh_generation_frames_for_formal_fit_v021,
    )

    try:
        prefix, coordinates, contract = (
            _extract_fresh_generation_frames_for_formal_fit_v021(bundle)
        )
        result = _fit_structure_library_formal_from_verified_frames_v021(
            prefix_pack=prefix,
            forecast_coordinates=coordinates,
            contract=contract,
        )
    except (V021FitError, V021IOError) as exc:
        raise V021PredictionError(
            "The sealed fresh-generation fit capability was rejected"
        ) from exc
    return V021VerifiedFitResult(
        _seal=_FIT_SEAL,
        bundle=bundle,
        result=result,
    )
...
def commit_verified_fit_result_v021(
    *,
    bundle: V021FreshGenerationBundle,
    fit_result: V021VerifiedFitResult,
    created_utc: str,
) -> str:
    ...
    from lifetwin.experiments.calendar_long_horizon_v016_io import (  # noqa: PLC0415
        V021IOError,
        _write_verified_fit_outputs_and_commitment_v021,
    )

    if (
        type(fit_result) is not V021VerifiedFitResult
    ...
    try:
        ...
        raise V021PredictionError("The sealed formal fit result changed") from exc
    if observed != fit_result._frame_hashes:
        raise V021PredictionError("The sealed formal fit result changed")
    try:
        return _write_verified_fit_outputs_and_commitment_v021(
            bundle,
...
def run_formal_prediction_v021(
    *,
    label_free_bundle: V021CommittedLabelFreeBundle,
    model_state_envelope: V021CommittedModelStateEnvelope,
) -> V021PredictionWriteResult:
    """Compute and exclusively write outputs without any truth/path capability."""

    from lifetwin.experiments.calendar_long_horizon_v016_io import (  # noqa: PLC0415
        V021IOError,
        _extract_prediction_inputs_v021,
        _write_prediction_outputs_v021,
    )
    from lifetwin.experiments.calendar_long_horizon_v016_pipeline import (  # noqa: PLC0415
        V021PipelineError,
        recompute_label_free_pipeline_v021,
    )

    try:
        frames, contract = _extract_prediction_inputs_v021(
            label_free_bundle,
            model_state=model_state_envelope,
        )
        output = recompute_label_free_pipeline_v021(
        ...
        artifacts = _write_prediction_outputs_v021(
            label_free_bundle,
            frames={
    ...
    except (V021IOError, V021PipelineError) as exc:
        raise V021PredictionError(
            "Formal V2.1 prediction capability was rejected"
        ) from exc
    return V021PredictionWriteResult(artifacts=artifacts)


def run_isolated_prediction_process_v021(
    *,
    ...
    from lifetwin.experiments.calendar_long_horizon_v016_prediction_capsule import (  # noqa: PLC0415
        V021PredictionCapsuleError,
        load_prediction_bundle,
        run_prediction_bundle,
        write_prediction_outputs,
    )
    from lifetwin.experiments.calendar_long_horizon_v016_prediction_environment import (  # noqa: PLC0415
...
__all__ = [
    ...
    "run_isolated_prediction_process_v021",
    "run_formal_prediction_v021",
]
