"""One-shot, post-freeze generation for the V2.4 synthetic protocol.

The public lifecycle has two explicit capabilities.  The first performs and
commits the complete RNG-free V2.4/V2 coordinate audit.  The second verifies
that exact commitment before it can consume the first V2.4 seed.  Neither
...
class V024GenerationError(ValueError):
    """Raised when the V2.4 generation lifecycle cannot advance exactly."""


@dataclass(frozen=True)
class WrittenV024GenerationArtifacts:
    """Metadata for one committed V2.4 generation bundle."""
...
def _is_reparse_entry(path: Path) -> bool:
    try:
        ...
        raise V024GenerationError(f"Cannot inspect formal path: {path}") from exc
...
def _physical_root_identity(
    ...
    *,
    ...
    try:
        ...
        raise V024GenerationError(f"Cannot bind {context} root identity") from exc
    if resolved != path:
        raise V024GenerationError(f"{context} root changed during resolution")
...
def _verify_physical_root_identity(
    ...
    *,
    ...
    if observed != identity:
        raise V024GenerationError(f"{context} directory identity changed")
...
def _require_exact_direct_files(
    ...
    *,
    ...
    if not root.is_dir() or _is_reparse_entry(root):
        raise V024GenerationError(f"{context} root is not a physical directory")
...
def _write_prepared_generation_artifacts_resumable(
    ...
    *,
    ...
    if (
        ...
        raise V024GenerationError("Prepared artifact membership or order changed")
...
def _decode_canonical_plan_commitment(raw: bytes) -> Mapping[str, object]:
    ...
    if not isinstance(decoded, Mapping):
        raise V024GenerationError("Generation-plan commitment must be an object")
    canonical = (
        json.dumps(
            decoded,
            ensure_ascii=True,
    ...
    if raw != canonical:
        raise V024GenerationError("Generation-plan commitment bytes are not canonical")
    return decoded
...
def _validate_attempt_identities(
    ...
    *,
    ...
    if not states:
        raise V024GenerationError("Formal exposure ledger contains no attempt")
...
def _select_generation_candidate(
    ...
    *,
    ...
    if len(candidates) != 1:
        raise V024GenerationError(
            "Exactly one plan-committed attempt may begin V2.4 generation"
        )
    if any(
        state.identity.attempt_id != candidates[0].identity.attempt_id
        and not state.terminal_failed
...
def commit_frozen_v024_generation_plan(
    *,
    ...
    except (OSError, V024FirewallError) as exc:
        raise V024GenerationError(
    ...
    append_formal_exposure_event(
        ...
        truth_commitments_byte_sha256=None,
        prediction_commitment_byte_sha256=None,
        message="Started the exhaustive RNG-free V2.4/V2 coordinate audit.",
    )
    try:
        commitment = audit_formal_v024_generation_plan(view)
...
def _verify_committed_generation_plan(
    *,
    ...
    except V024CollisionError as exc:
        raise V024GenerationError(
...
def generate_frozen_v024_artifacts(
    *,
    label_free_root: str | Path,
    sealed_truth_root: str | Path,
    _contract_view: V024ContractView | None = None,
    ...
    try:
        ...
        raise V024GenerationError(
            "Formal exposure ledger failed before V2.4 generation"
        ) from exc
    identity = _identity(progress)
    ...
    try:
        ...
        _append_failure(
            ...
            view=view,
            phase="truth_committed",
            message="V2.4 generation did not reach its truth commitment.",
        )
        raise
    ...
    append_formal_exposure_event(
        ...
        prediction_commitment_byte_sha256=None,
        message="All nine fresh V2.4 sealed files were committed before truth access.",
    )
    return WrittenV024GenerationArtifacts(
