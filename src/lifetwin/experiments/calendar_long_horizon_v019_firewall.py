"""V2.4 formal-attempt state machine and phase-scoped truth capabilities.

V2.4 inserts two outcome-free commitments into the inherited V2 lifecycle:
...
class V024FirewallError(ValueError):
    """Raised when a V2.4 attempt crosses a frozen information boundary."""
...
def _sha256_file(path: str | Path) -> str:
    try:
        ...
        raise V024FirewallError(f"Cannot read committed artifact: {path}") from exc
...
def _is_reparse_entry(path: Path) -> bool:
    try:
        ...
        raise V024FirewallError(f"Cannot inspect formal path: {path}") from exc
...
def _physical_root(raw_path: str | Path, *, context: str) -> Path:
    ...
    for component in (*reversed(path.parents), path):
        if os.path.lexists(component) and _is_reparse_entry(component):
            raise V024FirewallError(f"{context} root has a reparse-point ancestor")
...
def _direct_physical_child(
    ...
    *,
    ...
    if candidate != expected:
        raise V024FirewallError(f"{filename} must be a direct label-free artifact")
    if not os.path.lexists(candidate):
        if must_exist:
            raise V024FirewallError(f"{context} is absent")
        return candidate
    if _is_reparse_entry(candidate) or not candidate.is_file():
        raise V024FirewallError(f"{context} is not a direct regular file")
    try:
        if candidate.resolve(strict=True) != candidate:
            raise V024FirewallError(f"{context} changed during resolution")
    except OSError as exc:
        raise V024FirewallError(f"Cannot bind {context}") from exc
    return candidate
    ...
    *,
    ...
    def reject_duplicates(pairs: list[tuple[str, Any]]) -> dict[str, Any]:
        ...
        for key, value in pairs:
            if key in decoded:
                raise V024FirewallError(f"{filename} contains duplicate key {key!r}")
            decoded[key] = value
        return decoded

    def reject_constant(token: str) -> None:
        raise V024FirewallError(f"{filename} contains nonfinite token {token}")
    ...
    try:
        ...
        raise V024FirewallError(f"{filename} is not strict JSON") from exc
    if not isinstance(payload, dict):
        raise V024FirewallError(f"{filename} must contain one JSON object")
    if compact:
        try:
            canonical = (
                json.dumps(
            ...
            raise V024FirewallError(f"{filename} is not finite ASCII JSON") from exc
    else:
        try:
            canonical = canonical_json_bytes(payload)
        except V015ArtifactError as exc:
            raise V024FirewallError(f"{filename} is not finite JSON") from exc
    if raw != canonical:
        raise V024FirewallError(f"{filename} is not canonical JSON")
    return payload


def _require_sha256(value: object, *, context: str) -> str:
    if not isinstance(value, str) or _SHA256.fullmatch(value) is None:
        raise V024FirewallError(f"{context} is not lowercase SHA256")
    return value
...
def _artifact_path(
    *,
    ...
    if not os.path.lexists(candidate):
        raise V024FirewallError(f"Committed input is absent: {filename}")
    if _is_reparse_entry(candidate) or not candidate.is_file():
        raise V024FirewallError(
...
def validate_formal_exposure_events(
    ...
    *,
    ...
    try:
        ...
        raise V024FirewallError(str(exc)) from exc
...
def append_formal_exposure_event(
    *,
    ...
    """Validate a prospective V2.4 transition before appending its bytes."""
...
def _require_direct_commitment(
    *,
    ...
    if path is None or expected_sha256 is None:
        raise V024FirewallError(f"{context} lacks its checkpointed commitment")
    ...
    if _sha256_file(resolved) != expected_sha256:
        raise V024FirewallError(f"{context} differs from its ledger commitment")
    return resolved
...
def _verify_generation_plan(
    *,
    ...
    try:
        ...
        recomputed = audit_formal_v024_generation_plan(view)
    except V024FirewallError:
        raise
    except (TypeError, ValueError) as exc:
        raise V024FirewallError("Generation-plan semantic verification failed") from exc
...
def _verify_fit_commitment(
    *,
    ...
    if (
        ...
        raise V024FirewallError("fit_commitment.json identity changed")
    entries = payload.get("files")
    if not isinstance(entries, list) or len(entries) != len(_FIT_COMMITMENT_FILENAMES):
        raise V024FirewallError("fit_commitment.json file registry changed")
    for filename, entry in zip(
        ...
        if (
            ...
            raise V024FirewallError("A label-free fit input changed after commitment")
...
def _verify_input_hashes(
    ...
    *,
    ...
    if not isinstance(value, Mapping) or set(value) != set(expected_filenames):
        raise V024FirewallError(f"{context} input registry changed")
...
def _verify_center_checkpoint(
    *,
    ...
    if (
        ...
        raise V024FirewallError("center_state_checkpoint.json is invalid")
    _require_sha256(
...
def _verify_training_manifest(
    *,
    ...
    if _sha256_file(path) != risk_payload.get("training_manifest_byte_sha256"):
        raise V024FirewallError("Training manifest differs from the risk checkpoint")
    payload = _strict_json_object(
        path.read_bytes(),
        filename=path.name,
        compact=False,
    ...
    try:
        ...
        raise V024FirewallError(
            "V2.4 contract lacks the training-manifest schema"
        ) from exc
    if (
        ...
        raise V024FirewallError("training_manifest.json semantics changed")
...
def _verify_risk_checkpoint(
    *,
    ...
    if (
        ...
        raise V024FirewallError("risk_state_checkpoint.json is invalid")
    _require_sha256(
...
def _verify_mask_commitment(
    *,
    ...
    try:
        ...
        deserialize_calibration_mask_commitment_json_v024(committed.read_bytes())
    except (TypeError, ValueError) as exc:
        raise V024FirewallError(
...
def open_truth_for_phase(
    *,
    ...
    if prior.identity != identity:
        raise V024FirewallError("Truth capability identity differs from the ledger")
...
def reopen_authorized_truth_for_recovery(
    *,
    ...
    try:
        ...
        raise V024FirewallError("Recovery attempt is absent from the ledger") from exc
    ...
    if not set(requested).issubset(progress.opened_truth_files):
        raise V024FirewallError("Recovery requested truth that was not already exposed")
    ...
    for filename in requested:
        ...
        if (
            ...
            raise V024FirewallError(f"{filename} differs from its recovery commitment")
        opened[filename] = frame
    return opened
...
def verify_phase_artifact_commitment(
    ...
    *,
    ...
    if expected is None:
        raise V024FirewallError(f"{phase} has no completed commitment")
    observed = _sha256_file(artifact_path)
    if observed != expected:
        raise V024FirewallError(f"{phase} artifact differs from its ledger commitment")
    return observed
