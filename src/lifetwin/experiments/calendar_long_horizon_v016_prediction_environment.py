"""Standalone formal-environment attester for the V2.1 prediction capsule.

This module deliberately does not import the V2.1 protocol, contract, training,
...
class V021PredictionEnvironmentError(RuntimeError):
    """Raised when the prediction process is not the attested V2.1 build."""
...
_LOCK_RELATIVE_PATH = Path("requirements") / "v021-formal.txt"
_LOCK_BYTE_SHA256 = "c9f906589332ca4afbdb87ae9ff11e5876409137d1c3e1e186bede873180e6f1"
...
_PREREG_RELATIVE_PATH = (
    Path("reports") / "synthetic_long_horizon_identifiability_prereg_v2_1.md"
)
_FREEZE_RECORD_RELATIVE_PATH = (
    Path("reports") / "synthetic_long_horizon_identifiability_freeze_record_v2_1.json"
)
_IMPLEMENTATION_AUDIT_RELATIVE_PATH = (
    Path("reports")
    / "synthetic_long_horizon_identifiability_implementation_audit_v2_1.md"
)
...
_PREREG_BYTE_SHA256 = "ae346fe8aa5699a7b3ad0d124e8ec29851a16fcbf485309e8c3c441749540efc"
_DESIGN_FREEZE_COMMIT = "01c75354b22c51716bba16b3e4f11e2ca30c6aad"
_EXPECTED_HASH_SENTINEL = 2327476529618929314
_THREAD_ENVIRONMENT = (
    "OMP_NUM_THREADS",
...
def _git(repo_root: Path, *arguments: str) -> str:
    try:
        ...
        raise V021PredictionEnvironmentError(
...
def _reject_duplicate_keys(
    ...
    for key, value in pairs:
        if key in result:
            raise V021PredictionEnvironmentError(f"Duplicate JSON key: {key}")
        result[key] = value
    return result


def _reject_json_constant(token: str) -> None:
    raise V021PredictionEnvironmentError(f"Nonfinite JSON constant: {token}")
...
def _strict_json_object(path: Path, *, context: str) -> dict[str, object]:
    try:
        ...
        raise V021PredictionEnvironmentError(f"{context} is invalid JSON") from exc
    if not isinstance(decoded, dict):
        raise V021PredictionEnvironmentError(f"{context} must be a JSON object")
    return decoded
...
def _amendment_semantic_sha256(payload: Mapping[str, Any]) -> str:
    """Hash all amendment content except its paired implementation switch."""
    ...
    try:
        ...
        raise V021PredictionEnvironmentError(
            "V2.1 amendment cannot be canonicalized"
        ) from exc
    if not isinstance(normalized, dict):
        raise V021PredictionEnvironmentError("V2.1 amendment must be a JSON object")
    generation = _json_object(
        normalized.get("fresh_generation"),
        context="V2.1 fresh_generation",
    )
    normalized["status"] = "<IMPLEMENTATION_STATUS>"
    generation["implementation_exists"] = "<IMPLEMENTATION_EXISTS>"
...
def _validate_frozen_amendment(path: Path) -> str:
    payload = _strict_json_object(path, context="V2.1 amendment")
    if set(payload) != _AMENDMENT_TOP_LEVEL_KEYS:
        raise V021PredictionEnvironmentError("V2.1 amendment keys changed")
    if (
        payload.get("schema_version") != "1.0.0"
        or payload.get("protocol_id") != V021_PROTOCOL_ID
    ):
        raise V021PredictionEnvironmentError("V2.1 amendment identity changed")
    if payload.get("status") != "implementation_frozen":
        raise V021PredictionEnvironmentError(
            "V2.1 amendment has not entered immutable implementation_frozen status"
        )
    generation = _json_object(
        payload.get("fresh_generation"),
        context="V2.1 fresh_generation",
    )
    if generation.get("implementation_exists") is not True:
        raise V021PredictionEnvironmentError(
            "V2.1 amendment does not attest that the implementation exists"
        )
    if generation.get("generation_has_started") is not False:
        raise V021PredictionEnvironmentError(
            "V2.1 frozen amendment unexpectedly records prior generation"
        )
    if _amendment_semantic_sha256(payload) != V021_AMENDMENT_SEMANTIC_SHA256:
        raise V021PredictionEnvironmentError(
            "V2.1 amendment semantic content changed outside the status switch"
        )
...
def _source_hashes(repo_root: Path) -> Mapping[str, str]:
    ...
    amended = sorted(experiment_root.glob("calendar_long_horizon_v016_*.py"))
    shared = experiment_root / "calendar_long_horizon_synthetic.py"
    scripts = sorted((repo_root / "scripts").glob("*v016*.py"))
    if not inherited or not amended or not shared.is_file():
        raise V021PredictionEnvironmentError(
            "The complete V2.1 and inherited V2 implementation source set is absent"
        )
    paths = tuple(sorted({*inherited, *amended, shared, *scripts}))
    if any(not path.is_file() for path in paths):
        raise V021PredictionEnvironmentError(
            "A V2.1 implementation source is not a file"
        )
...
def _validate_freeze_record(
    ...
    *,
    ...
    payload = _strict_json_object(path, context="V2.1 freeze record")
    expected = {
    ...
    if any(payload.get(key) != value for key, value in expected.items()):
        raise V021PredictionEnvironmentError(
            "V2.1 freeze record does not bind the frozen protocol and environment"
        )
    ...
    if (
        ...
        raise V021PredictionEnvironmentError(
            "V2.1 freeze record does not bind the frozen implementation"
        )
...
def _runtime_hash_sentinel() -> int:
    return hash("lifetwin-v021-formal")
...
def verify_prediction_environment(
    ...
    if _FULL_GIT_HASH.fullmatch(git_commit) is None:
        raise V021PredictionEnvironmentError("Git did not return a full commit hash")
    if _git(root, "status", "--porcelain=v1", "--untracked-files=all"):
        raise V021PredictionEnvironmentError("Formal implementation worktree is dirty")
    ...
    if _sha256_path(prereg_path) != _PREREG_BYTE_SHA256:
        raise V021PredictionEnvironmentError("V2.1 preregistration hash changed")
    if _sha256_path(lock_path) != _LOCK_BYTE_SHA256:
        raise V021PredictionEnvironmentError("V2.1 environment lock hash changed")
    ...
    if _git(root, "rev-parse", "HEAD^") != implementation_commit:
        raise V021PredictionEnvironmentError(
            "Execution commit is not the direct V2.1 implementation attestation"
        )
...
__all__ = [
    "PredictionEnvironmentIdentity",
    "V021PredictionEnvironmentError",
