"""Non-generative V2.2 adapter for the frozen V2 protocol contracts.

The V2.2 amendment changes identity, generation roots, and calibration handling.
This module adapts only the identity/configuration surface needed by later V2.2
...
class V022ContractError(ValueError):
    """Raised when a V2.2 contract view would weaken frozen inheritance."""


@dataclass(frozen=True, slots=True)
class V022ContractView:
    """Immutable, non-generative V2.2 view over the two frozen V2 contracts.
    ...
    reject the V2.2 identity and amendment hash.
    """
...
def _file_sha256(path: Path, *, context: str) -> str:
    try:
        ...
        raise V022ContractError(f"Cannot read {context}") from exc
    return hashlib.sha256(raw).hexdigest()


def _require_exact_type(value: object, expected: type[object], *, context: str) -> None:
    if type(value) is not expected:
        raise V022ContractError(f"{context} must have exact type {expected.__name__}")
...
def _base_seed_roots(protocol: ValidatedV015Protocol) -> tuple[tuple[str, int], ...]:
    ...
    if (
        ...
        raise V022ContractError("Frozen V2 seed-root registry is not exact")
    return roots
...
def _validate_base_contracts(
    ...
    if V2_PROTOCOL_ID != V2_ARTIFACT_PROTOCOL_ID:
        raise V022ContractError("Imported V2 protocol identities disagree")
    if protocol.protocol_id != V2_PROTOCOL_ID:
        raise V022ContractError("Base protocol is not the frozen V2 identity")
    if artifacts.protocol_id != V2_PROTOCOL_ID:
        raise V022ContractError("Base artifact contract is not the frozen V2 identity")
    if protocol.config_sha256 != FROZEN_CONFIG_CANONICAL_SHA256:
        raise V022ContractError("Base protocol canonical commitment drifted")
    if artifacts.config_byte_sha256 != FROZEN_CONFIG_BYTE_SHA256:
        raise V022ContractError("Base artifact byte commitment drifted")
    if artifacts.schema_version != FROZEN_SCHEMA_VERSION:
        raise V022ContractError("Base artifact schema version drifted")
    if (
        _file_sha256(artifacts.config_path, context="frozen V2 config")
        != FROZEN_CONFIG_BYTE_SHA256
    ):
        raise V022ContractError("Base artifact config bytes drifted")
    if protocol.prefix_days != artifacts.prefix_days:
        raise V022ContractError("Base prefix grids disagree")
    if protocol.forecast_days != artifacts.forecast_days:
        raise V022ContractError("Base forecast grids disagree")
    ...
    for partition, family_counts in partition_counts.items():
        ...
        if artifacts.partition_member_counts.get(partition) != expected:
            raise V022ContractError(
                f"Base member count disagrees for partition {partition}"
            )

    config = protocol.config()
    if config.get("protocol_id") != V2_PROTOCOL_ID:
        raise V022ContractError("Base protocol JSON identity drifted")
    design_partitions = config.get("design_partitions")
    if not isinstance(design_partitions, Mapping):
        raise V022ContractError("Base design_partitions is not an object")
    raw_roots = design_partitions.get("seed_roots")
    if not isinstance(raw_roots, Mapping):
        raise V022ContractError("Base seed_roots is not an object")
    if tuple(raw_roots.items()) != _base_seed_roots(protocol):
        raise V022ContractError("Base protocol JSON and parsed seed roots disagree")


def _validate_design_identity(design: ValidatedV022Design) -> None:
    _require_exact_type(design, ValidatedV022Design, context="design")
    if design.protocol_id != V022_PROTOCOL_ID:
        raise V022ContractError("V2.2 design identity drifted")
    if design.status not in V022_ALLOWED_DESIGN_STATUSES:
        raise V022ContractError("V2.2 design status is not adapter-safe")
    if design.config_path != design.config_path.resolve():
        raise V022ContractError("V2.2 amendment path must be absolute")
    if (
        _file_sha256(design.config_path, context="V2.2 amendment")
        != design.config_byte_sha256
    ):
        raise V022ContractError("V2.2 amendment byte commitment drifted")
    expected_roots = tuple(V022_EXPECTED_SEED_ROOTS.items())
    observed_roots = tuple(design.seed_roots.items())
    if observed_roots != expected_roots:
        raise V022ContractError("V2.2 fresh seed roots drifted")
    if len(observed_roots) != 13 or len({root for _, root in observed_roots}) != 13:
        raise V022ContractError("V2.2 must have thirteen distinct fresh roots")


def _adapt_config_json(
    base_protocol: ValidatedV015Protocol,
    design: ValidatedV022Design,
) -> str:
    ...
    if not isinstance(design_partitions, dict):
        raise V022ContractError("Parsed base design_partitions is not mutable JSON")
    ...
    if adapted != expected:
        raise V022ContractError("V2.2 config adaptation changed an undeclared field")
    ...
    if not isinstance(restored_partitions, dict):
        raise V022ContractError("Adapted design_partitions is not an object")
    restored_partitions["seed_roots"] = dict(_base_seed_roots(base_protocol))
    if restored != base_for_comparison:
        raise V022ContractError("V2.2 config is not an exact two-field V2 adaptation")
...
def _assert_only_allowed_dataclass_changes(
    ...
    *,
    ...
    if type(base) is not type(adapted):
        raise V022ContractError(f"{context} type changed")
...
def _validate_adapted_view(view: V022ContractView) -> None:
    _require_exact_type(
    ...
    if view.protocol.protocol_id != V022_PROTOCOL_ID:
        raise V022ContractError("Adapted protocol identity is not V2.2")
    if view.artifacts.protocol_id != V022_PROTOCOL_ID:
        raise V022ContractError("Adapted artifact identity is not V2.2")
    if view.protocol.config_sha256 != view.artifacts.config_byte_sha256:
        raise V022ContractError("Adapted config commitments disagree")
    if view.protocol.config_sha256 != _file_sha256(
        view.artifacts.config_path, context="adapted amendment"
    ):
        raise V022ContractError("Adapted config commitment is not the amendment")
    if view.protocol.seed_roots != tuple(V022_EXPECTED_SEED_ROOTS.items()):
        raise V022ContractError("Adapted protocol roots are not the fresh V2.2 roots")
    if view.protocol.prefix_days != view.artifacts.prefix_days:
        raise V022ContractError("Adapted prefix grids disagree")
    if view.protocol.forecast_days != view.artifacts.forecast_days:
        raise V022ContractError("Adapted forecast grids disagree")
    if view.design_status not in V022_ALLOWED_DESIGN_STATUSES:
        raise V022ContractError("Adapted design status drifted")
    if view.base_config_canonical_sha256 != FROZEN_CONFIG_CANONICAL_SHA256:
        raise V022ContractError("Base canonical provenance drifted")
    if view.base_config_byte_sha256 != FROZEN_CONFIG_BYTE_SHA256:
        raise V022ContractError("Base byte provenance drifted")

    adapted_config = view.protocol.config()
    if adapted_config.get("protocol_id") != V022_PROTOCOL_ID:
        raise V022ContractError("Adapted protocol JSON identity drifted")
    design_partitions = adapted_config.get("design_partitions")
    if not isinstance(design_partitions, Mapping):
        raise V022ContractError("Adapted design_partitions is not an object")
    roots = design_partitions.get("seed_roots")
    if not isinstance(roots, Mapping):
        raise V022ContractError("Adapted seed_roots is not an object")
    if tuple(roots.items()) != view.protocol.seed_roots:
        raise V022ContractError("Adapted JSON and parsed roots disagree")


def adapt_v022_contract_view(
    *,
    design: ValidatedV022Design,
    base_protocol: ValidatedV015Protocol,
    base_artifacts: FrozenArtifactContract,
) -> V022ContractView:
    """Construct an immutable V2.2 view without deriving or consuming any seed."""
    ...
    if tuple(name for name, _ in fresh_roots) != base_names:
        raise V022ContractError("V2.2 seed-root registry names or order changed")
    if {root for _, root in fresh_roots}.intersection(
        root for _, root in base_protocol.seed_roots
    ):
        raise V022ContractError("V2.2 roots collide with frozen V2 roots")
...
def load_v022_contract_view(
    amendment_path: str | Path = DEFAULT_V022_AMENDMENT_PATH,
    base_config_path: str | Path = DEFAULT_V2_CONFIG_PATH,
) -> V022ContractView:
    """Validate both committed configs and return the non-generative V2.2 view."""

    design = load_v022_design(amendment_path)
    base_protocol = load_frozen_protocol_config(base_config_path)
    base_artifacts = load_artifact_contract(base_config_path)
    return adapt_v022_contract_view(
        design=design,
