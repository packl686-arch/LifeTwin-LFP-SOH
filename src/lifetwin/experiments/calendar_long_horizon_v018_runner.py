"""One-shot, phase-isolated orchestration for the frozen V2.3 experiment.
...
from lifetwin.experiments.calendar_long_horizon_v018_generation import (
...
from lifetwin.experiments.calendar_long_horizon_v018_prediction import (
...
from lifetwin.experiments.calendar_long_horizon_v018_provenance import (
    ...
    _issue_v023_training_provenance_from_fresh_bytes,
)
from lifetwin.experiments.calendar_long_horizon_v018_scoring import (
    REQUIRED_SCORE_ARTIFACTS,
    _issue_prediction_commitment_envelope_v023,
...
from lifetwin.experiments.calendar_long_horizon_v018_training import (
    ...
    derive_calibration_mask_commitment_v023,
    fit_calibration_development_state_v023,
)


_PROJECT_ROOT = Path(__file__).resolve().parents[3]
_FORMAL_SCRIPT = _PROJECT_ROOT / "scripts" / "run_calendar_long_horizon_v018.py"
_LABEL_INPUTS = (
    "prefix_pack.csv",
...
_FORMAL_ATTEMPT_ID = "v023-formal-20260810-a1"


class V023RunnerError(RuntimeError):
    ...
    *,
    ...
    for candidate in (lexical, *lexical.parents):
        if os.path.lexists(candidate) and _is_reparse(candidate):
            raise V023RunnerError(f"{context} root traverses a reparse point")


def _require_physical_directory(root: Path, *, context: str) -> None:
    if not root.is_dir() or _is_reparse(root):
        raise V023RunnerError(f"{context} root is not a physical directory")
...
def _prepare_empty_physical_root(root: Path, *, context: str) -> None:
    ...
    if any(root.iterdir()):
        raise V023RunnerError(f"{context} root must be completely empty")


def _direct_child(root: Path, filename: str) -> Path:
    if Path(filename).name != filename or filename in {"", ".", ".."}:
        raise V023RunnerError(f"Unsafe artifact filename: {filename!r}")
    child = (root / filename).resolve()
    if child.parent != root.resolve():
        raise V023RunnerError(f"Artifact escaped its root: {filename}")
    return child
...
def _exclusive_create(path: Path, raw: bytes) -> None:
    ...
    try:
        ...
        raise V023RunnerError(f"Formal artifact already exists: {path}") from exc
    except OSError as exc:
        raise V023RunnerError(f"Could not create formal artifact: {path}") from exc
    finally:
        try:
            temporary.unlink()
    ...
    if path.read_bytes() != raw:
        raise V023RunnerError(f"Formal artifact changed after write: {path}")
...
def _write_json_checkpoint(
    ...
    *,
    ...
    if (
        ...
        raise V023RunnerError(f"{path.name} identity or schema changed")
...
def initialize_formal_attempt(
    *,
    paths: V023RunPaths,
    attempt_id: str,
) -> tuple[FormalEnvironmentIdentity, V023ContractView, FormalAttemptIdentity]:
    """Attest the environment and create the sole initial ledger event."""

    if attempt_id != _FORMAL_ATTEMPT_ID:
        raise V023RunnerError(f"attempt_id must equal {_FORMAL_ATTEMPT_ID}")
    ...
    for context, expected in expected_roots.items():
        ...
        if actual != expected.resolve():
            raise V023RunnerError(f"{context} root is not the frozen V2.3 path")
        if actual.exists():
            raise V023RunnerError(f"{context} root must not exist before launch")
    ...
    if environment.config_byte_sha256 != view.artifacts.config_byte_sha256:
        raise V023RunnerError("Environment and V2.3 contract hashes differ")
    for root, context in (
    ...
    _append_phase(
        ...
        message="Clean frozen V2.3 environment verified before generation.",
    )
...
def run_isolated_generation_stage(
    *,
    ...
    commit_frozen_v023_generation_plan(label_free_root=label_free_root)
    generate_frozen_v023_artifacts(
...
def _run_checked_process(arguments: Sequence[str], *, context: str) -> None:
    try:
        ...
        raise V023RunnerError(f"Could not launch {context}") from exc
    if completed.returncode != 0:
        raise V023RunnerError(f"{context} exited with status {completed.returncode}")


def _launch_generation_process(paths: V023RunPaths) -> None:
    _run_checked_process(
        ...
        context="isolated V2.3 generation process",
    )
...
def _launch_prediction_process(
    *,
    ...
    _run_checked_process(
        ...
        context="truth-incapable V2.3 prediction process",
    )
...
def _commit_actual_analysis_hash_ledger(
    *,
    ...
    try:
        ...
        digest = create_actual_analysis_hash_ledger_commitment_v023(
        ...
        if _file_hash(artifact) != digest:
            raise V023RunnerError("Actual-analysis commitment changed after creation")
        _append_phase(
            paths=paths,
            identity=identity,
            contract=view.artifacts,
...
def _fit_structure_stage(
    *,
    ...
    try:
        bundle = load_fresh_generation_bundle_v023(
            label_free_root=paths.label_free_root,
            attempt_id=identity.attempt_id,
            contract_view=view,
        )
        fitted = fit_verified_generation_bundle_v023(bundle)
        digest = commit_verified_fit_result_v023(
            bundle=bundle,
            fit_result=fitted,
            created_utc=_utc_now(),
        )
...
def _apply_partition(
    ...
    *,
    ...
    pipeline = recompute_validated_partition_with_state_v023(
        partition_view,
...
def _ordered_cluster_ids(
    ...
    *,
    ...
    if content.empty or content["cluster_id"].duplicated().any():
        raise V023RunnerError(f"{partition} predictor-content rows are incomplete")
...
def _feature_rows(
    ...
    *,
    ...
    if rows.index.has_duplicates or rows.isna().all(axis=1).any():
        raise V023RunnerError(f"{partition} feature rows are incomplete")
...
def _fit_risk_stage(
    *,
    ...
    if (
        ...
        raise V023RunnerError("Center checkpoint changed before risk reveal")
    ...
    try:
        ...
        training_raw = serialize_training_manifest_json_v023(
...
def _calibration_evidence(
    *,
    ...
    if contents.index.has_duplicates or contents.isna().any(axis=None):
        raise V023RunnerError("Calibration predictor-content rows are incomplete")
    risks = (
    ...
    if not required_scores.issubset(risks.columns) or risks.loc[
        ...
        raise V023RunnerError("Calibration primary-risk rows are incomplete")
...
def _fit_calibration_stage(
    *,
    ...
    if (
        ...
        raise V023RunnerError("Risk checkpoint changed before mask derivation")
    ...
    try:
        mask = derive_calibration_mask_commitment_v023(**evidence_kwargs)
        mask_raw = serialize_calibration_mask_commitment_json_v023(mask)
        _exclusive_create(
    ...
    try:
        ...
        calibration, audit = fit_calibration_development_state_v023(
        ...
        calibration_raw = serialize_calibration_manifest_json_v023(
        ...
        provenance = _issue_v023_training_provenance_from_fresh_bytes(
            contract_view=view,
            attempt_id=identity.attempt_id,
        ...
        audit_raw = serialize_calibration_population_audit_json_v023(
        ...
        model_raw = serialize_model_state_json_v023(
            provenance,
...
def _prediction_and_commitment(
    *,
    ...
    try:
        bundle: V023CommittedLabelFreeBundle = load_committed_label_free_bundle_v023(
            label_free_root=paths.label_free_root,
            attempt_id=identity.attempt_id,
            contract_view=view,
        )
        ...
        precommit_evidence = create_prediction_commitment_v023(
            bundle,
            created_utc=_utc_now(),
        )
    ...
    evidence = verify_prediction_commitment_v023(
        label_free_root=paths.label_free_root,
        attempt_id=identity.attempt_id,
        contract_view=view,
    ...
    prediction_envelope = _issue_prediction_commitment_envelope_v023(
...
def _score_and_write(
    *,
    ...
    try:
        ...
        payloads = required_score_artifact_payloads_v023(result)
        if tuple(payloads) != tuple(REQUIRED_SCORE_ARTIFACTS):
            raise V023RunnerError("Scorer returned a changed artifact registry")
        ...
        if tuple(sorted(path.name for path in paths.score_root.iterdir())) != (
            ...
            raise V023RunnerError("Persisted score registry is incomplete")
        _append_phase(
            paths=paths,
            identity=identity,
            contract=view.artifacts,
...
def run_formal_attempt(
    *,
    ...
    """Execute the fixed V2.3 lifecycle without a scientific override surface."""

    started = time.monotonic()
    attempt_created_utc = _utc_now()
    paths = V023RunPaths.resolve(
        repo_root=repo_root,
        label_free_root=label_free_root,
        sealed_truth_root=sealed_truth_root,
    ...
    try:
        if view.design_status != "implementation_frozen":
            raise V023RunnerError("V2.3 design is not implementation_frozen")
        ...
        if (
            ...
            raise V023RunnerError("Isolated generation did not commit all truth")
        truth_hash = progress.truth_commitments_byte_sha256
        if _file_hash(paths.truth_commitment_path) != truth_hash:
            raise V023RunnerError("Truth commitment differs from the exposure ledger")
        ...
        if (
            ...
            raise V023RunnerError("Formal attempt did not reach scoring_completed")
    except BaseException as exc:
