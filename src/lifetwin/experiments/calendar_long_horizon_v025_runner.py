"""Fixed V2.10 profile over the shared V0.20 lifecycle implementation."""
...
_FORMAL_SCRIPT = _PROJECT_ROOT / "scripts" / "run_calendar_long_horizon_v025.py"


def run_isolated_generation_stage_v030(
    *,
    label_free_root: str | Path,
    sealed_truth_root: str | Path,
) -> None:
    """Run generation with the one authenticated V2.10 identity."""

    run_isolated_generation_stage(
        label_free_root=label_free_root,
        sealed_truth_root=sealed_truth_root,
        _contract_view=load_v030_contract_view(),
        _environment_verifier=verify_formal_environment_v030,
        _implementable_status=V030_DESIGN_STATUS,
    )


def run_isolated_prediction_process_v030(
    *,
    label_free_root: str | Path,
    attempt_id: str,
    repo_root: str | Path,
) -> V024PredictionWriteResult:
    """Run the truth-incapable prediction capsule under the V2.10 attester."""

    if attempt_id != V030_ONLY_ATTEMPT_ID:
        raise ValueError("Prediction attempt differs from the fixed V2.10 identity")
    return run_isolated_prediction_process_v024(
        label_free_root=label_free_root,
        attempt_id=attempt_id,
        repo_root=repo_root,
        _environment_verifier=verify_prediction_environment_v030,
        _input_filenames_by_stage=INPUT_FILENAMES_BY_STAGE,
    )


def run_formal_attempt_v030(
    *,
    attempt_id: str,
    label_free_root: str | Path,
    sealed_truth_root: str | Path,
    ...
    """Execute the sole V2.10 formal profile without a scientific override."""
    ...
    return run_formal_attempt(
        ...
        repo_root=repo_root,
        _contract_view=load_v030_contract_view(),
        _formal_attempt_id=V030_ONLY_ATTEMPT_ID,
...
__all__ = [
    "run_formal_attempt_v030",
    "run_isolated_generation_stage_v030",
    "run_isolated_prediction_process_v030",
]
