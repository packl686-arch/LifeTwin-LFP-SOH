"""Fail-closed environment attesters for the fixed V2.7 formal profile."""
...
class V027EnvironmentError(RuntimeError):
    """Raised when a process is not the attested V2.7 build."""


@dataclass(frozen=True, slots=True)
class V027EnvironmentIdentity:
    protocol_id: str
    git_commit: str
    git_dirty: bool
...
@dataclass(frozen=True, slots=True)
class V027PredictionEnvironmentIdentity:
    git_commit: str
    protocol_id: str
    config_byte_sha256: str
...
_AMENDMENT_PATH = Path(
    "configs/experiments/synthetic_long_horizon_identifiability_v2_7_amendment.json"
)
_PREREG_PATH = Path("reports/synthetic_long_horizon_identifiability_prereg_v2_7.md")
_LOCK_PATH = Path("requirements/v027-formal.txt")
_AUDIT_PATH = Path(
    "reports/synthetic_long_horizon_identifiability_implementation_audit_v2_7.md"
)
_FREEZE_PATH = Path(
    "reports/synthetic_long_horizon_identifiability_freeze_record_v2_7.json"
)
_METADATA_PATHS = tuple(sorted((_AUDIT_PATH.as_posix(), _FREEZE_PATH.as_posix())))
_EXPECTED_PYTHON = (3, 12, 13)
_EXPECTED_HASH_SENTINEL = 8772296692342749989
...
def _git(root: Path, *arguments: str) -> str:
    try:
        ...
        raise V027EnvironmentError("Git attestation failed") from exc
...
def _strict_json(path: Path) -> dict[str, object]:
    def reject_duplicates(pairs: list[tuple[str, object]]) -> dict[str, object]:
        ...
        for key, value in pairs:
            if key in result:
                raise V027EnvironmentError(f"Duplicate JSON key: {key}")
            result[key] = value
        return result

    def reject_constant(token: str) -> None:
        raise V027EnvironmentError(f"Nonfinite JSON constant: {token}")
    ...
    try:
        ...
        raise V027EnvironmentError("Freeze record is invalid JSON") from exc
    if not isinstance(payload, dict):
        raise V027EnvironmentError("Freeze record must be a JSON object")
    return payload
...
def _source_hashes(root: Path) -> Mapping[str, str]:
    ...
    if not ordered or any(not path.is_file() for path in ordered):
        raise V027EnvironmentError("A bound implementation source is absent")
...
def _require_tracked(root: Path, paths: tuple[Path, ...]) -> None:
    ...
    if observed != set(relative):
        raise V027EnvironmentError("A frozen input is not git-tracked")
...
def _validate_freeze_record(
    ...
    *,
    ...
    expected = {
        "schema_version": "lifetwin_v027_freeze_record/1.0.0",
        "protocol_id": V027_PROTOCOL_ID,
        "review_status": "implementation_frozen",
        "design_freeze_commit": V027_DESIGN_FREEZE_COMMIT,
        "amendment_path": _AMENDMENT_PATH.as_posix(),
        "amendment_byte_sha256": V027_AMENDMENT_BYTE_SHA256,
        "amendment_semantic_sha256": V027_AMENDMENT_SEMANTIC_SHA256,
        "preregistration_path": _PREREG_PATH.as_posix(),
        "preregistration_byte_sha256": V027_PREREG_BYTE_SHA256,
        "environment_lock_path": _LOCK_PATH.as_posix(),
        "environment_lock_byte_sha256": V027_REQUIREMENTS_BYTE_SHA256,
        "implementation_audit_path": _AUDIT_PATH.as_posix(),
        "execution_metadata_paths": list(_METADATA_PATHS),
        "formal_v2_7_generation_executed_before_implementation_freeze": False,
        "v2_7_outcome_exposure_before_implementation_freeze": False,
    }
    if any(payload.get(key) != value for key, value in expected.items()):
        raise V027EnvironmentError("Freeze record identity changed")
    ...
    if (
        ...
        raise V027EnvironmentError("Freeze record source commitment changed")
...
def _runtime() -> tuple[
    ...
    if sys.version_info[:3] != _EXPECTED_PYTHON:
        raise V027EnvironmentError("Formal Python must be CPython 3.12.13")
    if hash("lifetwin-v027-formal") != _EXPECTED_HASH_SENTINEL:
        raise V027EnvironmentError("PYTHONHASHSEED was not active at process start")
    packages: dict[str, str] = {}
    for package, expected in _EXPECTED_PACKAGES.items():
        try:
            observed = metadata.version(package)
        except metadata.PackageNotFoundError as exc:
            raise V027EnvironmentError(f"Formal package is absent: {package}") from exc
        if observed != expected:
            raise V027EnvironmentError(f"Formal package changed: {package}")
    ...
    if any(environment[name] != "1" for name in _THREAD_VARIABLES) or (
        ...
        raise V027EnvironmentError("Formal deterministic environment is not locked")
    pools: list[Mapping[str, object]] = []
    for pool in threadpool_info():
        if pool.get("num_threads") != 1:
            raise V027EnvironmentError("A loaded thread pool is not single-threaded")
        pools.append(MappingProxyType(dict(pool)))
    if not pools:
        raise V027EnvironmentError("No active numeric thread pool was attested")
    return (
...
def _attest(
    ...
    if Path(_git(root, "rev-parse", "--show-toplevel")).resolve() != root:
        raise V027EnvironmentError("Repository root is not the git top level")
    head = _git(root, "rev-parse", "HEAD")
    if _FULL_GIT_HASH.fullmatch(head) is None:
        raise V027EnvironmentError("Git HEAD is not a full object ID")
    if _git(root, "status", "--porcelain=v1", "--untracked-files=all"):
        raise V027EnvironmentError("Formal implementation worktree is dirty")
    design = load_v027_design(root / _AMENDMENT_PATH)
    if (
        design.config_byte_sha256 != V027_AMENDMENT_BYTE_SHA256
        or design.config_semantic_sha256 != V027_AMENDMENT_SEMANTIC_SHA256
    ):
        raise V027EnvironmentError("V2.7 amendment identity changed")
    paths = tuple(
        root / path for path in (_PREREG_PATH, _LOCK_PATH, _AUDIT_PATH, _FREEZE_PATH)
    )
    if any(not path.is_file() for path in paths):
        raise V027EnvironmentError("A required freeze input is absent")
    if _sha256(root / _PREREG_PATH) != V027_PREREG_BYTE_SHA256:
        raise V027EnvironmentError("V2.7 preregistration changed")
    if _sha256(root / _LOCK_PATH) != V027_REQUIREMENTS_BYTE_SHA256:
        raise V027EnvironmentError("V2.7 environment lock changed")
    ...
    if changes != _METADATA_PATHS:
        raise V027EnvironmentError("Freeze commit changed files outside the allowlist")
    runtime = _runtime()
    return head, source_hashes, runtime


def verify_formal_environment_v027(
    repo_root: str | Path,
    contract_view: V024ContractView,
) -> V027EnvironmentIdentity:
    """Attest V2.7 and bind the authenticated contract carried by the caller."""

    root = Path(repo_root).resolve()
    if (
        contract_view.protocol.protocol_id != V027_PROTOCOL_ID
        or contract_view.artifacts.config_byte_sha256 != V027_AMENDMENT_BYTE_SHA256
        or contract_view.config_canonical_sha256 != V027_AMENDMENT_SEMANTIC_SHA256
        or contract_view.design_status != "implementation_frozen"
    ):
        raise V027EnvironmentError("Formal contract differs from V2.7")
    head, source_hashes, runtime = _attest(root)
    packages, environment, pools = runtime
    return V027EnvironmentIdentity(
        protocol_id=V027_PROTOCOL_ID,
        git_commit=head,
        git_dirty=False,
        config_byte_sha256=V027_AMENDMENT_BYTE_SHA256,
        config_canonical_sha256=V027_AMENDMENT_SEMANTIC_SHA256,
        preregistration_byte_sha256=V027_PREREG_BYTE_SHA256,
        environment_lock_byte_sha256=V027_REQUIREMENTS_BYTE_SHA256,
...
def verify_prediction_environment_v027(
    repo_root: str | Path,
) -> V027PredictionEnvironmentIdentity:
    """Attest the same frozen identity without importing a truth capability."""

    head, _, _ = _attest(Path(repo_root).resolve())
    return V027PredictionEnvironmentIdentity(
        git_commit=head,
        protocol_id=V027_PROTOCOL_ID,
        config_byte_sha256=V027_AMENDMENT_BYTE_SHA256,
    )
...
__all__ = [
    "V027EnvironmentError",
    "V027EnvironmentIdentity",
    "V027PredictionEnvironmentIdentity",
    "source_tree_sha256",
    "verify_formal_environment_v027",
    "verify_prediction_environment_v027",
]
