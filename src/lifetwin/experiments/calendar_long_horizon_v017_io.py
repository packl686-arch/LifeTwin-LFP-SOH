"""Strict V2.2 label-free IO and commitment capabilities.

This module is the only filesystem boundary used by V2.2 fitting and
...
from lifetwin.experiments.calendar_long_horizon_v017_provenance import (
    ...
    V022ProvenanceError,
    _issue_committed_model_state_envelope_v022,
    _rehydrate_v022_training_provenance_after_strict_io,
)
from lifetwin.experiments.calendar_long_horizon_v017_state import (
    V022StateCodecError,
    _AUDIT_KEYS,
...
class V022IOError(ValueError):
    """Raised when a V2.2 filesystem capability cannot be proven."""
...
_ARTIFACT_SET_DOMAIN = b"lifetwin-v022-prediction-artifact-set-v1\0"
_EVIDENCE_DOMAIN = b"lifetwin-v022-prediction-evidence-v1\0"
...
class V022PredictionCommitmentEvidence:
    ...
    def __init__(
        ...
        *,
        ...
        if (
            ...
            or type(self) is not V022PredictionCommitmentEvidence
        ):
            raise TypeError(
                "Prediction commitment evidence is issued only by strict V2.2 IO"
            )
...
class V022FreshGenerationBundle:
    ...
    def __init__(
        ...
        *,
        ...
        if _seal is not _SEAL or type(self) is not V022FreshGenerationBundle:
            raise TypeError("Fresh-generation bundles are issued only by V2.2 IO")
        object.__setattr__(self, "_seal", _seal)
    ...
    def protocol_id(self) -> str:
        return V022_PROTOCOL_ID


class V022CommittedLabelFreeBundle:
    ...
    def __init__(
        ...
        *,
        ...
        model_state: V022CommittedModelStateEnvelope,
    ) -> None:
        if _seal is not _SEAL or type(self) is not V022CommittedLabelFreeBundle:
            raise TypeError("Committed label-free bundles are issued only by V2.2 IO")
        object.__setattr__(self, "_seal", _seal)
        object.__setattr__(self, "_root", root)
    ...
    @property
    def model_state(self) -> V022CommittedModelStateEnvelope:
...
def _prediction_artifact_contract_snapshot(
    ...
    return FrozenArtifactContract(
        ...
        config_path=Path("__v022_prediction_config_withheld__"),
...
def _bundle_artifact_contract(
    value: V022FreshGenerationBundle | V022CommittedLabelFreeBundle,
) -> FrozenArtifactContract:
    if type(value) is V022FreshGenerationBundle:
        return value._contract_view.artifacts
    if type(value) is V022CommittedLabelFreeBundle:
        return value._artifact_contract
    raise V022IOError("Unknown sealed bundle type")
...
def _verify_stored_bundle_frames(
    ...
    *,
    ...
    if tuple(name for name, _ in frames) != tuple(expected_filenames):
        raise V022IOError("Sealed in-memory frame registry changed")
    file_hashes = dict(value._file_hashes)
    for filename, frame in frames:
        if type(frame) is not pd.DataFrame:
            raise V022IOError(f"Sealed frame type changed: {filename}")
        try:
            ...
            raise V022IOError(f"Sealed in-memory frame changed: {filename}") from exc
        if file_hashes.get(filename) != _sha256(raw):
            raise V022IOError(f"Sealed in-memory frame changed: {filename}")
...
def _evidence_digest(
    *,
    ...
    if not isinstance(attempt_id, str) or _ATTEMPT_ID.fullmatch(attempt_id) is None:
        raise V022IOError("Prediction evidence attempt ID is invalid")
    if type(ledger_committed) is not bool:
        raise V022IOError("Prediction evidence ledger status is invalid")
...
def _validated_evidence_file_entries(
    ...
    if not isinstance(value, Sequence) or isinstance(
        ...
        raise V022IOError("Prediction evidence file entries must be a sequence")
    result: list[tuple[str, int, int, str]] = []
    for entry in value:
        if not isinstance(entry, Mapping) or set(entry) != _FILE_ENTRY_KEYS:
            raise V022IOError("Prediction evidence contains an invalid file entry")
        path = entry.get("path")
        ...
        if (
            ...
            raise V022IOError("Prediction evidence file metadata is invalid")
        result.append(
            (
                path,
                row_count,
    ...
    if not result or tuple(item[0] for item in result) != _COMMITMENT_FILE_REGISTRY:
        raise V022IOError("Prediction evidence file registry changed")
    return tuple(result)
...
def _issue_prediction_commitment_evidence(
    *,
    ...
    ledger_committed: bool,
) -> V022PredictionCommitmentEvidence:
    ...
    if type(ledger_committed) is not bool:
        raise V022IOError("ledger_committed must be an exact boolean")
    ...
    return V022PredictionCommitmentEvidence(
...
def _require_prediction_commitment_evidence_v022(
    value: object,
    *,
    ...
    if (
        type(value) is not V022PredictionCommitmentEvidence
        or value._seal is not _EVIDENCE_SEAL
    ):
        raise V022IOError(
            "Exact prediction commitment evidence issued by V2.2 IO is required"
        )
    ...
    if value._provenance_sha256 != expected:
        raise V022IOError("Prediction commitment evidence digest changed")
    if require_ledger_committed and not value.ledger_committed:
        raise V022IOError("Prediction commitment evidence is not ledger committed")
    return value
...
def _is_reparse(path: Path) -> bool:
    try:
        ...
        raise V022IOError(f"Cannot inspect physical path: {path}") from exc
...
def _physical_root(raw: str | Path) -> Path:
    ...
    if not root.is_dir() or _is_reparse(root):
        raise V022IOError("Label-free root must be a direct physical directory")
    for parent in root.parents:
        if parent.exists() and _is_reparse(parent):
            raise V022IOError("Label-free root traverses a reparse point")
    return root


def _direct_file(root: Path, filename: str) -> Path:
    if Path(filename).name != filename or filename in {"", ".", ".."}:
        raise V022IOError(f"Unsafe label-free filename: {filename!r}")
    path = root / filename
    if not path.exists() or _is_reparse(path) or not path.is_file():
        raise V022IOError(f"{filename} is not a direct physical file")
    return path
...
def _require_membership(root: Path, expected: frozenset[str], *, context: str) -> None:
    ...
    for entry in entries:
        if _is_reparse(entry) or not entry.is_file():
            raise V022IOError(f"{context} contains a nonphysical artifact")


def _require_contract(view: object) -> V022ContractView:
    if type(view) is not V022ContractView:
        raise V022IOError("contract_view must be an exact V022ContractView")
    if (
        ...
        raise V022IOError("V2.2 formal IO requires the implementation-frozen contract")
    return view
...
def _strict_object(pairs: list[tuple[str, object]]) -> dict[str, object]:
    ...
    for key, value in pairs:
        if key in result:
            raise V022IOError(f"Duplicate JSON key: {key}")
        result[key] = value
    return result


def _reject_constant(token: str) -> None:
    raise V022IOError(f"Nonfinite JSON constant is forbidden: {token}")


def _strict_json(raw: bytes, *, filename: str, compact: bool = False) -> dict[str, Any]:
    if type(raw) is not bytes:
        raise V022IOError(f"{filename} must be exact bytes")
    try:
        payload = json.loads(
            raw.decode("utf-8"),
        ...
        raise V022IOError(f"{filename} is not strict UTF-8 JSON") from exc
    if not isinstance(payload, dict):
        raise V022IOError(f"{filename} must be a JSON object")
    canonical = (
        (
            json.dumps(
                payload,
    ...
    if canonical != raw:
        raise V022IOError(f"{filename} is not canonical JSON")
    return payload


def _utc(value: object, *, context: str) -> str:
    if not isinstance(value, str) or _UTC.fullmatch(value) is None:
        raise V022IOError(f"{context} must be an RFC3339 UTC timestamp")
    try:
        parsed = datetime.fromisoformat(value[:-1] + "+00:00")
    except ValueError as exc:
        raise V022IOError(f"{context} is not a valid timestamp") from exc
    if parsed.utcoffset() is None:
        raise V022IOError(f"{context} lacks a timezone")
    return value


def _digest(value: object, *, context: str) -> str:
    if not isinstance(value, str) or _SHA256.fullmatch(value) is None:
        raise V022IOError(f"{context} must be lowercase SHA256")
    return value


def _positive_int(value: object, *, context: str) -> int:
    if type(value) is not int or value < 1:
        raise V022IOError(f"{context} must be a positive integer")
    return value
...
def _identity_json(
    ...
    *,
    ...
    if (
        ...
        raise V022IOError(f"{filename} identity changed")


def _load_ledger(
    root: Path,
    *,
    view: V022ContractView,
    attempt_id: str,
    ...
    try:
        ...
        raise V022IOError(str(exc)) from exc
    try:
        progress = states[attempt_id]
    except KeyError as exc:
        raise V022IOError("Attempt is absent from the canonical ledger") from exc
    if progress.identity.attempt_id != attempt_id or progress.terminal_failed:
        raise V022IOError("Attempt ledger identity is unusable")
...
def _require_phase_hash(
    ...
    *,
    ...
    if expected is None or expected != observed:
        raise V022IOError(f"{context} differs from its ledger commitment")
    return observed
...
def _verify_generation_and_truth(
    ...
    *,
    ...
    if require_semantic_plan_recompute:
        from lifetwin.experiments.calendar_long_horizon_v017_actual_ledger_io import (  # noqa: PLC0415
            V022ActualLedgerIOError,
            recompute_generation_plan_commitment_bytes_v022,
        )

        try:
            expected_plan_raw = recompute_generation_plan_commitment_bytes_v022(view)
        except V022ActualLedgerIOError as exc:
            raise V022IOError("Formal generation-plan recomputation failed") from exc
    ...
    if progress.truth_commitments_byte_sha256 != _sha256(truth_raw):
        raise V022IOError("Truth commitment differs from the ledger")
    ...
    if (
        ...
        or truth_payload.get("protocol_id") != V022_PROTOCOL_ID
        ...
        raise V022IOError("Truth commitment identity or schema changed")
    _utc(
    ...
    if (
        ...
        raise V022IOError("Truth commitment file registry changed")
    ...
    for filename, entry in zip(
        ...
        if (
            ...
            raise V022IOError(f"Truth commitment metadata changed for {filename}")
...
def _recompute_actual_analysis_hash_ledger_bytes(
    ...
    *,
    ...
    from lifetwin.experiments.calendar_long_horizon_v017_actual_ledger_io import (  # noqa: PLC0415
        V022ActualLedgerIOError,
        recompute_actual_analysis_hash_ledger_bytes_v022,
    )

    try:
        return recompute_actual_analysis_hash_ledger_bytes_v022(
            frames,
            view=view,
        )
    except V022ActualLedgerIOError as exc:
...
def _ordinary_family_lookup(
    ...
    from lifetwin.experiments.calendar_long_horizon_v017_actual_ledger_io import (  # noqa: PLC0415
        V022ActualLedgerIOError,
        ordinary_family_lookup_v022,
    )

    try:
        return ordinary_family_lookup_v022(view)
    except V022ActualLedgerIOError as exc:
        raise V022IOError("Formal ordinary identity derivation failed") from exc
...
def _verify_actual_analysis_hash_ledger(
    ...
    *,
    ...
    if require_semantic_recompute:
        from lifetwin.experiments.calendar_long_horizon_v017_actual_ledger_io import (  # noqa: PLC0415
            V022ActualLedgerIOError,
            verify_actual_analysis_hash_ledger_payload_v022,
        )
...
def _verify_fit_commitment(
    ...
    *,
    ...
    if (
        ...
        raise V022IOError("fit_commitment.json semantics changed")
    ...
    if not isinstance(entries, list) or len(entries) != len(_FIT_COMMITMENT_FILES):
        raise V022IOError("Fit commitment file registry changed")
    for filename, entry in zip(_FIT_COMMITMENT_FILES, entries, strict=True):
        if not isinstance(entry, Mapping) or set(entry) != _FILE_ENTRY_KEYS:
            raise V022IOError("Fit commitment contains an invalid file entry")
        ...
        if (
            ...
            raise V022IOError(f"Fit commitment does not bind {filename}")
    return raw
...
def _resolve_input_hashes(
    ...
    *,
    ...
    if not isinstance(value, Mapping) or not value:
        raise V022IOError(f"{context} must be a nonempty hash mapping")
    result: dict[str, str] = {}
    for raw_name, raw_digest in value.items():
        if not isinstance(raw_name, str):
            raise V022IOError(f"{context} contains a non-string filename")
        ...
        if actual != digest:
            raise V022IOError(f"{context} input changed: {raw_name}")
...
def _decode_model_state(
    ...
    *,
    ...
    if set(payload) != view.artifacts.json_keys("model_state.json"):
        raise V022IOError("model_state.json schema changed")
    ...
    try:
        ...
        raise V022IOError("V2.2 model state is numerically invalid") from exc
...
def _verify_calibration_audit(
    ...
    *,
    ...
    if set(payload) != _AUDIT_KEYS:
        raise V022IOError("Calibration population audit schema changed")
    _identity_json(
        payload,
    ...
    except (TypeError, ValueError, V022StateCodecError) as exc:
        raise V022IOError("Calibration population audit is invalid") from exc
    ...
    if (
        ...
        raise V022IOError("Calibration population audit bindings changed")
...
def _translate_manifest_to_v2(
    ...
    *,
    ...
    if set(payload) != view.artifacts.json_keys(filename):
        raise V022IOError(f"{filename} schema changed")
...
def _verify_training_chain(
    ...
    *,
    ...
    if (
        ...
        raise V022IOError("Center checkpoint semantics changed")
    if (
        set(risk) != _RISK_KEYS
        ...
        raise V022IOError("Risk checkpoint semantics changed")
    ...
    if risk.get("training_manifest_byte_sha256") != _sha256(training_raw):
        raise V022IOError("Risk checkpoint does not bind training manifest")
    ...
    try:
        ...
        raise V022IOError("Training manifest state bindings changed") from exc
    if (
        training.get("center_development_input_hashes") != center_inputs
        or training.get("risk_development_input_hashes") != risk_inputs
    ):
        raise V022IOError("Training manifest input hashes changed")
    ...
    try:
        ...
        raise V022IOError("Calibration manifest state bindings changed") from exc
    ...
    if decoded.input_byte_hashes != expected_inputs:
        raise V022IOError("Model-state input hashes differ from verified artifacts")
...
def _verify_model_commitment(
    ...
    *,
    ...
    if (
        ...
        raise V022IOError("Model-state commitment identity changed")
    _identity_json(
        payload,
        contract=view.artifacts,
    ...
    if len(entries) != len(_MODEL_STATE_COMMITMENT_FILES):
        raise V022IOError("Model-state commitment registry changed")
    ...
    for filename, entry in zip(
        ...
        if not isinstance(entry, Mapping) or set(entry) != _FILE_ENTRY_KEYS:
            raise V022IOError("Model-state commitment file entry changed")
        ...
        if (
            ...
            raise V022IOError(f"Model-state commitment does not bind {filename}")
        artifacts[filename] = artifact
    return raw, artifacts


def create_actual_analysis_hash_ledger_commitment_v022(
    *,
    label_free_root: str | Path,
    attempt_id: str,
    contract_view: V022ContractView | None = None,
) -> str:
    """Recompute and exclusively persist the formal post-generation ledger."""

    view = _require_contract(contract_view or load_v022_contract_view())
...
def load_fresh_generation_bundle_v022(
    *,
    label_free_root: str | Path,
    attempt_id: str,
    contract_view: V022ContractView | None = None,
    ...
    if (
        ...
        raise V022IOError("Ledger is not at the pending formal-fit boundary")
    _verify_generation_and_truth(
        root,
        view=view,
        progress=progress,
    ...
    return V022FreshGenerationBundle(
        _seal=_SEAL,
        root=root,
        contract_view=view,
...
def _load_committed_bundle(
    *,
...
) -> tuple[V022CommittedLabelFreeBundle, AttemptProgress]:
    ...
    if (progress.completed_phase, progress.pending_phase) not in allowed:
        raise V022IOError("Ledger is not at a committed prediction boundary")
    ...
    try:
        mask = deserialize_calibration_mask_commitment_json_v022(mask_raw)
    except (TypeError, ValueError) as exc:
        raise V022IOError("Calibration mask commitment is invalid") from exc
    _require_phase_hash(
        progress,
    ...
    try:
        training_provenance = _rehydrate_v022_training_provenance_after_strict_io(
            contract_view=view,
            attempt_id=attempt_id,
        ...
        validated_model = deserialize_model_state_json_v022(
            model_raw,
            provenance_envelope=training_provenance,
        )
    except (V022ProvenanceError, V022StateCodecError) as exc:
        raise V022IOError("Model-state provenance rehydration failed") from exc
    ...
    try:
        committed_model = _issue_committed_model_state_envelope_v022(
        ...
        raise V022IOError("Committed model-state envelope issuance failed") from exc
    hashes = {
        filename: _sha256(_direct_file(root, filename).read_bytes())
        for filename in _PRE_PREDICTION_FILES
    }
    bundle = V022CommittedLabelFreeBundle(
        _seal=_SEAL,
        root=root,
...
def load_committed_label_free_bundle_v022(
    *,
    label_free_root: str | Path,
    attempt_id: str,
    contract_view: V022ContractView | None = None,
    ...
    if (
        ...
        raise V022IOError("Prediction has already advanced past its sole write window")
    return bundle


def _require_fresh_bundle(value: object) -> V022FreshGenerationBundle:
    if type(value) is not V022FreshGenerationBundle or value._seal is not _SEAL:
        raise V022IOError("An exact IO-issued fresh-generation bundle is required")
...
def _require_fresh_bundle_unchanged(
    value: V022FreshGenerationBundle,
    *,
    expected_membership: frozenset[str],
) -> V022FreshGenerationBundle:
    if type(value) is not V022FreshGenerationBundle or value._seal is not _SEAL:
    ...
    for filename, digest in value._file_hashes:
        if _sha256(_direct_file(value._root, filename).read_bytes()) != digest:
            raise V022IOError("Fresh-generation artifact changed after issuance")
    if _direct_file(value._root, "exposure_log.jsonl").read_bytes() != (
        value._ledger_prefix
    ):
        raise V022IOError("Fresh-generation ledger changed after issuance")
    _verify_stored_bundle_frames(value, expected_filenames=_LABEL_INPUTS)
    return value


def _extract_fresh_generation_frames_for_formal_fit_v022(
    value: object,
...
def _require_committed_bundle(value: object) -> V022CommittedLabelFreeBundle:
    if type(value) is not V022CommittedLabelFreeBundle or value._seal is not _SEAL:
        raise V022IOError("An exact IO-issued committed label-free bundle is required")
    return value
    ...
    *,
    ...
    for filename, digest in bundle._file_hashes:
        if _sha256(_direct_file(bundle._root, filename).read_bytes()) != digest:
            raise V022IOError("A committed label-free artifact changed")
    if _direct_file(bundle._root, "exposure_log.jsonl").read_bytes() != (
        bundle._ledger_prefix
    ):
        raise V022IOError("Committed prediction ledger changed")
...
def _extract_prediction_inputs_v022(
    value: object,
    *,
    model_state: V022CommittedModelStateEnvelope,
    ...
    bundle = _require_committed_bundle(value)
    if (
        type(model_state) is not V022CommittedModelStateEnvelope
        or model_state.provenance_sha256 != bundle._model_state.provenance_sha256
    ):
        raise V022IOError("Prediction model does not belong to the label-free bundle")
...
def _exclusive_create(path: Path, raw: bytes) -> None:
    try:
        ...
        raise V022IOError(f"Formal artifact already exists: {path.name}") from exc
    try:
        ...
        raise V022IOError(f"Could not write {path.name} exclusively") from exc
    finally:
        os.close(descriptor)
    if path.read_bytes() != raw:
        raise V022IOError(f"{path.name} changed after its exclusive write")


def _write_verified_fit_outputs_and_commitment_v022(
    ...
    *,
    ...
    if set(frames) != set(_FIT_OUTPUTS):
        raise V022IOError("Formal fit output registry changed")
    ...
    for filename in (*_FIT_OUTPUTS, "fit_commitment.json"):
        if (value._root / filename).exists():
            raise V022IOError(f"Formal fit artifact already exists: {filename}")
    ...
    for filename in _FIT_COMMITMENT_FILES:
        ...
        if filename in _FIT_OUTPUTS:
            row_count = len(frames[filename])
        elif filename in _LABEL_INPUTS:
            row_count = len(dict(value._frames)[filename])
        else:
            row_count = 1
        entries.append(
            {
    ...
    if set(payload) != _FIT_COMMITMENT_KEYS:
        raise V022IOError("Fit commitment schema changed")
...
def _write_prediction_outputs_v022(
    bundle: V022CommittedLabelFreeBundle,
    *,
    frames: Mapping[str, pd.DataFrame],
    ...
    if set(frames) != set(_PREDICTION_OUTPUTS):
        raise V022IOError("Prediction output file registry changed")
    raw_by_name: dict[str, bytes] = {}
    for filename in _PREDICTION_OUTPUTS:
        try:
            ...
            raise V022IOError(str(exc)) from exc
        if (value._root / filename).exists():
            raise V022IOError(f"Prediction artifact already exists: {filename}")
...
def _verify_file_entries_current(
    ...
    if tuple(entry.get("path") for entry in entries) != (_COMMITMENT_FILE_REGISTRY):
        raise V022IOError("Prediction commitment file registry changed")
    for entry in entries:
        ...
        if entry.get("byte_count") != len(raw) or entry.get("byte_sha256") != _sha256(
            ...
            raise V022IOError(f"Prediction commitment input changed: {filename}")


def create_prediction_commitment_v022(
    bundle: V022CommittedLabelFreeBundle,
    *,
    created_utc: str,
) -> V022PredictionCommitmentEvidence:
    ...
    if set(payload) != _PREDICTION_COMMITMENT_KEYS:
        raise V022IOError("Prediction commitment schema changed")
    raw = canonical_json_bytes(payload)
    _require_committed_bundle_unchanged(
        value,
        expected_membership=_PREDICTION_FILES,
...
def verify_prediction_commitment_v022(
    *,
    label_free_root: str | Path,
    attempt_id: str,
    contract_view: V022ContractView | None = None,
    ...
    if (
        ...
        raise V022IOError("Prediction commitment identity changed")
    ...
    if prefix_count > len(lines):
        raise V022IOError("Prediction commitment ledger prefix is unavailable")
    prefix = b"".join(lines[:prefix_count])
    if payload.get("ledger_prefix_byte_sha256") != _sha256(prefix):
        raise V022IOError("Prediction commitment ledger prefix changed")
    if payload.get("ledger_phase") != {
        "completed_phase": "model_state_committed",
        "pending_phase": "prediction_started",
    }:
        raise V022IOError("Prediction commitment precommit phase changed")
    ...
    if payload.get("files") != list(entries) or payload.get("row_counts") != row_counts:
        raise V022IOError("Prediction commitment artifact metadata changed")
    artifact_set = _artifact_set_digest(entries)
    if payload.get("artifact_set_sha256") != artifact_set:
        raise V022IOError("Prediction commitment artifact-set digest changed")
    ...
    if payload.get("model_state_commitment_byte_sha256") != model_commitment_hash:
        raise V022IOError("Prediction commitment model-state binding changed")
    actual_analysis_hash = _sha256(
    ...
    if (
        ...
        raise V022IOError("Prediction commitment actual-analysis binding changed")
    ...
    if require_ledger_committed and not ledger_committed:
        raise V022IOError("Prediction commitment is absent from the completed ledger")
    if not ledger_committed and (
        progress.completed_phase != "model_state_committed"
        or progress.pending_phase != "prediction_started"
        or event_count != prefix_count
    ):
        raise V022IOError("Prediction commitment and ledger phase disagree")
    return _issue_prediction_commitment_evidence(
        attempt_id=attempt_id,
...
__all__ = [
    ...
    "V022IOError",
    "V022PredictionCommitmentEvidence",
    "create_actual_analysis_hash_ledger_commitment_v022",
    "create_prediction_commitment_v022",
    "load_committed_label_free_bundle_v022",
    "load_fresh_generation_bundle_v022",
    "verify_prediction_commitment_v022",
]
