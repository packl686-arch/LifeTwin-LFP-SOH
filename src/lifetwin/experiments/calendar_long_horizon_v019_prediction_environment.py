"""Standalone formal-environment attester for the V2.4 prediction capsule.

This module deliberately does not import the V2.4 protocol, contract, training,
...
class V024PredictionEnvironmentError(RuntimeError):
    """Raised when the prediction process is not the attested V2.4 build."""
...
def _git(repo_root: Path, *arguments: str) -> str:
    try:
        ...
        raise V024PredictionEnvironmentError(
...
def _reject_duplicate_keys(
    ...
    for key, value in pairs:
        if key in result:
            raise V024PredictionEnvironmentError(f"Duplicate JSON key: {key}")
        result[key] = value
    return result


def _reject_json_constant(token: str) -> None:
    raise V024PredictionEnvironmentError(f"Nonfinite JSON constant: {token}")
...
def _strict_json_object(path: Path, *, context: str) -> dict[str, object]:
    try:
        ...
        raise V024PredictionEnvironmentError(f"{context} is invalid JSON") from exc
    if not isinstance(decoded, dict):
        raise V024PredictionEnvironmentError(f"{context} must be a JSON object")
    return decoded
...
def _validate_frozen_amendment(path: Path) -> str:
    payload = _strict_json_object(path, context="V2.4 amendment")
    if set(payload) != _AMENDMENT_TOP_LEVEL_KEYS:
        raise V024PredictionEnvironmentError("V2.4 amendment keys changed")
    if (
        payload.get("schema_version") != "lifetwin_synthetic_long_horizon_v2_4/1.0.0"
        or payload.get("protocol_id") != V024_PROTOCOL_ID
    ):
        raise V024PredictionEnvironmentError("V2.4 amendment identity changed")
    ...
    if (
        ...
        raise V024PredictionEnvironmentError(
            "V2.4 frozen amendment unexpectedly records prior generation"
        )
    ...
    if hashlib.sha256(canonical).hexdigest() != V024_AMENDMENT_SEMANTIC_SHA256:
        raise V024PredictionEnvironmentError("V2.4 amendment semantic content changed")
    byte_sha256 = _sha256_path(path)
    if byte_sha256 != V024_AMENDMENT_BYTE_SHA256:
        raise V024PredictionEnvironmentError("V2.4 amendment bytes changed")
    return byte_sha256
...
def _source_hashes(repo_root: Path) -> Mapping[str, str]:
    ...
    if any(not path.is_file() for path in paths):
        raise V024PredictionEnvironmentError(
            "A V2.4 implementation source is not a file"
        )
...
def verify_prediction_environment(
    ...
    if _FULL_GIT_HASH.fullmatch(git_commit) is None:
        raise V024PredictionEnvironmentError("Git did not return a full commit hash")
    if _git(root, "status", "--porcelain=v1", "--untracked-files=all"):
        raise V024PredictionEnvironmentError("Formal implementation worktree is dirty")
    ...
    if _sha256_path(prereg_path) != _PREREG_BYTE_SHA256:
        raise V024PredictionEnvironmentError("V2.4 preregistration hash changed")
    if _sha256_path(lock_path) != _LOCK_BYTE_SHA256:
        raise V024PredictionEnvironmentError("V2.4 environment lock hash changed")
    if _sha256_path(implementation_audit_path) != _IMPLEMENTATION_AUDIT_BYTE_SHA256:
        raise V024PredictionEnvironmentError("V2.4 implementation audit hash changed")
