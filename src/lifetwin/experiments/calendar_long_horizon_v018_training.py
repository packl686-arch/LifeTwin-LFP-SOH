"""V2.3 calibration-pool split without changing the frozen V0.15 code.

The V2.3 rule separates the rows used for probability calibration from the
...
V023_MINIMUM_ELIGIBLE_FRACTION = 0.95
V023_MINIMUM_ELIGIBLE_COUNT = math.ceil(
    V023_MINIMUM_ELIGIBLE_FRACTION * CALIBRATION_COUNT
)
_MAXIMUM_STRUCTURE_FAMILY_COUNT = 7
_MASK_HASH_DOMAIN = b"lifetwin-v023-calibration-mask-v2\0"
_ROW_HASH_DOMAIN = b"lifetwin-v023-calibration-row-v2\0"
_SUPPORT_HASH_DOMAIN = b"lifetwin-v023-structural-support-v1\0"
_V023_PROTOCOL_ID = "synthetic_long_horizon_identifiability_v2_3"
...
class V023CalibrationError(ValueError):
    """Raised when V2.3 calibration inputs violate their typed contract."""


@dataclass(frozen=True)
class V023CommittedMaskRow:
...
@dataclass(frozen=True)
class V023PretruthMaskCommitment:
    ...
    def __post_init__(self) -> None:
        if self.protocol_id != _V023_PROTOCOL_ID:
...
def _numeric_matrix(
    ...
    *,
    ...
    try:
        ...
        raise V023CalibrationError(f"{name} must be a numeric matrix") from exc
    if matrix.shape != (rows, columns):
        raise V023CalibrationError(
            f"{name} must have shape ({rows}, {columns}), got {matrix.shape}"
        )
    if require_finite and not np.isfinite(matrix).all():
        raise V023CalibrationError(f"{name} must contain only finite values")
    return matrix
...
def _numeric_vector(
    ...
    *,
    ...
    try:
        ...
        raise V023CalibrationError(f"{name} must be a numeric vector") from exc
    if vector.shape != (length,):
        raise V023CalibrationError(f"{name} must contain exactly {length} values")
...
def _strict_text_vector(
    ...
    *,
    ...
    if raw.shape != (CALIBRATION_COUNT,) or any(
        ...
        raise V023CalibrationError(f"{name} must contain exactly 900 valid strings")
    result = tuple(str(value) for value in raw)
    if require_unique and len(set(result)) != CALIBRATION_COUNT:
        raise V023CalibrationError(f"{name} must contain 900 unique values")
    return result
...
def _require_source_count(**row_aligned: object) -> None:
    ...
    if mismatches:
        raise V023CalibrationTerminalInconclusive(
            "CALIBRATION_SOURCE_COUNT_NOT_900",
            "V2.3 calibration source count must be 900 for every aligned "
...
def _derive_pretruth_evidence(
    *,
    ...
    if not isinstance(risk_state, RiskDevelopmentState):
        raise V023CalibrationError("risk_state must be a RiskDevelopmentState")
    ...
    except V023CalibrationError as exc:
        raise V023CalibrationTerminalInconclusive(
            "CALIBRATION_BASELINE_INCOMPLETE",
    ...
    committed_rows: list[V023CommittedMaskRow] = []
    ...
    commitment = V023PretruthMaskCommitment(
        protocol_id=_V023_PROTOCOL_ID,
        source_calibration_count=CALIBRATION_COUNT,
        rows=canonical_rows,
        eligibility_mask_sha256=commitment_hasher.hexdigest(),
    )
    if commitment.eligible_count < V023_MINIMUM_ELIGIBLE_COUNT:
        raise V023CalibrationTerminalInconclusive(
            "CALIBRATION_RISK_ELIGIBLE_BELOW_855",
            "V2.3 isotonic calibration requires at least "
            f"{V023_MINIMUM_ELIGIBLE_COUNT} eligible rows; observed "
...
def derive_calibration_mask_commitment_v023(
    *,
    risk_state: RiskDevelopmentState,
    cluster_ids: Sequence[str],
    arm_a_predictor_content_sha256: Sequence[str],
...
def calibration_eligibility_mask_sha256_v023(
    commitment: V023PretruthMaskCommitment,
) -> str:
    """Return the digest of an already-derived immutable pretruth commitment."""

    if not isinstance(commitment, V023PretruthMaskCommitment):
        raise TypeError("commitment must be a V023PretruthMaskCommitment")
...
def fit_calibration_development_state_v023(
    *,
    pretruth_commitment: V023PretruthMaskCommitment,
    risk_state: RiskDevelopmentState,
    cluster_ids: Sequence[str],
    arm_a_predictor_content_sha256: Sequence[str],
    arm_b_predictor_content_sha256: Sequence[str],
    ...
    latent_target_forecast_days: Sequence[Sequence[float]],
    latent_targets_pct: Sequence[Sequence[float]],
) -> tuple[CalibrationDevelopmentState, V023CalibrationAudit]:
    ...
    if not isinstance(pretruth_commitment, V023PretruthMaskCommitment):
        raise V023CalibrationError(
            "pretruth_commitment must be a V023PretruthMaskCommitment"
        )
    ...
    try:
        ...
        if (
            ...
            raise V023CalibrationTerminalInconclusive(
                ...
                "A V2.3 isotonic map has fewer than two score thresholds",
            )
    except V023CalibrationTerminalInconclusive:
        raise
    except V2ModelError as exc:
        raise V023CalibrationTerminalInconclusive(
            "CALIBRATION_ISOTONIC_FIT_UNDEFINED",
            "V2.3 isotonic fitting is undefined on the shared eligible pool",
        ) from exc
    ...
    try:
        ...
        raise V023CalibrationTerminalInconclusive(
            ...
            "V2.3 conformal calibration is undefined on the 900-row source",
        ) from exc
    if (
        ...
        raise V023CalibrationTerminalInconclusive(
            ...
            "V2.3 conformal state differs from the 900/811 rule",
        )
...
__all__ = [
    ...
    "V023CalibrationError",
    "V023CalibrationTerminalInconclusive",
    "V023CommittedMaskRow",
    "V023PretruthMaskCommitment",
    "V023_MINIMUM_ELIGIBLE_COUNT",
    "V023_MINIMUM_ELIGIBLE_FRACTION",
    "calibration_eligibility_mask_sha256_v023",
