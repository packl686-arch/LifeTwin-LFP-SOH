"""Pure-memory V2.4 state, manifest, and calibration-audit codecs.

The inherited V2 numeric codec remains the sole validator for fitted numeric
state.  V2.4 translation is performed on detached in-memory objects and changes
...
from lifetwin.experiments.calendar_long_horizon_v019_provenance import (
    V024ProvenanceError,
    V024TrainingProvenanceEnvelope,
    V024ValidatedModelStateEnvelope,
    _issue_validated_model_state_envelope_v024,
    _require_training_envelope,
)
from lifetwin.experiments.calendar_long_horizon_v019_training import (
    V024_MINIMUM_ELIGIBLE_COUNT,
    V024CalibrationAudit,
    V024CommittedMaskRow,
    V024PretruthMaskCommitment,
)


class V024StateCodecError(ValueError):
    """Raised when V2.4 state or manifest bytes fail closed."""
...
class DecodedV024CalibrationPopulationAudit:
...
def _exact_object(
    ...
    *,
    ...
    if not isinstance(value, Mapping) or any(not isinstance(key, str) for key in value):
        raise V024StateCodecError(f"{context} must be a JSON object")
...
def _digest(value: object, *, context: str) -> str:
    if not isinstance(value, str) or _SHA256.fullmatch(value) is None:
        raise V024StateCodecError(f"{context} must be a lowercase SHA256")
    return value


def _integer(value: object, *, context: str) -> int:
    if isinstance(value, bool) or not isinstance(value, int):
        raise V024StateCodecError(f"{context} must be an integer")
    return value


def _created_utc(value: object) -> str:
    if not isinstance(value, str) or _UTC.fullmatch(value) is None:
        raise V024StateCodecError("created_utc must be an RFC3339 UTC timestamp")
    try:
        parsed = datetime.fromisoformat(value[:-1] + "+00:00")
    except ValueError as exc:
        raise V024StateCodecError("created_utc is not a valid timestamp") from exc
    if parsed.utcoffset() != timezone.utc.utcoffset(parsed):
        raise V024StateCodecError("created_utc must use UTC")
    return value
...
def _canonical_pretty_bytes(payload: Mapping[str, Any]) -> bytes:
    try:
        ...
        raise V024StateCodecError("Payload is not finite canonical JSON") from exc
...
def _duplicate_rejecting_object(pairs: list[tuple[str, object]]) -> dict[str, object]:
    ...
    for key, value in pairs:
        if key in result:
            raise V024StateCodecError(f"Duplicate JSON key: {key}")
        result[key] = value
    return result


def _reject_json_constant(token: str) -> None:
    raise V024StateCodecError(f"Nonfinite JSON constant is forbidden: {token}")
...
def _decode_canonical_object(
    ...
    *,
    ...
    if type(raw) is not bytes:
        raise V024StateCodecError(f"{filename} input must be exact bytes")
    try:
        decoded = json.loads(
            raw.decode("utf-8"),
        ...
        raise V024StateCodecError(f"{filename} is not valid UTF-8 JSON") from exc
    if not isinstance(decoded, dict):
        raise V024StateCodecError(f"{filename} root must be an object")
    canonical = (
    ...
    if canonical != raw:
        raise V024StateCodecError(f"{filename} bytes are not canonical")
    return decoded
...
def _require_current_identity(
    ...
    *,
    ...
    if payload.get("protocol_id") != protocol_id:
        raise V024StateCodecError(f"{context} protocol_id changed")
...
def _translate_identity(
    ...
    *,
    ...
    if payload.get("protocol_id") != source_protocol_id:
        raise V024StateCodecError(f"{context} source protocol identity changed")
    if payload.get("config_sha256") != source_config_sha256:
        raise V024StateCodecError(f"{context} source config identity changed")
...
def _validate_committed_row(
    ...
    *,
    ...
    if type(row) is not V024CommittedMaskRow:
        raise V024StateCodecError(f"Mask row {index} has an invalid type")
    try:
        normalized = V024CommittedMaskRow(
        ...
        raise V024StateCodecError(f"Mask row {index} is invalid") from exc
    if normalized != row:
        raise V024StateCodecError(f"Mask row {index} is not canonical")
    return normalized


def validate_calibration_mask_commitment_v024(
    commitment: V024PretruthMaskCommitment,
) -> V024PretruthMaskCommitment:
    """Recompute every count, order constraint, and mask digest."""

    if type(commitment) is not V024PretruthMaskCommitment:
        raise V024StateCodecError(
            "commitment must be an exact V024PretruthMaskCommitment"
        )
    rows = tuple(
    ...
    if (
        ...
        raise V024StateCodecError("Mask protocol identity or source count changed")
    identifiers = tuple(row.cluster_id for row in rows)
    if identifiers != tuple(sorted(identifiers)) or len(set(identifiers)) != len(
        identifiers
    ):
        raise V024StateCodecError("Mask rows are reordered or duplicated")
    eligible_count = sum(row.eligible for row in rows)
    if eligible_count < V024_MINIMUM_ELIGIBLE_COUNT:
        raise V024StateCodecError("Mask eligible count is below the V2.4 minimum")
    ...
    if commitment.eligibility_mask_sha256 != expected_digest:
        raise V024StateCodecError("Mask eligibility digest does not match its rows")
    try:
        normalized = V024PretruthMaskCommitment(
        ...
        raise V024StateCodecError("Mask commitment is invalid") from exc
    if normalized != commitment or normalized.eligible_count != eligible_count:
        raise V024StateCodecError("Mask commitment is not canonical")
    return normalized
...
def build_calibration_mask_commitment_v024(
    *,
    ...
    if not isinstance(rows, tuple) or any(
        type(row) is not V024CommittedMaskRow for row in rows
    ):
        raise V024StateCodecError("rows must be a tuple of committed mask rows")
    ...
    try:
        ...
        raise V024StateCodecError("Committed mask rows are invalid") from exc
    return validate_calibration_mask_commitment_v024(commitment)


def serialize_calibration_mask_commitment_json_v024(
    commitment: V024PretruthMaskCommitment,
) -> bytes:
    """Return the strict compact canonical commitment bytes."""

    validated = validate_calibration_mask_commitment_v024(commitment)
    raw = validated.canonical_bytes()
    if deserialize_calibration_mask_commitment_json_v024(raw) != validated:
        raise V024StateCodecError("Mask commitment failed its roundtrip")
    return raw


def deserialize_calibration_mask_commitment_json_v024(
    raw: bytes,
) -> V024PretruthMaskCommitment:
    ...
    if not isinstance(row_payloads, list):
        raise V024StateCodecError("Mask rows must be a JSON array")
    rows: list[V024CommittedMaskRow] = []
    for index, value in enumerate(row_payloads):
        ...
        if not isinstance(family_ids, list) or any(
            ...
            raise V024StateCodecError(f"mask rows[{index}] family IDs are invalid")
        if not isinstance(reasons, list) or any(
            not isinstance(item, str) for item in reasons
        ):
            raise V024StateCodecError(f"mask rows[{index}] reasons are invalid")
        ...
        try:
            ...
            raise V024StateCodecError(f"mask rows[{index}] is invalid") from exc
    try:
        commitment = V024PretruthMaskCommitment(
        ...
        raise V024StateCodecError("Mask row count or order is invalid") from exc
    validated = validate_calibration_mask_commitment_v024(commitment)
    if declared_eligible != validated.eligible_count:
        raise V024StateCodecError("Declared mask eligible count is false")
    if validated.canonical_bytes() != raw:
        raise V024StateCodecError("Mask commitment does not roundtrip canonically")
    return validated


def _mask_byte_sha256(commitment: V024PretruthMaskCommitment) -> str:
    raw = serialize_calibration_mask_commitment_json_v024(commitment)
...
def _require_mask_input_hash(
    ...
    *,
    ...
    if not isinstance(calibration_input_hashes, Mapping):
        raise V024StateCodecError("calibration_input_hashes must be an object")
...
def build_model_state_payload_v024(
    provenance_envelope: V024TrainingProvenanceEnvelope,
    *,
    software_versions: Mapping[str, str],
    created_utc: str,
) -> dict[str, Any]:
    """Build state only from a factory-issued fresh V2.4 provenance chain."""

    try:
        provenance = _require_training_envelope(provenance_envelope)
    except V024ProvenanceError as exc:
        raise V024StateCodecError(str(exc)) from exc
    ...
    try:
        ...
        raise V024StateCodecError("Inherited model state is invalid") from exc
    return _v2_to_v024_bound(
        inherited,
...
def serialize_model_state_json_v024(
    provenance_envelope: V024TrainingProvenanceEnvelope,
    *,
    software_versions: Mapping[str, str],
    created_utc: str,
    ...
    payload = build_model_state_payload_v024(
        provenance_envelope,
        software_versions=software_versions,
        created_utc=created_utc,
    )
...
def deserialize_model_state_json_v024(
    raw: bytes,
    *,
    provenance_envelope: V024TrainingProvenanceEnvelope,
) -> V024ValidatedModelStateEnvelope:
    ...
    try:
        ...
        raise V024StateCodecError(
            f"V2.4 model state is numerically invalid: {exc}"
        ) from exc
    if decoded.training_state != provenance.training_state:
        raise V024StateCodecError(
            "V2.4 numeric state does not match its training provenance"
        )
    if decoded.input_byte_hashes != provenance.input_byte_hashes:
        raise V024StateCodecError(
            "V2.4 input hashes do not match fresh bytes in its provenance"
        )
    normalized = build_model_state_payload_v024(
        provenance,
        software_versions=decoded.software_versions,
        created_utc=decoded.created_utc,
    )
    if _canonical_pretty_bytes(normalized) != raw:
        raise V024StateCodecError("V2.4 model state is not canonical numeric state")
    try:
        return _issue_validated_model_state_envelope_v024(
...
def build_training_manifest_payload_v024(
    *,
    ...
    """Build the inherited training manifest with V2.4 external identity."""
    ...
    try:
        ...
        raise V024StateCodecError("Inherited training manifest is invalid") from exc
    return _v2_to_v024(
        inherited,
...
def serialize_training_manifest_json_v024(
    *,
    center_development_input_hashes: Mapping[str, str],
    risk_development_input_hashes: Mapping[str, str],
    center_state: _v015.CenterDevelopmentState,
    ...
    return _canonical_pretty_bytes(
        build_training_manifest_payload_v024(
            center_development_input_hashes=center_development_input_hashes,
            risk_development_input_hashes=risk_development_input_hashes,
            center_state=center_state,
            risk_state=risk_state,
...
def deserialize_training_manifest_json_v024(
    raw: bytes,
    *,
    center_state: _v015.CenterDevelopmentState,
    risk_state: _v015.RiskDevelopmentState,
    ...
    try:
        ...
        raise V024StateCodecError("V2.4 training manifest is invalid") from exc
    normalized = build_training_manifest_payload_v024(
    ...
    if _canonical_pretty_bytes(normalized) != raw:
        raise V024StateCodecError("V2.4 training manifest is not canonical state")
    return payload


def build_calibration_manifest_payload_v024(
    *,
    ...
    try:
        ...
        raise V024StateCodecError("Inherited calibration manifest is invalid") from exc
    return _v2_to_v024(
        inherited,
        contract_view=contract_view,
...
def serialize_calibration_manifest_json_v024(
    *,
    calibration_input_hashes: Mapping[str, str],
    calibration_state: _v015.CalibrationDevelopmentState,
    created_utc: str,
    ...
    return _canonical_pretty_bytes(
        build_calibration_manifest_payload_v024(
            calibration_input_hashes=calibration_input_hashes,
            calibration_state=calibration_state,
            created_utc=created_utc,
            mask_commitment=mask_commitment,
...
def deserialize_calibration_manifest_json_v024(
    raw: bytes,
    *,
    calibration_state: _v015.CalibrationDevelopmentState,
    mask_commitment: V024PretruthMaskCommitment,
    ...
    try:
        ...
        raise V024StateCodecError("V2.4 calibration manifest is invalid") from exc
    _require_mask_input_hash(
        inherited["calibration_input_hashes"],
        mask_commitment=mask_commitment,
    )
    normalized = build_calibration_manifest_payload_v024(
    ...
    if _canonical_pretty_bytes(normalized) != raw:
        raise V024StateCodecError("V2.4 calibration manifest is not canonical state")
    return payload
...
def _validate_calibration_audit(
    ...
    *,
    ...
    if type(audit) is not V024CalibrationAudit:
        raise V024StateCodecError("audit must be an exact V024CalibrationAudit")
    validated_mask = validate_calibration_mask_commitment_v024(mask_commitment)
    if tuple(field.name for field in fields(audit)) != (
        *_REQUIRED_AUDIT_COUNT_FIELDS,
        "eligibility_mask_sha256",
    ):
        raise V024StateCodecError("V2.4 calibration audit field registry changed")
    counts = {
    ...
    if (
        ...
        or eligible_count < V024_MINIMUM_ELIGIBLE_COUNT
        or zero_count < 0
        or one_count < 0
        or other_count < 0
        ...
        raise V024StateCodecError("V2.4 calibration audit risk counts disagree")
    expected_zero = sum(
    ...
    if (zero_count, one_count, other_count) != (
        ...
        raise V024StateCodecError("V2.4 ineligible-family counts disagree with mask")
    if (
        ...
        raise V024StateCodecError("V2.4 calibration audit state bindings disagree")
    try:
        _v015.isotonic_state_sha256(calibration_state)
        _v015.conformal_state_sha256(calibration_state)
    except (TypeError, ValueError) as exc:
        raise V024StateCodecError("Calibration state is invalid") from exc
    return audit


def build_calibration_population_audit_payload_v024(
    *,
    provenance_envelope: V024TrainingProvenanceEnvelope,
    created_utc: str,
) -> dict[str, Any]:
...
def _validate_calibration_population_audit_payload(
    ...
    *,
    provenance_envelope: V024TrainingProvenanceEnvelope,
) -> DecodedV024CalibrationPopulationAudit:
    try:
        provenance = _require_training_envelope(provenance_envelope)
    except V024ProvenanceError as exc:
        raise V024StateCodecError(str(exc)) from exc
    ...
    if top["schema_version"] != _AUDIT_SCHEMA_VERSION:
        raise V024StateCodecError("Calibration audit schema changed")
    _require_current_bound_identity(
        top,
        protocol_id=provenance.protocol_id,
        config_sha256=provenance.config_sha256,
    ...
    if not isinstance(identifiers, list) or any(
        ...
        raise V024StateCodecError("Calibration audit mask IDs are invalid")
    if not isinstance(mask, list) or any(not isinstance(item, bool) for item in mask):
        raise V024StateCodecError("Calibration audit mask is invalid")
    ...
    if mask_byte_hash != _mask_byte_sha256(validated_mask):
        raise V024StateCodecError("Calibration audit mask byte hash mismatch")
    ...
    if isotonic_hash != _v015.isotonic_state_sha256(calibration_state):
        raise V024StateCodecError("Calibration audit isotonic state hash mismatch")
    if conformal_hash != _v015.conformal_state_sha256(calibration_state):
        raise V024StateCodecError("Calibration audit conformal state hash mismatch")
...
def serialize_calibration_population_audit_json_v024(
    *,
    provenance_envelope: V024TrainingProvenanceEnvelope,
    created_utc: str,
) -> bytes:
    payload = build_calibration_population_audit_payload_v024(
...
def deserialize_calibration_population_audit_json_v024(
    raw: bytes,
    *,
    provenance_envelope: V024TrainingProvenanceEnvelope,
) -> DecodedV024CalibrationPopulationAudit:
    ...
    normalized = build_calibration_population_audit_payload_v024(
        provenance_envelope=provenance_envelope,
        created_utc=decoded.created_utc,
    )
    if _canonical_pretty_bytes(normalized) != raw:
        raise V024StateCodecError("Calibration population audit is not canonical state")
    return decoded
...
__all__ = [
    ...
    "V024StateCodecError",
    "build_calibration_manifest_payload_v024",
    "build_calibration_mask_commitment_v024",
    "build_calibration_population_audit_payload_v024",
    "build_model_state_payload_v024",
    "build_training_manifest_payload_v024",
    ...
    "validate_calibration_mask_commitment_v024",
]
