"""Fail-closed implementation and environment guard for the V2.3 attempt."""
...
class V023EnvironmentError(RuntimeError):
    """Raised when a formal process is not the attested V2.3 implementation."""
...
def _git(repo_root: Path, *arguments: str) -> str:
    try:
        ...
        raise V023EnvironmentError(f"Cannot verify git {' '.join(arguments)}") from exc
...
def _source_hashes(repo_root: Path) -> Mapping[str, str]:
    ...
    if any(not path.is_file() for path in paths):
        raise V023EnvironmentError("A V2.3 implementation source is not a file")
...
def _strict_json_object(path: Path, *, context: str) -> dict[str, object]:
    try:
        ...
        raise V023EnvironmentError(f"{context} is invalid JSON") from exc
    if not isinstance(decoded, dict):
        raise V023EnvironmentError(f"{context} must be a JSON object")
    return decoded
...
def _reject_duplicate_keys(
    ...
    for key, value in pairs:
        if key in result:
            raise V023EnvironmentError(f"Duplicate JSON key: {key}")
        result[key] = value
    return result


def _reject_json_constant(token: str) -> None:
    raise V023EnvironmentError(f"Nonfinite JSON constant: {token}")
...
def verify_formal_environment(
    ...
    if not (root / ".git").exists():
        raise V023EnvironmentError("Formal repository root has no .git directory")
    top_level = Path(_git(root, "rev-parse", "--show-toplevel")).resolve()
    if top_level != root:
        raise V023EnvironmentError("Formal repository root is not the git top level")
    git_commit = _git(root, "rev-parse", "HEAD")
    if _FULL_GIT_HASH.fullmatch(git_commit) is None:
        raise V023EnvironmentError("Git did not return a full commit hash")
    if _git(root, "status", "--porcelain=v1", "--untracked-files=all"):
        raise V023EnvironmentError("Formal implementation worktree is dirty")
    ...
    for path in (
        ...
        if not path.is_file():
            raise V023EnvironmentError(f"Required freeze file is absent: {path}")
    try:
        design = load_v023_design(amendment_path)
    except (OSError, ValueError) as exc:
        raise V023EnvironmentError("V2.3 amendment validation failed") from exc
    ...
    if _sha256_path(prereg_path) != _PREREG_BYTE_SHA256:
        raise V023EnvironmentError("V2.3 preregistration hash changed")
    if _sha256_path(lock_path) != _LOCK_BYTE_SHA256:
        raise V023EnvironmentError("V2.3 environment lock hash changed")
    if _sha256_path(implementation_audit_path) != _IMPLEMENTATION_AUDIT_BYTE_SHA256:
        raise V023EnvironmentError("V2.3 implementation audit hash changed")
    ...
    if sys.version_info[:3] != _EXPECTED_PYTHON:
        raise V023EnvironmentError("Formal Python runtime differs from CPython 3.12.13")
    ...
    return FormalEnvironmentIdentity(
        ...
        config_canonical_sha256=V023_AMENDMENT_SEMANTIC_SHA256,
        preregistration_byte_sha256=_PREREG_BYTE_SHA256,
        environment_lock_byte_sha256=_LOCK_BYTE_SHA256,
