"""Outcome-free construction of the V2.3 actual-analysis hash ledger.
...
class V023ActualLedgerIOError(ValueError):
...
def recompute_generation_plan_commitment_bytes_v023(
    view: V023ContractView,
) -> bytes:
    """Return the exact fully enumerated frozen generation-plan bytes."""

    try:
        return audit_formal_v023_generation_plan(view).canonical_bytes
    except V023CollisionError as exc:
        raise V023ActualLedgerIOError(
            "Formal generation-plan recomputation failed"
        ) from exc


def ordinary_family_lookup_v023(
    view: V023ContractView,
...
def actual_analysis_content_records_v023(
    ...
    *,
    ...
    if set(frames) != set(_LABEL_INPUTS):
        raise V023ActualLedgerIOError("Actual-analysis source table registry changed")
    family_by_member = ordinary_family_lookup_v023(view)
    ...
    if tuple(arm_sources) != ANALYSIS_TIE_ARMS:
        raise V023ActualLedgerIOError("Actual-analysis tie-arm mapping changed")
    ...
    for partition, member_id in sorted(expected):
        ...
        if len(operating_rows) != 1:
            raise V023ActualLedgerIOError("Actual-analysis operating row is not unique")
        try:
        ...
        except (V015ArtifactError, V023CollisionError) as exc:
            raise V023ActualLedgerIOError(
...
def recompute_actual_analysis_hash_ledger_bytes_v023(
    frames: Mapping[str, pd.DataFrame],
    *,
    view: V023ContractView,
) -> bytes:
    ...
    records = actual_analysis_content_records_v023(frames, view=view)
    try:
        return bind_formal_v023_actual_analysis_hash_ledger(
            records,
            view,
        ).canonical_bytes
    except V023CollisionError as exc:
...
def verify_actual_analysis_hash_ledger_payload_v023(
...
__all__ = [
    ...
    "actual_analysis_content_records_v023",
