"""Capability-bound whole-bundle and partition validation for V2.2.
...
class V022WholeBundleContractError(ValueError):
    """Raised before slicing when a complete formal bundle is invalid."""


class V022PartitionContractError(ValueError):
    """Raised when a capability-derived partition violates its exact contract."""


class V022PartitionCapabilityError(ValueError):
...
def _partition_counts() -> Mapping[str, Mapping[str, int]]:
    ...
    for partition, values in raw.items():
        if not isinstance(partition, str) or len(values) != len(_COUNT_ORDER):
            raise V022PartitionContractError("Partition cardinality registry changed")
        parsed = {
            name: int(value) for name, value in zip(_COUNT_ORDER, values, strict=True)
        }
        if any(value <= 0 for value in parsed.values()):
            raise V022PartitionContractError("Partition cardinality is not positive")
...
def _whole_counts() -> Mapping[str, int]:
    ...
    if set(raw) != set(INPUT_FILENAMES):
        raise V022WholeBundleContractError("Whole-bundle table registry changed")
    parsed = {name: int(raw[name]) for name in INPUT_FILENAMES}
    if any(value <= 0 for value in parsed.values()):
        raise V022WholeBundleContractError("Whole-bundle cardinality is not positive")
    ...
    def __init__(
        ...
        *,
        ...
        if issuer is not _ISSUER:
            raise V022PartitionCapabilityError(
...
class ValidatedPartitionView:
    """Opaque immutable view over one exact V2.2 partition."""
...
def _require_contract(contract: FrozenArtifactContract) -> None:
    if type(contract) is not FrozenArtifactContract:
        raise V022PartitionCapabilityError("Artifact contract has the wrong exact type")
    if contract.protocol_id != V022_PROTOCOL_ID:
        raise V022PartitionCapabilityError("Artifact contract is not V2.2")


def _direct_file(root: Path, filename: str) -> Path:
    path = root / filename
    if path.parent != root or not path.is_file() or path.is_symlink():
        raise V022WholeBundleContractError(f"Invalid direct artifact: {filename}")
    return path
...
def _require_finite_numeric(frame: pd.DataFrame, *, filename: str) -> None:
    ...
    if not numeric.empty and not np.isfinite(numeric.to_numpy(dtype=float)).all():
        raise V022PartitionContractError(
...
def validate_whole_bundle_from_root(
    ...
    if not source_root.is_dir() or source_root.is_symlink():
        raise V022WholeBundleContractError("Label-free root is not a physical directory")
    ...
    for filename in INPUT_FILENAMES:
        ...
        try:
            ...
            raise V022WholeBundleContractError(str(exc)) from exc
        try:
            _require_finite_numeric(frames[filename], filename=filename)
        except V022PartitionContractError as exc:
            raise V022WholeBundleContractError(str(exc)) from exc
        if len(frames[filename]) != WHOLE_COUNTS[filename]:
            raise V022WholeBundleContractError(
    ...
    if tuple(frames) != INPUT_FILENAMES:
        raise V022WholeBundleContractError("Whole-bundle file order changed")
...
def _canonical_partition(
    ...
    *,
    ...
    try:
        ...
        raise V022PartitionContractError(str(exc)) from exc
...
def derive_partition_view(
    ...
    *,
    ...
    if type(whole) is not WholeBundleValidated:
        raise V022PartitionCapabilityError("Whole bundle capability has wrong type")
    if whole._contract_hash != contract.config_byte_sha256:
        raise V022PartitionCapabilityError("Whole capability contract binding changed")
    if partition not in PARTITION_COUNTS:
        raise V022PartitionContractError(f"Unknown frozen partition: {partition}")
...
def consume_partition_frames(
    ...
    *,
    ...
    if type(view) is not ValidatedPartitionView:
        raise V022PartitionCapabilityError("Partition capability has wrong type")
    if view._contract_hash != contract.config_byte_sha256:
        raise V022PartitionCapabilityError("Partition capability contract changed")
    counts = PARTITION_COUNTS.get(view.partition)
    if counts is None:
        raise V022PartitionCapabilityError("Partition capability identity changed")
