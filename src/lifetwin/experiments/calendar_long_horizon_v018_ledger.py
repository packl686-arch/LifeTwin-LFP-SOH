"""Strict, capability-free V2.3 exposure-ledger parsing and locking.
...
class V023LedgerError(ValueError):
    """Raised when the V2.3 append-only ledger is not exact."""
...
class FormalAttemptIdentity:
    """Immutable implementation identity for one V2.3 attempt."""
    ...
    def __post_init__(self) -> None:
        if _ATTEMPT_ID.fullmatch(self.attempt_id) is None:
            raise V023LedgerError("attempt_id is not a safe stable identifier")
        if _HEX_COMMIT.fullmatch(self.git_commit) is None:
            raise V023LedgerError("git_commit must be a full lowercase hash")
        if _SHA256.fullmatch(self.config_byte_sha256) is None:
            raise V023LedgerError("config_byte_sha256 is invalid")
...
def phase_commitment_message(phase: str, byte_sha256: str) -> str:
    ...
    try:
        ...
        raise V023LedgerError(f"{phase!r} is not a phase-specific commitment") from exc
    if not isinstance(byte_sha256, str) or _SHA256.fullmatch(byte_sha256) is None:
        raise V023LedgerError("Phase commitment is not lowercase SHA256")
...
def _strict_object(pairs: list[tuple[str, Any]]) -> dict[str, Any]:
    ...
    for key, value in pairs:
        if key in result:
            raise V023LedgerError(f"Exposure JSON contains duplicate key {key!r}")
        result[key] = value
    return result


def _reject_constant(token: str) -> None:
    raise V023LedgerError(f"Exposure JSON contains nonfinite token {token}")
...
def canonical_json_line_bytes(payload: Mapping[str, Any]) -> bytes:
    ...
    try:
        ...
        raise V023LedgerError("Exposure row is not finite canonical JSON") from exc
    return encoded + b"\n"


def _decode_canonical_line(line: bytes, *, line_number: int) -> dict[str, Any]:
    if not line.endswith(b"\n"):
        raise V023LedgerError("Exposure log has a truncated final line")
    try:
        value = json.loads(
            line.decode("ascii"),
    ...
    if not isinstance(value, dict):
        raise V023LedgerError(f"Exposure line {line_number} is not an object")
    if canonical_json_line_bytes(value) != line:
        raise V023LedgerError(f"Exposure line {line_number} is not canonical JSONL")
    return value


def _parse_timestamp(value: object) -> datetime:
    if not isinstance(value, str):
        raise V023LedgerError("Exposure event timestamp is invalid")
    normalized = value[:-1] + "+00:00" if value.endswith("Z") else value
    try:
        parsed = datetime.fromisoformat(normalized)
    except ValueError as exc:
        raise V023LedgerError("Exposure event timestamp is invalid") from exc
    if parsed.tzinfo is None or parsed.utcoffset() is None:
        raise V023LedgerError("Exposure event timestamp lacks a timezone")
    return parsed
...
def _validate_event_shape(
    ...
    *,
    ...
    if set(event) != EXPOSURE_EVENT_KEYS:
        raise V023LedgerError("Exposure event keys differ from the V2.3 freeze")
    if (
    ...
    for key in (
        ...
        if value is not None and (
            ...
            raise V023LedgerError(f"Exposure event {key} is invalid")
    ...
    if (
        ...
        raise V023LedgerError("Exposure event opened-truth list is invalid")
...
def _validate_attempt_events(
    ...
    *,
    ...
    if not events:
        raise V023LedgerError("A formal attempt has no exposure events")
    for event in events:
    ...
    for position, event in enumerate(events):
        if str(event["attempt_id"]) != identity.attempt_id:
            raise V023LedgerError("Attempt event was grouped under the wrong ID")
        if (
        ...
        if terminal_failed:
            raise V023LedgerError("A failed formal attempt has later events")

        phase = str(event["phase"])
        status = str(event["exit_status"])
        if phase not in PHASES or status not in EXIT_STATUSES:
            raise V023LedgerError("Exposure phase or exit_status is not frozen")
        expected_index = completed_index + 1
        if expected_index >= len(PHASES):
            raise V023LedgerError("A completed attempt has later events")
        ...
        if pending_phase is not None and phase != pending_phase:
            raise V023LedgerError("An interrupted or started phase was skipped")
        ...
        if not set(opened).issubset(event_opened):
            raise V023LedgerError("A formal attempt forgot an opened truth file")
        opened = event_opened
        ...
        if observed_truth_hash is not None:
            if truth_hash is None:
                ...
                raise V023LedgerError("Truth commitment changed within an attempt")
        ...
        if observed_prediction_hash is not None:
            if prediction_hash is None:
                ...
                raise V023LedgerError("Prediction commitment changed within an attempt")

        if position == 0 and (phase != "before_generation" or status != "completed"):
            raise V023LedgerError("The first event must checkpoint before generation")
...
def validate_exposure_events(
    ...
    *,
    ...
    if (
        ...
        raise V023LedgerError("Expected config hash is invalid")
...
def parse_exposure_log_bytes(
    ...
    *,
    ...
    if type(raw) is not bytes:
        raise V023LedgerError("Exposure log input must be exact bytes")
    ...
    for event in events:
        ...
        if previous is not None and observed < previous:
            raise V023LedgerError("Exposure timestamps moved backwards")
        previous = observed
...
def read_exposure_log(
    ...
    *,
    ...
    try:
        ...
        raise V023LedgerError(f"Cannot read exposure log: {target}") from exc
...
def locked_ledger(path: str | Path) -> Iterator[None]:
    """Hold the one OS-level lock shared by every V2.3 ledger writer."""
...
def append_exposure_event_cas(
    ...
    *,
    ...
    with locked_ledger(target):
        ...
        if current != prefix:
            raise V023LedgerError("Exposure ledger changed before atomic append")
        descriptor = os.open(
            target,
        ...
        if observed != prefix + raw:
            raise V023LedgerError("Exposure ledger append was not exact")
