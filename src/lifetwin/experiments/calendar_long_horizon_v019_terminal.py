"""Fail-closed V2.4 pre-prediction terminal publication.

The writer emits exactly the three terminal artifacts frozen by the V2.4
...
class V019TerminationError(ValueError):
    """Raised when terminal evidence is invalid, unsafe, or conflicting."""


class TerminalReason(str, Enum):
    """The exact reason-code registry frozen by the V2.4 amendment."""
    ...
    INTEGRITY_PARTITION_CONTRACT_MISMATCH = "INTEGRITY_PARTITION_CONTRACT_MISMATCH"
    INTEGRITY_PARTITION_CAPABILITY_MISMATCH = "INTEGRITY_PARTITION_CAPABILITY_MISMATCH"
...
_INTEGRITY_MESSAGES: Mapping[TerminalReason, str] = {
    ...
    TerminalReason.INTEGRITY_MEMBER_FIT_NUMERIC_CONTRACT_MISMATCH: (
        "A member-fit artifact violated its exact status-bound numeric contract."
    ),
    TerminalReason.INTEGRITY_PATH_ISOLATION_FAILURE: (
        "The frozen V2.4 path-isolation contract failed."
    ),
...
class V019TerminalInconclusive(RuntimeError):
...
class V019IntegrityError(RuntimeError):
...
class V019Interrupted(RuntimeError):
...
class V019ConservativeVoid(RuntimeError):
...
def _v024_training_reason(error: BaseException) -> TerminalReason | None:
    """Recognize only the shared nominal calibration signal."""

    if type(error) is not V024CalibrationTerminalInconclusive:
        return None
...
def classify_terminal_exception(error: BaseException) -> TerminalClassification:
    ...
    if type(error) in {V024EnvironmentError, V024PredictionEnvironmentError}:
        return _classification(
            error=error,
            disposition=TerminalDisposition.INTEGRITY_FAILURE,
            mode=ClassificationMode.PROVEN_INTEGRITY,
            reason=TerminalReason.INTEGRITY_ENVIRONMENT_MISMATCH,
        )
    if type(error) is V019TerminationError:
        return _classification(
            error=error,
            disposition=TerminalDisposition.INTEGRITY_FAILURE,
            mode=ClassificationMode.PROVEN_INTEGRITY,
    ...
    if type(error) is V024NumericContractError:
        return _classification(
            error=error,
            disposition=TerminalDisposition.INTEGRITY_FAILURE,
            mode=ClassificationMode.PROVEN_INTEGRITY,
    ...
    if type(error) is V024MemberFitNumericContractError:
        return _classification(
            error=error,
            disposition=TerminalDisposition.INTEGRITY_FAILURE,
            mode=ClassificationMode.PROVEN_INTEGRITY,
            reason=TerminalReason.INTEGRITY_MEMBER_FIT_NUMERIC_CONTRACT_MISMATCH,
        )
    if isinstance(error, V019ConservativeVoid):
        return _classification(
            error=error,
            disposition=TerminalDisposition.INTEGRITY_FAILURE,
    ...
    if isinstance(error, V019TerminalInconclusive):
        return _classification(
            error=error,
    ...
    training_reason = _v024_training_reason(error)
    ...
    if isinstance(error, V019Interrupted):
        return _classification(
            error=error,
...
def _canonical_json_bytes(payload: Mapping[str, Any]) -> bytes:
    try:
        ...
        raise V019TerminationError("Terminal JSON is not canonicalizable") from exc
    return f"{text}\n".encode("ascii")
...
def sanitized_structural_traceback(
    ...
    *,
    ...
    if (
        ...
        raise V019TerminationError("Structural traceback sanitization failed")
    return raw
...
def _validate_exact_keys(
    ...
    *,
    ...
    if not isinstance(value, Mapping) or set(value) != keys:
        raise V019TerminationError(f"{context} keys are not exact")
    return value
...
def _validate_identity(
    ...
    *,
    ...
    if (
        ...
        raise V019TerminationError(f"{context} identity is invalid")
    ...
    *,
    ...
    for phase_name in ("phase", "last_completed_phase"):
        if (
            ...
            raise V019TerminationError("Terminal attempt phase is invalid")
    try:
        reason = TerminalReason(record["reason_code"])
        mode = ClassificationMode(record["classification_mode"])
    except (TypeError, ValueError) as exc:
        raise V019TerminationError("Terminal classification code is invalid") from exc
    ...
    if (
        ...
        raise V019TerminationError("Terminal disposition is invalid")
    ...
    if (
        ...
        raise V019TerminationError("Terminal truth exposure is invalid")
    ...
    if (
        ...
        raise V019TerminationError("Sanitized stderr metadata is invalid")
...
def _validate_preterminal_file_record(value: object) -> Mapping[str, Any]:
    ...
    if (
        ...
        raise V019TerminationError("Preterminal artifact record is invalid")
    return record
...
def validate_terminal_artifact_manifest(
    ...
    *,
    ...
    if not isinstance(preterminal, list) or not preterminal:
        raise V019TerminationError("Preterminal artifact registry is invalid")
    ...
    if (
        ...
        raise V019TerminationError("Preterminal artifact registry is incomplete")
    ...
    for key, value in commitments.items():
        if key == "prediction_commitment_byte_sha256":
            if value is not None:
                raise V019TerminationError("Prediction commitment is forbidden")
        elif value is not None and (
            not isinstance(value, str) or _SHA256.fullmatch(value) is None
        ):
            raise V019TerminationError("Preterminal commitment is invalid")
    ...
    for commitment_name, path in commitment_paths.items():
        ...
        if committed is not None and observed is None:
            raise V019TerminationError("Committed preterminal artifact is absent")
        if committed is not None and observed["byte_sha256"] != committed:
            raise V019TerminationError("Preterminal artifact commitment hash differs")
    ...
    if policy != {
        ...
        raise V019TerminationError("Terminal artifact policy is false")
    if (
...
def _parse_ledger_prefix(raw: bytes, context: TerminalContext) -> LedgerSnapshot:
    if not raw or not raw.endswith(b"\n"):
        raise V019TerminationError("Preterminal ledger is empty or truncated")
    try:
        ...
        raise V019TerminationError(
            "Preterminal ledger failed the strict canonical V2.4 parser"
        ) from exc
    try:
        progress = states[context.attempt_id]
    except KeyError:
        raise V019TerminationError("Attempt is absent from the preterminal ledger")
    if (
        ...
        raise V019IntegrityError(
            TerminalReason.INTEGRITY_CONFIG_OR_SOURCE_HASH_MISMATCH
        )
    opened = set(progress.opened_truth_files)
    if opened & _HELD_OUT_TRUTH:
        raise V019IntegrityError(TerminalReason.INTEGRITY_FORBIDDEN_TRUTH_ACCESS)
    if opened - _DEVELOPMENT_TRUTH:
        raise V019IntegrityError(TerminalReason.INTEGRITY_INFORMATION_LEAK)
    ...
    if progress.truth_commitments_byte_sha256 != context.truth_commitments_byte_sha256:
        raise V019IntegrityError(TerminalReason.INTEGRITY_MISSING_COMMITMENT)
    ...
    if (
        ...
        raise V019IntegrityError(TerminalReason.INTEGRITY_ARTIFACT_HASH_MISMATCH)
...
def _build_attempt_record(
    *,
    ...
    try:
        ...
        raise V019TerminationError("Sanitized structural traceback is invalid") from exc
...
def build_termination_manifest(
    *,
    ...
    """Compatibility alias for the exact V2.4 artifact-manifest builder."""
...
def _validate_terminal_ledger_record(
    ...
    *,
    ...
    if record != expected:
        raise V019TerminationError("Terminal ledger commitment conflicts")
    ...
    *,
    ...
    if current == expected_prefix:
        ...
        if path.read_bytes() != expected_prefix + record_raw:
            raise V019TerminationError("Terminal ledger append was not exact")
        return True
    if not current.startswith(expected_prefix):
        raise V019TerminationError("Live ledger prefix changed after snapshot")
    suffix = current[len(expected_prefix) :]
    if not suffix.endswith(b"\n") or len(suffix.splitlines()) != 1:
        raise V019TerminationError("Terminal ledger has later or malformed records")
...
def _canonical_mapping_from_bytes(
    ...
    *,
    ...
    try:
        ...
        raise V019TerminationError(f"Existing {context} is invalid") from exc
    if not isinstance(value, Mapping):
        raise V019TerminationError(f"Existing {context} is not an object")
    if _canonical_json_bytes(value) != raw:
        raise V019TerminationError(f"Existing {context} is noncanonical")
    return value
...
def _resolve_publication_roots(
    *,
    ...
    def physical_root(raw: str | Path, *, context: str) -> Path:
        ...
        for candidate in (lexical, *lexical.parents):
            if os.path.lexists(candidate) and _is_reparse(candidate):
                raise V019TerminationError(f"{context} root traverses a reparse point")
        resolved = lexical.resolve()
        if not resolved.is_dir():
            raise V019TerminationError("Terminal publication roots must exist")
    ...
    if (
        ...
        raise V019TerminationError("Termination and label-free roots must be disjoint")
    ...
    *,
    ...
    with _publication_lock(root), locked_ledger(ledger):
        ...
        if any(
            ...
            raise V019TerminationError("Terminal registry contains unsafe entries")
        ...
        for name, path in observed_paths.items():
            if path.read_bytes() != expected_bytes[name]:
                raise V019TerminationError(f"Terminal artifact conflicts: {name}")
...
__all__ = [
    ...
    "TerminalDisposition",
    "TerminalReason",
    "V019ConservativeVoid",
    "V019IntegrityError",
    "V019Interrupted",
    "V019TerminalInconclusive",
