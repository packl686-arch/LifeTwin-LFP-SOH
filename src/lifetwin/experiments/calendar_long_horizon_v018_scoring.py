"""Committed-artifact scoring adapter for the frozen V2.3 protocol.
...
stochastic roots, and the V2.3 risk-calibration population.  Formal scoring
requires an IO-issued committed model capability and an envelope bound to
verified V2.3 prediction-commitment evidence; it never accepts a bare V2
decoded model state.
"""
...
from lifetwin.experiments.calendar_long_horizon_v018_analysis import (
    BOOTSTRAP_RESAMPLES,
...
from lifetwin.experiments.calendar_long_horizon_v018_io import (
    ...
    _require_prediction_commitment_evidence_v023,
)
from lifetwin.experiments.calendar_long_horizon_v018_pipeline import (
    V023PipelineError,
...
class V023ScoringError(ValueError):
    """Raised when committed V2.3 scoring cannot prove its boundaries."""
...
_ATTEMPT_ID = re.compile(r"^v023-[A-Za-z0-9][A-Za-z0-9._:-]{0,95}$")
...
_PREDICTION_ENVELOPE_DOMAIN = b"lifetwin-v023-scoring-prediction-envelope-v1\0"
...
if _FORMAL_ANALYSIS_COUNTS != _AnalysisCounts(
    ...
    raise RuntimeError("V2.3 formal analysis cardinalities drifted")
if (
    V023_EXPECTED_SEED_ROOTS["random_rankings"] != 202608100310
    or V023_EXPECTED_SEED_ROOTS["bootstrap"] != 202608100311
    or V023_EXPECTED_SEED_ROOTS["stress_permutations"] != 202608100312
):
    raise RuntimeError("V2.3 formal stochastic roots drifted")


@dataclass(frozen=True, slots=True)
class V023ScoringResult:
    """The ten-artifact V2.3 scoring registry in structured form."""
...
class V023PredictionCommitmentEnvelope:
    """Opaque scoring capability bound to verified V2.3 IO evidence."""
    ...
    def __init__(
        ...
        *,
        ...
        config_sha256: str,
        attempt_id: str,
        evidence: V023PredictionCommitmentEvidence,
        ...
        if (
            ...
            or type(self) is not V023PredictionCommitmentEnvelope
        ):
            raise TypeError(
                "V023PredictionCommitmentEnvelope is issued only from an exact "
    ...
    def __setattr__(self, name: str, value: object) -> None:
        raise AttributeError("V2.3 prediction commitment envelopes are immutable")
...
def _sha256(value: object, *, context: str) -> str:
    if not isinstance(value, str) or _SHA256.fullmatch(value) is None:
        raise V023ScoringError(f"{context} must be a lowercase SHA-256")
    return value


def _component(value: object, *, context: str) -> str:
    if not isinstance(value, str) or _COMPONENT.fullmatch(value) is None:
        raise V023ScoringError(f"{context} must be a canonical component")
    return value


def _contract(view: V023ContractView | None = None) -> V023ContractView:
    contract = load_v023_contract_view() if view is None else view
    if type(contract) is not V023ContractView:
        raise V023ScoringError("The V2.3 contract view has an invalid type")
    if (
        contract.protocol.protocol_id != V023_PROTOCOL_ID
        or contract.artifacts.protocol_id != V023_PROTOCOL_ID
        or contract.protocol.config_sha256 != contract.artifacts.config_byte_sha256
    ):
        raise V023ScoringError("The active artifact contract is not exact V2.3")
    return contract
...
def _require_evidence_model_binding(
    evidence: V023PredictionCommitmentEvidence,
    model: V023CommittedModelStateEnvelope,
) -> None:
    entries = {
...
def _issue_prediction_commitment_envelope_v023(
    *,
    prediction_frames: Mapping[str, pd.DataFrame],
    model_state_envelope: V023CommittedModelStateEnvelope,
    prediction_commitment_evidence: V023PredictionCommitmentEvidence,
    formal: bool,
    contract_view: V023ContractView | None = None,
) -> V023PredictionCommitmentEnvelope:
    ...
    try:
        ...
        raise V023ScoringError(str(exc)) from exc
    if (
        model.protocol_id != V023_PROTOCOL_ID
        or model.config_sha256 != view.artifacts.config_byte_sha256
    ):
        raise V023ScoringError("Committed model identity differs from V2.3")
    try:
        evidence = _require_prediction_commitment_evidence_v023(
    ...
    return V023PredictionCommitmentEnvelope(
...
def _require_prediction_envelope(
    ...
    *,
    ...
    if (
        type(value) is not V023PredictionCommitmentEnvelope
        or value._issuer_key is not _PREDICTION_ENVELOPE_KEY
    ):
        raise V023ScoringError("An exact V023PredictionCommitmentEnvelope is required")
    envelope = value
    if (
        envelope.protocol_id != V023_PROTOCOL_ID
        or envelope.config_sha256 != view.artifacts.config_byte_sha256
        or envelope.attempt_id != model.attempt_id
    ):
        raise V023ScoringError("Prediction commitment provenance identity changed")
    try:
        evidence = _require_prediction_commitment_evidence_v023(
            envelope._evidence,
            require_ledger_committed=True,
    ...
    if (
        ...
        raise V023ScoringError("Prediction commitment IO evidence changed")
    _require_evidence_model_binding(evidence, model)
    commitment_hash = _sha256(
    ...
    if tuple(name for name, _, _, _ in metadata) != _PREDICTION_FRAME_FILENAMES:
        raise V023ScoringError("Prediction commitment artifact registry changed")
    ...
    if envelope.provenance_sha256 != expected:
        raise V023ScoringError("Prediction commitment envelope digest changed")
    return envelope
...
def _canonical_committed_equal(
    ...
    *,
    ...
    if canonical_csv_bytes(
        ...
        raise V023ScoringError(
            f"{filename} differs byte-for-byte from V2.3 recomputation"
        )


def validate_and_recompute_committed_predictions_v023(
    *,
    prediction_frames: Mapping[str, pd.DataFrame],
    model_state_envelope: V023CommittedModelStateEnvelope,
    prediction_commitment_envelope: V023PredictionCommitmentEnvelope,
    formal: bool,
    contract_view: V023ContractView | None = None,
    ...
    recomputed = recompute_label_free_pipeline_v023(
...
def _policy_comparison_v023(
    ...
    *,
    ...
    if len(random_rankings) < 1:
        raise V023ScoringError("Random-ranking table is empty")
    random_mean = float(
        pd.to_numeric(
    ...
    if not math.isfinite(random_mean) or random_mean <= 0.0:
        raise V023ScoringError("Random-ranking mean risk is unavailable")
...
def _risk_coverage_curves_v023(test: pd.DataFrame) -> list[dict[str, Any]]:
...
def _isotonic_calibration_diagnostics_v023(
...
def _bootstrap_summary_v023(
    ...
    *,
    ...
    if len(replicates) != expected_count:
        raise V023ScoringError("Bootstrap replicate count changed")
...
def canonicalize_score_frame_v023(
    frame: pd.DataFrame,
    filename: str,
    *,
    ...
    if "protocol_id" in result.columns and not result.empty:
        if set(result["protocol_id"].astype(str)) != {V023_PROTOCOL_ID}:
            raise V023ScoringError(f"{filename} protocol_id changed")
    return result


def canonical_score_csv_bytes_v023(
    frame: pd.DataFrame,
    filename: str,
    *,
    allow_empty: bool = False,
) -> bytes:
    ordered = canonicalize_score_frame_v023(
        frame,
        filename,
        allow_empty=allow_empty,
    )
...
def canonical_result_summary_bytes_v023(payload: Mapping[str, Any]) -> bytes:
    if set(payload) != SCORE_REPORT_KEYS:
        raise V023ScoringError("score_report.json keys changed")
    if payload.get("protocol_id") != V023_PROTOCOL_ID:
        raise V023ScoringError("score_report.json protocol_id changed")
...
def _json_artifact_bytes(
    ...
    *,
    ...
    if set(payload) != expected_keys:
        raise V023ScoringError(f"{filename} keys changed")
    if payload.get("protocol_id") != V023_PROTOCOL_ID:
        raise V023ScoringError(f"{filename} protocol_id changed")
    return canonical_json_bytes(payload)
...
def _payloads_without_manifest(result: V023ScoringResult) -> dict[str, bytes]:
    ...
    for filename, value in _artifacts_without_manifest(result).items():
        if filename.endswith(".csv"):
            if not isinstance(value, pd.DataFrame):
                raise V023ScoringError(f"{filename} must be a dataframe")
            payloads[filename] = canonical_score_csv_bytes_v023(
                value,
                filename,
                allow_empty=allow_empty,
            )
            ...
            payloads[filename] = canonical_result_summary_bytes_v023(value)
    if tuple(payloads) != REQUIRED_SCORE_ARTIFACTS[:-1]:
        raise V023ScoringError("V2.3 score artifact registry changed")
    return payloads
...
def _finalize_registry(
    ...
    *,
    model: V023CommittedModelStateEnvelope,
    prediction: V023PredictionCommitmentEnvelope,
    view: V023ContractView,
) -> V023ScoringResult:
    ...
    manifest = {
        "schema_version": "lifetwin.v023.scored_registry.v1",
        "protocol_id": V023_PROTOCOL_ID,
        "config_sha256": view.artifacts.config_byte_sha256,
        "attempt_id": model.attempt_id,
    ...
    if set(manifest) != _RUN_MANIFEST_KEYS:
        raise V023ScoringError("run_manifest.json keys changed")
    return replace(result, run_manifest=manifest)


def required_score_artifact_payloads_v023(
    result: V023ScoringResult,
) -> dict[str, bytes]:
    """Return all ten committed V2.3 score artifacts as canonical bytes."""

    if type(result) is not V023ScoringResult:
        raise V023ScoringError("An exact V023ScoringResult is required")
    manifest = result.run_manifest
    if not isinstance(manifest, Mapping) or set(manifest) != _RUN_MANIFEST_KEYS:
        raise V023ScoringError("run_manifest.json is not finalized")
    ...
    if (
        manifest.get("schema_version") != "lifetwin.v023.scored_registry.v1"
        or manifest.get("protocol_id") != V023_PROTOCOL_ID
        or manifest.get("config_sha256")
        != load_v023_contract_view().artifacts.config_byte_sha256
        ...
        raise V023ScoringError("run_manifest.json identity or registry changed")
    payloads = _payloads_without_manifest(result)
    artifacts = _artifacts_without_manifest(result)
    expected_records = [
        {
    ...
    if tuple(payloads) != REQUIRED_SCORE_ARTIFACTS:
        raise V023ScoringError("The complete V2.3 score registry changed")
    return payloads
...
def _terminal_result(
    ...
    *,
    ...
    if status_kind not in {"void", "inconclusive_not_success"}:
        raise V023ScoringError("Unknown terminal score status")
    unavailable = (
...
def _score_committed_artifacts_v023(
    *,
    prediction_frames: Mapping[str, pd.DataFrame],
    ...
    decoded, recomputed = validate_and_recompute_committed_predictions_v023(
    ...
    policy_test, test_reductions = _policy_comparison_v023(
        test,
        random_rankings,
        issue_count=TEST_ISSUE_COUNT,
        partition="test",
    ...
    policy_audit, _ = _policy_comparison_v023(
        audit,
        audit_random,
        issue_count=AUDIT_ISSUE_COUNT,
        partition="audit",
    ...
    bootstrap_summary = _bootstrap_summary_v023(
        bootstrap,
        issue_count=TEST_ISSUE_COUNT,
    ...
    risk_coverage_curves = _risk_coverage_curves_v023(test)
    calibration_audit = (
        model_state_envelope.validated_model_state.training_provenance.calibration_audit
    )
    isotonic_diagnostics = _isotonic_calibration_diagnostics_v023(
        trajectories,
        prediction_frames["risk_bundle.csv"],
        decoded,
    ...
    canonical = {
        name: canonicalize_score_frame_v023(frame, name)
        for name, frame in frames.items()
    }
    canonical_result_summary_bytes_v023(score_report)
    result = V023ScoringResult(
...
def score_committed_artifacts(
    *,
    ...
    """Run the formal V2.3 10k/5k/10k committed-artifact scorer."""
    ...
    try:
        return _score_committed_artifacts_v023(
            prediction_frames=prediction_frames,
...
score_committed_artifacts_v023 = score_committed_artifacts
required_score_artifact_payloads = required_score_artifact_payloads_v023
canonicalize_score_frame = canonicalize_score_frame_v023
canonical_score_csv_bytes = canonical_score_csv_bytes_v023
canonical_result_summary_bytes = canonical_result_summary_bytes_v023


def _run_stochastic_fixture_analyses_v023(
    trajectories: pd.DataFrame,
    *,
    issue_count: int,
    counts: _AnalysisCounts,
...
__all__ = [
    ...
    "canonical_result_summary_bytes_v023",
    "canonical_score_csv_bytes",
    "canonical_score_csv_bytes_v023",
    "canonicalize_score_frame",
    "canonicalize_score_frame_v023",
    "required_score_artifact_payloads",
    "required_score_artifact_payloads_v023",
    "score_committed_artifacts",
    "score_committed_artifacts_v023",
    "validate_and_recompute_committed_predictions_v023",
]
