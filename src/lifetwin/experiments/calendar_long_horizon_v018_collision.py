"""RNG-free generation-coordinate namespace commitments for V2.3.

The formal entry point enumerates the complete V2.3 and V2 coordinate plans.
...
class V023CollisionError(ValueError):
...
def _component(value: object, *, context: str) -> str:
    if (
        ...
        raise V023CollisionError(f"{context} is not a canonical ASCII component")
    return value
...
def _positive_integer(value: object, *, context: str) -> int:
    if (
        ...
        raise V023CollisionError(f"{context} must be a positive bounded integer")
    return value


def _count(value: object, *, context: str) -> int:
    if isinstance(value, bool) or not isinstance(value, int) or value < 1:
        raise V023CollisionError(f"{context} must be a positive integer")
    return value


def _sha256(value: object, *, context: str) -> str:
    if not isinstance(value, str) or _SHA256.fullmatch(value) is None:
        raise V023CollisionError(f"{context} must be a lowercase SHA256")
    return value
...
def _canonical_json_bytes(value: object) -> bytes:
    try:
        ...
        raise V023CollisionError("Commitment payload is not canonical JSON") from exc
...
class ActualHashDomainCommitment:
    ...
    def __post_init__(self) -> None:
        ...
        if (
            ...
            raise V023CollisionError("Actual-hash domain count is invalid")
        _sha256(
...
class CoordinateCollisionLedger:
    ...
    def register_seed(self, *, label: str, seed: int) -> None:
        ...
        if (
            ...
            raise V023CollisionError(f"{label} is not a valid derived seed")
        prior = self._seeds.get(seed)
        if prior is not None:
            raise V023CollisionError(f"Seed collision between {prior!r} and {label!r}")
    ...
    def register_identifier(self, *, label: str, identifier: str) -> None:
        ...
        if (
            ...
            raise V023CollisionError(f"{label} is not a canonical opaque identifier")
...
def _component_path(value: object, *, context: str) -> str:
    if not isinstance(value, str) or not value or not value.isascii():
        raise V023CollisionError(f"{context} must be nonempty ASCII")
    parts = value.split("/")
    if any(_COMPONENT.fullmatch(part) is None for part in parts):
        raise V023CollisionError(f"{context} is not a canonical component path")
    return value
...
def _validate_string_tuple(
    ...
    *,
    ...
    if not isinstance(value, tuple) or not value:
        raise V023CollisionError(f"{context} must be a nonempty tuple")
    result = tuple(_component(item, context=f"{context} member") for item in value)
    if len(result) != len(set(result)):
        raise V023CollisionError(f"{context} contains duplicates")
    return result
...
def _validate_plan_spec(spec: GenerationPlanSpec) -> None:
    ...
    if not isinstance(spec.seed_roots, tuple) or not spec.seed_roots:
        raise V023CollisionError("Seed roots must be a nonempty ordered tuple")
    root_names: list[str] = []
    root_values: list[int] = []
    for item in spec.seed_roots:
        if not isinstance(item, tuple) or len(item) != 2:
            raise V023CollisionError("Each seed root must be a name/value pair")
        name, value = item
    ...
    if spec.bootstrap_partition not in ordinary_by_partition:
        raise V023CollisionError("Bootstrap partition is not ordinary")
    ...
    if any(partition not in ordinary_by_partition for partition in ranking_partitions):
        raise V023CollisionError("A ranking partition is not ordinary")
...
def derive_stream_seed(
    ...
    *,
    ...
    if (
        ...
        raise V023CollisionError("Stream index must be a nonnegative integer")
...
def derive_bootstrap_seed(
    ...
    *,
    ...
    if (
        ...
        raise V023CollisionError("Bootstrap index must be a nonnegative integer")
...
def derive_random_ranking_digest(
    *,
    ...
    if (
        ...
        raise V023CollisionError("Ranking index must be a nonnegative integer")
...
def derive_stress_permutation_digest(
    *,
    ...
    if (
        ...
        raise V023CollisionError("Permutation index must be a nonnegative integer")
    ...
    def register(
        ...
        *,
        ...
        if domain not in self.domain_hashers:
            raise V023CollisionError("Internal actual-hash domain is invalid")
...
def _validate_expected_actual_groups(
    ...
    for item in expected_group_counts:
        ...
        if key in expected:
            raise V023CollisionError("Expected actual-hash groups contain duplicates")
...
def bind_actual_analysis_hash_ledger(
    *,
    ...
    if canonical_stress_partition not in partitions:
        raise V023CollisionError("Actual stress partition is not a ranking partition")
    ...
    if any(type(record) is not ActualAnalysisContentRecord for record in records):
        raise V023CollisionError("Actual analysis content record type is invalid")
    ...
    for record in records:
        ...
        if key in keys:
            raise V023CollisionError("Actual analysis content coordinate is duplicated")
        keys.add(key)
    ...
    if any(
        ...
        raise V023CollisionError("An actual random-ranking partition is absent")
    ordered = tuple(
        sorted(
            records,
            key=lambda record: (
    ...
    if any(not family_records for family_records in stress_records_by_family.values()):
        raise V023CollisionError("An actual stress-permutation family is absent")
    ...
    def register(
        ...
        *,
        ...
        if domain not in self.domain_hashers or kind not in self.kind_counts:
            raise V023CollisionError("Internal ledger domain or kind is invalid")
        coordinate_text = "/".join(str(item) for item in coordinate)
        label = f"{self.role}/{kind}/{coordinate_text}"
        if kind == "seed":
            if not isinstance(value, int):
                raise V023CollisionError("Internal seed ledger value is invalid")
            self.ledger.register_seed(label=label, seed=value)
        elif kind == "identifier":
            if not isinstance(value, str):
                raise V023CollisionError("Internal identifier ledger value is invalid")
            self.ledger.register_identifier(label=label, identifier=value)
        else:
            if not isinstance(value, str):
                raise V023CollisionError("Internal digest ledger value is invalid")
...
def _enumerate_plan(
    *,
    ...
    if sum(item.record_count for item in domains) != counts.ordered_ledger_records:
        raise V023CollisionError("Expanded ledger domain counts are inconsistent")
...
def commit_generation_coordinate_namespaces(
    *,
    ...
    if type(current) is not GenerationPlanSpec:
        raise V023CollisionError("Current plan has an invalid type")
    if type(predecessor) is not GenerationPlanSpec:
        raise V023CollisionError("Predecessor plan has an invalid type")
    _validate_plan_spec(current)
    _validate_plan_spec(predecessor)
    if current.protocol_id == predecessor.protocol_id:
        raise V023CollisionError("Current and predecessor protocol IDs must differ")
    if current.protocol_byte_sha256 == predecessor.protocol_byte_sha256:
        raise V023CollisionError("Current and predecessor byte identities collide")
    if current.protocol_semantic_sha256 == predecessor.protocol_semantic_sha256:
        raise V023CollisionError("Current and predecessor semantic identities collide")
    if set(current.seed_root_map().values()).intersection(
        predecessor.seed_root_map().values()
    ):
        raise V023CollisionError("Current and predecessor seed roots overlap")
...
def _validate_formal_analysis_plan_lock(
    ...
    for role, spec in (("current", current), ("predecessor", predecessor)):
        if spec.bootstrap_resamples != _FORMAL_BOOTSTRAP_RESAMPLES:
            raise V023CollisionError(f"Formal {role} bootstrap count changed")
        if spec.random_ranking_count != _FORMAL_RANDOM_RANKINGS:
            raise V023CollisionError(f"Formal {role} random-ranking count changed")
        if spec.stress_permutation_count != _FORMAL_STRESS_PERMUTATIONS:
            raise V023CollisionError(f"Formal {role} stress-permutation count changed")
        if spec.ranking_partitions != _FORMAL_RANKING_PARTITIONS:
            raise V023CollisionError(f"Formal {role} ranking partitions changed")
        if spec.bootstrap_partition != "test" or spec.stress_partition != "test":
            raise V023CollisionError(f"Formal {role} analysis partition changed")
        if (
            spec.bootstrap_families != _FORMAL_TEST_FAMILIES
            or spec.stress_families != _FORMAL_TEST_FAMILIES
        ):
            raise V023CollisionError(f"Formal {role} analysis families changed")
        if spec.analysis_tie_arms != ANALYSIS_TIE_ARMS:
            raise V023CollisionError(f"Formal {role} tie-arm families changed")


def build_formal_plan_specs(
    view: V023ContractView | None = None,
) -> tuple[GenerationPlanSpec, GenerationPlanSpec]:
    """Build, but do not enumerate, the frozen-size V2.3 and V2 plans."""

    contract = load_v023_contract_view() if view is None else view
    if type(contract) is not V023ContractView:
        raise V023CollisionError("Formal contract view has an invalid type")
    protocol = contract.protocol
    if protocol.protocol_id != V023_PROTOCOL_ID:
        raise V023CollisionError("Formal current protocol identity is not V2.3")
    if protocol.config_sha256 != contract.artifacts.config_byte_sha256:
        raise V023CollisionError("Formal V2.3 amendment commitments disagree")
    current_roots = tuple(protocol.seed_roots)
    if current_roots != tuple(V023_EXPECTED_SEED_ROOTS.items()):
        raise V023CollisionError("Formal V2.3 seed roots changed")
    if set(dict(current_roots).values()).intersection(
        (*V2_SEED_ROOTS, *V021_SEED_ROOTS, *V022_SEED_ROOTS)
    ):
        raise V023CollisionError("Formal V2.3 roots collide with a predecessor")
    ...
    if contract.base_config_byte_sha256 == protocol.config_sha256:
        raise V023CollisionError("Formal current and predecessor hashes collide")
    ...
    if test_families != _FORMAL_TEST_FAMILIES:
        raise V023CollisionError("Formal V2.3 test-family registry changed")
    ...
    current = GenerationPlanSpec(
        ...
        protocol_semantic_sha256=V023_AMENDMENT_SEMANTIC_SHA256,
...
def audit_formal_v023_generation_plan(
    view: V023ContractView | None = None,
) -> GenerationCoordinateNamespaceCommitment:
...
def bind_formal_v023_actual_analysis_hash_ledger(
    records: tuple[ActualAnalysisContentRecord, ...],
    view: V023ContractView | None = None,
) -> ActualAnalysisHashLedgerCommitment:
...
def verify_generation_coordinate_namespace_commitment(
    ...
    *,
    ...
    if not all(isinstance(item, Mapping) for item in (current, predecessor, cross)):
        raise V023CollisionError("Generation-plan commitment sections are invalid")
    ...
    for name, section in (("current", current), ("predecessor", predecessor)):
        if set(section) != expected_plan_keys:
            raise V023CollisionError(f"{name} plan commitment keys changed")
        ...
        if not isinstance(section.get("counts"), Mapping):
            raise V023CollisionError(f"{name} counts are invalid")
        if not isinstance(section.get("coordinate_domains"), list):
            raise V023CollisionError(f"{name} coordinate domains are invalid")
        if not isinstance(section.get("seed_roots"), list):
            raise V023CollisionError(f"{name} seed roots are invalid")
    ...
    if set(cross) != expected_cross_keys:
        raise V023CollisionError("Cross-protocol namespace commitment keys changed")
    if (
    ...
    if (
        ...
        raise V023CollisionError("Formula-witness content hash changed")
    if cross.get("ledger_order") != "current_then_predecessor":
        raise V023CollisionError("Complete namespace ledger order changed")
    if cross.get("scope") != (
        "actual_seeds_identifiers_identity_digests_and_"
        "content_independent_formula_witnesses"
    ):
        raise V023CollisionError("Pre-generation namespace scope changed")
    _sha256(
    ...
    if isinstance(records, bool) or not isinstance(records, int) or records < 1:
        raise V023CollisionError("Complete ledger record count is invalid")
    ...
    for name, section, protocol_id, protocol_hash in expected_identities:
        if protocol_id is not None and section.get("protocol_id") != protocol_id:
            raise V023CollisionError(f"{name} protocol identity changed")
        if protocol_hash is not None:
            ...
            if section.get("protocol_byte_sha256") != protocol_hash:
                raise V023CollisionError(f"{name} protocol byte identity changed")
...
def verify_actual_analysis_hash_ledger_commitment(
    ...
    *,
    ...
    if not isinstance(payload, Mapping):
        raise V023CollisionError("Actual analysis hash commitment must be an object")
    expected_keys = {
        "coordinate_semantics",
        "domains",
        "ledger_counts",
    ...
    if set(payload) != expected_keys:
        raise V023CollisionError("Actual analysis hash commitment keys changed")
    if payload.get("schema_version") != _ACTUAL_HASH_COMMITMENT_SCHEMA:
        raise V023CollisionError("Actual analysis hash commitment schema changed")
    if payload.get("stage") != "post_generation_pre_prediction":
        raise V023CollisionError("Actual analysis hash commitment stage changed")
    ...
    if not all(
        ...
        raise V023CollisionError("Actual analysis hash registries are invalid")
    ...
    if stress_partition not in partitions:
        raise V023CollisionError("Committed stress partition is not ranked")
    ...
    if semantics != {
        ...
        raise V023CollisionError("Actual analysis hash coordinate semantics changed")

    domains = payload.get("domains")
    if not isinstance(domains, list) or len(domains) != len(_ACTUAL_HASH_DOMAIN_ORDER):
        raise V023CollisionError("Actual analysis hash domains are invalid")
    ...
    for expected_name, section in zip(
        ...
        if not isinstance(section, Mapping) or set(section) != {
            ...
            raise V023CollisionError("Actual analysis hash domain keys changed")
        if section.get("name") != expected_name:
            raise V023CollisionError("Actual analysis hash domain order changed")
        _sha256(
        ...
        if isinstance(count, bool) or not isinstance(count, int) or count < 0:
            raise V023CollisionError("Actual analysis hash domain count is invalid")
    ...
    if not isinstance(ledger_counts, Mapping) or set(ledger_counts) != (
        ...
        raise V023CollisionError("Actual analysis ledger counts changed")
    for name, count in domain_counts.items():
        if ledger_counts.get(name) != count:
            raise V023CollisionError("Actual analysis domain counts disagree")
    ...
    if (
        ...
        raise V023CollisionError("Actual analysis source count is invalid")
    if ledger_counts.get("complete_ordered_ledger_records") != sum(
        domain_counts.values()
    ):
        raise V023CollisionError("Actual analysis total ledger count disagrees")
    if domain_counts["predictor_content_hash"] != source_count * len(arms):
        raise V023CollisionError("Actual predictor content count disagrees")
    if domain_counts["analysis_tie_hash"] != source_count * len(arms):
        raise V023CollisionError("Actual tie-hash count disagrees")
    if domain_counts["random_ranking_hash"] != source_count * ranking_count:
        raise V023CollisionError("Actual random-ranking hash count disagrees")
    ...
    if not isinstance(source_counts, Mapping) or set(source_counts) != {
        ...
        raise V023CollisionError("Actual analysis source-coordinate counts changed")
    ...
    if not isinstance(partition_sections, list) or not isinstance(
        ...
        raise V023CollisionError("Actual analysis source counts are invalid")
    partition_total = 0
    ...
    for section in partition_sections:
        if not isinstance(section, Mapping) or set(section) != {
            ...
            raise V023CollisionError("Actual ranking-partition counts are invalid")
    ...
    if tuple(observed_partitions) != partitions or partition_total != source_count:
        raise V023CollisionError("Actual ranking-partition counts disagree")
    ...
    for section in family_sections:
        if not isinstance(section, Mapping) or set(section) != {
            ...
            raise V023CollisionError("Actual stress-family counts are invalid")
    ...
    if tuple(observed_families) != families:
        raise V023CollisionError("Actual stress-family order changed")
    if domain_counts["stress_permutation_hash"] != stress_total * permutation_count:
        raise V023CollisionError("Actual stress-permutation hash count disagrees")
    ...
    if expected_protocol_id is not None and protocol_id != expected_protocol_id:
        raise V023CollisionError("Actual analysis protocol identity changed")
    if (
        expected_random_ranking_root is not None
        and random_root != expected_random_ranking_root
    ):
        raise V023CollisionError("Actual analysis random-ranking root changed")
    if (
        expected_stress_permutation_root is not None
        and stress_root != expected_stress_permutation_root
    ):
        raise V023CollisionError("Actual analysis stress-permutation root changed")

    observed = hashlib.sha256(_canonical_json_bytes(payload) + b"\n").hexdigest()
    if observed != expected_hash:
        raise V023CollisionError("Actual analysis hash commitment bytes changed")
    return observed
...
__all__ = [
    ...
    "bind_formal_v023_actual_analysis_hash_ledger",
