"""V2.2 formal-attempt state machine and phase-scoped truth capabilities.

V2.2 inserts two outcome-free commitments into the inherited V2 lifecycle:
...
from lifetwin.experiments.calendar_long_horizon_v017_ledger import (
    ...
    read_exposure_log as read_v022_exposure_log,
...
class V022FirewallError(ValueError):
    """Raised when a V2.2 attempt crosses a frozen information boundary."""
...
def _sha256_file(path: str | Path) -> str:
    try:
        ...
        raise V022FirewallError(f"Cannot read committed artifact: {path}") from exc
...
def _is_reparse_entry(path: Path) -> bool:
    try:
        ...
        raise V022FirewallError(f"Cannot inspect formal path: {path}") from exc
...
def _physical_root(raw_path: str | Path, *, context: str) -> Path:
    ...
    for component in (*reversed(path.parents), path):
        if os.path.lexists(component) and _is_reparse_entry(component):
            raise V022FirewallError(f"{context} root has a reparse-point ancestor")
    if not path.is_dir() or path.resolve(strict=True) != path:
        raise V022FirewallError(
...
def _direct_physical_child(
    ...
    *,
    ...
    if candidate != expected:
        raise V022FirewallError(f"{filename} must be a direct label-free artifact")
    if not os.path.lexists(candidate):
        if must_exist:
            raise V022FirewallError(f"{context} is absent")
        return candidate
    if _is_reparse_entry(candidate) or not candidate.is_file():
        raise V022FirewallError(f"{context} is not a direct regular file")
    try:
        if candidate.resolve(strict=True) != candidate:
            raise V022FirewallError(f"{context} changed during resolution")
    except OSError as exc:
        raise V022FirewallError(f"Cannot bind {context}") from exc
    return candidate
    ...
    *,
    ...
    def reject_duplicates(pairs: list[tuple[str, Any]]) -> dict[str, Any]:
        ...
        for key, value in pairs:
            if key in decoded:
                raise V022FirewallError(f"{filename} contains duplicate key {key!r}")
            decoded[key] = value
        return decoded

    def reject_constant(token: str) -> None:
        raise V022FirewallError(f"{filename} contains nonfinite token {token}")
    ...
    try:
        ...
        raise V022FirewallError(f"{filename} is not strict JSON") from exc
    if not isinstance(payload, dict):
        raise V022FirewallError(f"{filename} must contain one JSON object")
    if compact:
        try:
            canonical = (
                json.dumps(
            ...
            raise V022FirewallError(f"{filename} is not finite ASCII JSON") from exc
    else:
        try:
            canonical = canonical_json_bytes(payload)
        except V015ArtifactError as exc:
            raise V022FirewallError(f"{filename} is not finite JSON") from exc
    if raw != canonical:
        raise V022FirewallError(f"{filename} is not canonical JSON")
    return payload


def _require_sha256(value: object, *, context: str) -> str:
    if not isinstance(value, str) or _SHA256.fullmatch(value) is None:
        raise V022FirewallError(f"{context} is not lowercase SHA256")
    return value
...
def _artifact_path(
    *,
    ...
    if not os.path.lexists(candidate):
        raise V022FirewallError(f"Committed input is absent: {filename}")
    if _is_reparse_entry(candidate) or not candidate.is_file():
        raise V022FirewallError(
...
def verify_formal_attempt_environment(identity: FormalAttemptIdentity) -> None:
    """Bind a truth-capability call to the frozen V2.2 implementation."""
    ...
    if (
        ...
        or environment.protocol_id != V022_PROTOCOL_ID
    ):
        raise V022FirewallError(
            "Current formal environment differs from the V2.2 attempt identity"
        )
...
def validate_formal_exposure_events(
    ...
    *,
    ...
    """Validate all interleaved V2.2 attempts in one publication ledger."""

    if contract.protocol_id != V022_PROTOCOL_ID:
        raise V022FirewallError("Firewall requires the V2.2 artifact contract")
    try:
        ...
        raise V022FirewallError(str(exc)) from exc
...
def validate_formal_exposure_log(
    ...
    """Read canonical JSONL and enforce the complete V2.2 state machine."""

    if contract.protocol_id != V022_PROTOCOL_ID:
        raise V022FirewallError("Firewall requires the V2.2 artifact contract")
    try:
        _, states, _ = read_v022_exposure_log(
            path,
            expected_config_sha256=contract.config_byte_sha256,
            sealed_filenames=contract.sealed_filenames,
        )
...
def append_formal_exposure_event(
    *,
    ...
    """Validate a prospective V2.2 transition before appending its bytes."""
...
def _require_direct_commitment(
    *,
    ...
    if path is None or expected_sha256 is None:
        raise V022FirewallError(f"{context} lacks its checkpointed commitment")
    ...
    if _sha256_file(resolved) != expected_sha256:
        raise V022FirewallError(f"{context} differs from its ledger commitment")
    return resolved
...
def _verify_generation_plan(
    *,
    ...
    try:
        from lifetwin.experiments.calendar_long_horizon_v017_collision import (
        ...
        if view.artifacts != contract:
            raise V022FirewallError(
                "Generation plan uses a different V2.2 artifact contract"
            )
        assert expected is not None
        verify_generation_plan_commitment(
            payload,
            expected_byte_sha256=expected,
            expected_current_protocol_id=V022_PROTOCOL_ID,
        ...
        recomputed = audit_formal_v022_generation_plan(view)
    except V022FirewallError:
        raise
    except (TypeError, ValueError) as exc:
        raise V022FirewallError("Generation-plan semantic verification failed") from exc
    ...
    *,
    ...
    try:
        ...
        verify_actual_analysis_hash_ledger_commitment(
            payload,
            expected_byte_sha256=expected,
            expected_protocol_id=V022_PROTOCOL_ID,
            expected_random_ranking_root=V022_EXPECTED_SEED_ROOTS["random_rankings"],
            expected_stress_permutation_root=V022_EXPECTED_SEED_ROOTS[
                "stress_permutations"
            ],
        )
    except (TypeError, ValueError) as exc:
...
def _verify_fit_commitment(
    *,
    ...
    if (
        ...
        raise V022FirewallError("fit_commitment.json identity changed")
    entries = payload.get("files")
    if not isinstance(entries, list) or len(entries) != len(_FIT_COMMITMENT_FILENAMES):
        raise V022FirewallError("fit_commitment.json file registry changed")
    for filename, entry in zip(
        ...
        if (
            ...
            raise V022FirewallError("A label-free fit input changed after commitment")
...
def _verify_input_hashes(
    ...
    *,
    ...
    if not isinstance(value, Mapping) or set(value) != set(expected_filenames):
        raise V022FirewallError(f"{context} input registry changed")
...
def _positive_int(value: object, *, context: str) -> int:
    if isinstance(value, bool) or not isinstance(value, int) or value < 1:
        raise V022FirewallError(f"{context} must be a positive integer")
    return value
...
def _verify_center_checkpoint(
    *,
    ...
    if (
        ...
        raise V022FirewallError("center_state_checkpoint.json is invalid")
    _require_sha256(
    ...
    if (
        ...
        raise V022FirewallError("center ridge_penalty is invalid")
...
def _verify_training_manifest(
    *,
    ...
    if _sha256_file(path) != risk_payload.get("training_manifest_byte_sha256"):
        raise V022FirewallError("Training manifest differs from the risk checkpoint")
    payload = _strict_json_object(
        path.read_bytes(),
        filename=path.name,
        compact=False,
    ...
    try:
        ...
        raise V022FirewallError(
            "V2.2 contract lacks the training-manifest schema"
        ) from exc
    if (
        ...
        raise V022FirewallError("training_manifest.json semantics changed")
...
def _verify_risk_checkpoint(
    *,
    ...
    if (
        ...
        raise V022FirewallError("risk_state_checkpoint.json is invalid")
    _require_sha256(
    ...
    if eligible > development or positive + negative != eligible:
        raise V022FirewallError("Risk checkpoint counts are inconsistent")
    _verify_input_hashes(
        payload.get("input_byte_hashes"),
...
def _verify_mask_commitment(
    *,
    ...
    try:
        ...
        deserialize_calibration_mask_commitment_json_v022(committed.read_bytes())
    except (TypeError, ValueError) as exc:
        raise V022FirewallError(
...
def open_truth_for_phase(
    *,
    ...
    """Open only truth authorized by one V2.2 reveal phase.
    ...
    if phase not in _OPEN_PHASE_FILES:
        raise V022FirewallError("This phase has no truth-access capability")
    if formal:
    ...
    if prior.identity != identity:
        raise V022FirewallError("Truth capability identity differs from the ledger")
    ...
    if _sha256_file(commitment) != truth_hash:
        raise V022FirewallError("Truth capability does not match its ledger commitment")
    try:
    ...
    try:
        ...
        for filename in _OPEN_PHASE_FILES[phase]:
            ...
            if path.parent != sealed_root:
                raise V022FirewallError("Sealed truth path escaped its root")
...
def reopen_authorized_truth_for_recovery(
    *,
    ...
    if (
        ...
        raise V022FirewallError("Recovery truth-file request is invalid")
    if formal:
        verify_formal_attempt_environment(identity)
    physical_label = _physical_root(label_free_root, context="Label-free")
    physical_sealed = _physical_root(sealed_truth_root, context="Sealed-truth")
    ...
    try:
        ...
        raise V022FirewallError("Recovery attempt is absent from the ledger") from exc
    ...
    if not set(requested).issubset(progress.opened_truth_files):
        raise V022FirewallError("Recovery requested truth that was not already exposed")
    ...
    for filename in requested:
        ...
        path = (sealed_root / filename).resolve()
        if path.parent != sealed_root:
            raise V022FirewallError("Recovery truth path escaped its root")
        frame = read_canonical_csv(path, contract, formal=formal)
        raw = path.read_bytes()
        if (
            len(frame) != int(entry["row_count"])
            ...
            raise V022FirewallError(f"{filename} differs from its recovery commitment")
        opened[filename] = frame
    return opened
...
def verify_phase_artifact_commitment(
    ...
    *,
    ...
    if expected is None:
        raise V022FirewallError(f"{phase} has no completed commitment")
    observed = _sha256_file(artifact_path)
    if observed != expected:
        raise V022FirewallError(f"{phase} artifact differs from its ledger commitment")
    return observed
