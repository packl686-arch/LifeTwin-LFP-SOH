"""Sealed, pure-memory provenance capabilities for the V2.2 formal path.

The V2.2 numerical state intentionally reuses frozen V2 dataclasses.  Their
Python type therefore cannot prove that the values were produced by a fresh
V2.2 attempt.  This module supplies that missing distinction: only package-
private issuers can wrap numeric state, and the wrapper binds the complete
V2.2 commitment chain plus hashes computed from exact input bytes.
...
class V022ProvenanceError(ValueError):
    """Raised when a value cannot prove fresh V2.2 provenance."""
...
_ISSUER_KEY = object()
_TRAINING_DOMAIN = b"lifetwin-v022-training-provenance-v1\0"
_MODEL_DOMAIN = b"lifetwin-v022-model-provenance-v1\0"
_COMMITTED_MODEL_DOMAIN = b"lifetwin-v022-committed-model-provenance-v1\0"
...
def _canonical_bytes(payload: object) -> bytes:
    try:
        ...
        raise V022ProvenanceError("Provenance payload is not canonical JSON") from exc
    return encoded + b"\n"
...
def _exact_raw(raw: object, *, context: str) -> bytes:
    if type(raw) is not bytes or not raw:
        raise V022ProvenanceError(f"{context} must be nonempty exact bytes")
    return raw
...
def _hash_exact_inputs(
    ...
    *,
    ...
    if not isinstance(values, Mapping) or not values:
        raise V022ProvenanceError(f"{context} must be a nonempty byte mapping")
    ...
    for name, raw in values.items():
        if (
            ...
            raise V022ProvenanceError(f"{context} contains an invalid input name")
        items.append(
            (
                name,
    ...
    if len({name for name, _ in canonical}) != len(canonical):
        raise V022ProvenanceError(f"{context} contains duplicate input names")
    return canonical
...
def _validate_preverified_input_hashes(
    ...
    *,
    ...
    if not isinstance(values, Mapping) or not values:
        raise V022ProvenanceError(f"{context} must be a nonempty hash mapping")
    items: list[tuple[str, str]] = []
    for name, digest in values.items():
        if (
            ...
            raise V022ProvenanceError(f"{context} contains an invalid entry")
        items.append((name, digest))
    return tuple(sorted(items))


def _audit_payload(audit: V022CalibrationAudit) -> dict[str, object]:
    return {
...
def _require_contract(contract_view: object) -> V022ContractView:
    if type(contract_view) is not V022ContractView:
        raise V022ProvenanceError(
    ...
    if (
        ...
        raise V022ProvenanceError("Validated V2.2 contract identity changed")
    return view
...
def _validate_numeric_state(
    ...
    *,
    ...
    try:
        ...
        raise V022ProvenanceError("Numeric training state is invalid") from exc
...
class V022TrainingProvenanceEnvelope:
    """Immutable capability proving the complete pre-model V2.2 chain."""
    ...
    def __init__(
        ...
        *,
        ...
        if (
            ...
            or type(self) is not V022TrainingProvenanceEnvelope
        ):
            raise TypeError(
                "V022TrainingProvenanceEnvelope is issued only by the V2.2 "
                "training provenance factory"
            )
        object.__setattr__(self, "_issuer_key", _issuer_key)
        object.__setattr__(self, "_protocol_id", V022_PROTOCOL_ID)
        object.__setattr__(
            self,
            "_config_sha256",
    ...
    def __setattr__(self, name: str, value: object) -> None:
        raise AttributeError("V2.2 provenance envelopes are immutable")
    ...
    @property
    def calibration_audit(self) -> V022CalibrationAudit:
        return self._audit

    @property
    def mask_commitment(self) -> V022PretruthMaskCommitment:
...
class V022ValidatedModelStateEnvelope:
    ...
    def __init__(
        ...
        *,
        _issuer_key: object,
        training_provenance: V022TrainingProvenanceEnvelope,
        ...
        if (
            _issuer_key is not _ISSUER_KEY
            or type(self) is not V022ValidatedModelStateEnvelope
        ):
            raise TypeError(
                "V022ValidatedModelStateEnvelope is issued only by the "
                "validated V2.2 model-state codec"
            )
        object.__setattr__(self, "_issuer_key", _issuer_key)
    ...
    def __setattr__(self, name: str, value: object) -> None:
        raise AttributeError("V2.2 model-state envelopes are immutable")
    ...
    @property
    def training_provenance(self) -> V022TrainingProvenanceEnvelope:
...
class V022CommittedModelStateEnvelope:
    ...
    def __init__(
        ...
        *,
        _issuer_key: object,
        validated_model_state: V022ValidatedModelStateEnvelope,
        ...
        if (
            _issuer_key is not _ISSUER_KEY
            or type(self) is not V022CommittedModelStateEnvelope
        ):
            raise TypeError(
                "V022CommittedModelStateEnvelope is issued only after exact "
    ...
    def __setattr__(self, name: str, value: object) -> None:
        raise AttributeError("V2.2 committed model envelopes are immutable")
    ...
    @property
    def validated_model_state(self) -> V022ValidatedModelStateEnvelope:
...
def _require_training_envelope(
    ...
    if (
        type(value) is not V022TrainingProvenanceEnvelope
        or value._issuer_key is not _ISSUER_KEY
    ):
        raise V022ProvenanceError(
            "An exact factory-issued V022TrainingProvenanceEnvelope is required"
        )
    ...
    if _training_digest(payload) != value.provenance_sha256:
        raise V022ProvenanceError("Training provenance envelope digest changed")
    return value


def _issue_v022_training_provenance_from_fresh_bytes(
    *,
    contract_view: V022ContractView,
    attempt_id: str,
    training_state: _v015.FrozenTrainingState,
    calibration_audit: V022CalibrationAudit,
    mask_commitment: V022PretruthMaskCommitment,
    ...
    if not isinstance(attempt_id, str) or _ATTEMPT_ID.fullmatch(attempt_id) is None:
        raise V022ProvenanceError("attempt_id is not a canonical V2.2 attempt ID")
    ...
    from lifetwin.experiments import (  # noqa: PLC0415
        calendar_long_horizon_v017_state as _state_codec,
    )

    try:
        validated_mask = _state_codec.validate_calibration_mask_commitment_v022(
            mask_commitment
        )
        ...
        raise V022ProvenanceError(
            "Calibration output cannot be bound to the V2.2 mask"
        ) from exc
    mask_raw = _state_codec.serialize_calibration_mask_commitment_json_v022(
        validated_mask
    )
    ...
    return V022TrainingProvenanceEnvelope(
...
def _rehydrate_v022_training_provenance_after_strict_io(
    *,
    contract_view: V022ContractView,
    attempt_id: str,
    training_state: _v015.FrozenTrainingState,
    ...
    if not isinstance(verified_input_byte_hashes, Mapping) or set(
        ...
        raise V022ProvenanceError("Verified input phase registry changed")
    ...
    try:
        ...
        raise V022ProvenanceError(
            "Calibration output cannot be rebound to the V2.2 mask"
        ) from exc
    mask_raw = _state_codec.serialize_calibration_mask_commitment_json_v022(
        validated_mask
    )
...
def _issue_validated_model_state_envelope_v022(
    *,
    training_provenance: V022TrainingProvenanceEnvelope,
    decoded: _v015.DecodedModelState,
    ...
    if type(decoded) is not _v015.DecodedModelState:
        raise V022ProvenanceError("Decoded model state has an unexpected type")
    ...
    return V022ValidatedModelStateEnvelope(
        _issuer_key=_ISSUER_KEY,
...
def _require_validated_model_state_envelope(
    ...
    if (
        type(value) is not V022ValidatedModelStateEnvelope
        or value._issuer_key is not _ISSUER_KEY
    ):
        raise V022ProvenanceError(
            "An exact codec-issued V022ValidatedModelStateEnvelope is required"
        )
    ...
    if _SHA256.fullmatch(model_hash) is None:
        raise V022ProvenanceError("Model-state commitment is invalid")
    hasher = hashlib.sha256()
    hasher.update(_MODEL_DOMAIN)
    hasher.update(bytes.fromhex(provenance.provenance_sha256))
    hasher.update(bytes.fromhex(model_hash))
    if hasher.hexdigest() != value.provenance_sha256:
        raise V022ProvenanceError("Model-state provenance envelope digest changed")
    ...
    if (
        ...
        raise V022ProvenanceError("Decoded model state left its provenance chain")
    return value
...
def _decode_model_commitment(raw: bytes) -> dict[str, Any]:
    ...
    try:
        payload = json.loads(
            ...
            parse_constant=lambda token: (_ for _ in ()).throw(
                V022ProvenanceError(f"Nonfinite model-state commitment value: {token}")
            ),
        )
    ...
    if not isinstance(payload, dict) or _v015.canonical_json_bytes(payload) != raw:
        raise V022ProvenanceError("model_state_commitment.json is not canonical JSON")
    return payload


def _issue_committed_model_state_envelope_v022(
    *,
    validated_model_state: V022ValidatedModelStateEnvelope,
    ...
    if set(payload) != {
        ...
        raise V022ProvenanceError("Model-state commitment schema changed")
    if (
        payload["protocol_id"] != validated.protocol_id
        or payload["config_sha256"] != validated.config_sha256
    ):
        raise V022ProvenanceError("Model-state commitment identity changed")
    files = payload["files"]
    if not isinstance(files, list) or not files:
        raise V022ProvenanceError("Model-state commitment files are missing")
    ...
    for name, raw in committed_state_artifact_bytes.items():
        if (
            ...
            raise V022ProvenanceError("Committed state artifact name is invalid")
    ...
    for index, raw_entry in enumerate(files):
        ...
        if (
            ...
            raise V022ProvenanceError("Model-state commitment file registry changed")
        ...
        if (
            ...
            raise V022ProvenanceError(f"Committed state artifact {name} changed")
    ...
    digest = hasher.hexdigest()
    return V022CommittedModelStateEnvelope(
        _issuer_key=_ISSUER_KEY,
...
def _require_committed_model_state_envelope(
    ...
    if (
        type(value) is not V022CommittedModelStateEnvelope
        or value._issuer_key is not _ISSUER_KEY
    ):
        raise V022ProvenanceError(
            "An exact IO-issued V022CommittedModelStateEnvelope is required"
        )
    ...
    if (
        ...
        raise V022ProvenanceError("Committed model-state phase binding changed")
    hasher = hashlib.sha256()
    hasher.update(_COMMITTED_MODEL_DOMAIN)
    hasher.update(bytes.fromhex(validated.provenance_sha256))
    hasher.update(bytes.fromhex(commitment_hash))
    if hasher.hexdigest() != value.provenance_sha256:
        raise V022ProvenanceError("Committed model-state envelope digest changed")
    return value


def _extract_label_free_state_for_formal_v022(
    value: object,
    *,
    config_sha256: str,
    ...
    if (
        ...
        raise V022ProvenanceError(
            "Model-state provenance does not match the active V2.2 contract"
        )
