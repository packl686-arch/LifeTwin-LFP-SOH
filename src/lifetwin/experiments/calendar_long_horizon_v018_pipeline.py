"""V2.3 identity adapter for the frozen V2 label-free numerical pipeline.

The inherited V2 implementation hard-codes its protocol identity.  This module
validates V2.3 inputs, translates identity only in memory, reuses the frozen
numerical core, and then rebuilds every identity-sensitive V2.3 output.  It has
...
from lifetwin.experiments.calendar_long_horizon_v018_provenance import (
    ...
    V023ProvenanceError,
    _extract_label_free_state_for_formal_v023,
)
...
class V023PipelineError(ValueError):
    """Raised when the V2.3 label-free adapter cannot prove its boundaries."""
...
def _canonical_input(
    ...
    *,
    ...
    try:
        ...
        raise V023PipelineError(str(exc)) from exc
...
def _translate_protocol_column(
    ...
    *,
    ...
    if "protocol_id" not in frame.columns:
        raise V023PipelineError("An identity-bearing table lacks protocol_id")
    if set(frame["protocol_id"].astype(str)) != {source}:
        raise V023PipelineError("A table contains an unexpected protocol identity")
    translated = frame.copy(deep=False)
    translated["protocol_id"] = destination
    return translated


def _tie_hash_v023(arm: str, content_hash: str) -> str:
    material = f"{V023_PROTOCOL_ID}|{arm}|{content_hash}".encode("ascii")
    return hashlib.sha256(material).hexdigest()


def rank_primary_arms_v023(
    *,
    ...
    """Apply the inherited ranking rule with the V2.3 tie-hash domain."""
    ...
    if (
        ...
        raise V023PipelineError("Primary ranking inputs have inconsistent lengths")
    if any(not isinstance(value, (bool, np.bool_)) for value in eligible_raw):
        raise V023PipelineError("hard_eligible must contain strict booleans")
    if isinstance(issue_count, bool) or not isinstance(issue_count, int):
        raise V023PipelineError("issue_count must be a strict integer")

    eligible = eligible_raw.astype(bool)
    indices = np.flatnonzero(eligible)
    if issue_count < 0 or issue_count > len(indices):
        raise V023PipelineError(
    ...
    if (
        ...
        raise V023PipelineError("An eligible primary risk score is nonfinite")

    ranking_a = rank_for_issuance(
        prefix_scores[indices],
        tuple(_tie_hash_v023("prefix_only", hashes_a[index]) for index in indices),
        issue_count,
    )
    ranking_b = rank_for_issuance(
        visible_scores[indices],
        tuple(_tie_hash_v023("visible_stress", hashes_b[index]) for index in indices),
        issue_count,
    )
...
def _suppress_ineligible_probabilities(
    ...
    try:
        ...
        raise V023PipelineError("Risk rows do not align with eligibility rows") from exc
    if eligibility.index.has_duplicates or eligibility.reindex(keys).isna().any():
        raise V023PipelineError("Risk rows do not align with eligibility rows")
    ...
    if not np.isfinite(probabilities).all() or np.any(
        ...
        raise V023PipelineError("An eligible primary calibrated probability is invalid")
    return result
    ...
    *,
    ...
    for partition, raw_features in feature_bundle.groupby("partition", sort=True):
        ...
        if set(risks.index.astype(str)) != set(cluster_ids) or set(
            ...
            raise V023PipelineError("Decision inputs contain different cluster sets")
        risks = risks.loc[cluster_ids]
        contents = contents.loc[cluster_ids]
        if partition in PRIMARY_ISSUE_COUNTS:
            ranking = rank_primary_arms_v023(
...
def _recompute_label_free_pipeline_with_state_v023(
    *,
    ...
    if contract.protocol_id != V023_PROTOCOL_ID:
        raise V023PipelineError("The artifact contract is not V2.3")
    supplied = (
        prefix_pack,
    ...
    try:
        ...
        raise V023PipelineError(
            "The inherited label-free numerical pipeline rejected V2.3 inputs"
        ) from exc

    prediction = _translate_protocol_column(
        inherited.prediction_bundle,
        source=V2_PROTOCOL_ID,
        destination=V023_PROTOCOL_ID,
    )
...
def recompute_label_free_pipeline_v023(
    *,
    prefix_pack: pd.DataFrame,
    forecast_coordinates: pd.DataFrame,
    operating_pack: pd.DataFrame,
    ...
    """Run V2.3 prediction only from a codec-validated model capability."""

    if contract.protocol_id != V023_PROTOCOL_ID:
        raise V023PipelineError("The artifact contract is not V2.3")
    try:
        state = _extract_label_free_state_for_formal_v023(
            model_state_envelope,
            config_sha256=contract.config_byte_sha256,
        )
    except V023ProvenanceError as exc:
        raise V023PipelineError(str(exc)) from exc
    return _recompute_label_free_pipeline_with_state_v023(
...
def recompute_validated_partition_with_state_v023(
    ...
    *,
    ...
    if type(state) is not FrozenLabelFreeState:
        raise V023PipelineError("Partition state has the wrong exact type")
    if contract.protocol_id != V023_PROTOCOL_ID:
        raise V023PipelineError("The artifact contract is not V2.3")
    ...
    try:
        ...
        raise V023PipelineError(
            "The inherited numerical core rejected a validated V2.3 partition"
        ) from exc
