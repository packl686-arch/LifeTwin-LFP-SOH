"""V2.3 identity boundary around the inherited prefix-only optimizer.

The optimizer's numerical declarations are frozen in V2 and contain no truth.
Only its table identity is hard-coded.  This adapter validates V2.3 tables,
maps identity in memory for the inherited computation, then validates and
returns V2.3 tables.  No V2-identified table is exposed to the caller.
"""
...
class V023FitError(RuntimeError):
    """Raised when the inherited fit cannot cross the V2.3 identity boundary."""
...
def _canonical(
    ...
    *,
    ...
    try:
        ...
        raise V023FitError(str(exc)) from exc
...
def _translate(
    ...
    *,
    ...
    if "protocol_id" not in frame.columns:
        raise V023FitError("A fit table lacks protocol_id")
    if set(frame["protocol_id"].astype(str)) != {source}:
        raise V023FitError("A fit table contains an unexpected protocol identity")
...
def _prepare_inputs(
    *,
    ...
    if contract.protocol_id != V023_PROTOCOL_ID:
        raise V023FitError("The artifact contract is not V2.3")
    prefix = _canonical(
        prefix_pack,
...
def _fit_structure_library_formal_from_verified_frames_v023(
    *,
    prefix_pack: pd.DataFrame,
    forecast_coordinates: pd.DataFrame,
    contract: FrozenArtifactContract,
    ...
    try:
        ...
        raise V023FitError("The inherited formal structure fit failed") from exc
