"""Committed-artifact scoring adapter for the frozen V2.4 protocol.
...
stochastic roots, and the V2.4 risk-calibration population.  Formal scoring
requires an IO-issued committed model capability and an envelope bound to
verified V2.4 prediction-commitment evidence; it never accepts a bare V2
decoded model state.
"""
...
from lifetwin.experiments.calendar_long_horizon_v019_analysis import (
    BOOTSTRAP_RESAMPLES,
...
from lifetwin.experiments.calendar_long_horizon_v019_io import (
    ...
    _require_prediction_commitment_evidence_v024,
)
...
class V024ScoringError(ValueError):
    """Raised when committed V2.4 scoring cannot prove its boundaries."""
...
_PREDICTION_ENVELOPE_DOMAIN = b"lifetwin-v024-scoring-prediction-envelope-v1\0"
...
@dataclass(frozen=True, slots=True)
class V024ScoringResult:
    """The ten-artifact V2.4 scoring registry in structured form."""
...
class V024PredictionCommitmentEnvelope:
    """Opaque scoring capability bound to verified V2.4 IO evidence."""
    ...
    def __init__(
        ...
        *,
        ...
        config_sha256: str,
        attempt_id: str,
        evidence: V024PredictionCommitmentEvidence,
        ...
        if (
            ...
            or type(self) is not V024PredictionCommitmentEnvelope
        ):
            raise TypeError(
                "V024PredictionCommitmentEnvelope is issued only from an exact "
    ...
    def __setattr__(self, name: str, value: object) -> None:
        raise AttributeError("V2.4 prediction commitment envelopes are immutable")
...
def _sha256(value: object, *, context: str) -> str:
    if not isinstance(value, str) or _SHA256.fullmatch(value) is None:
        raise V024ScoringError(f"{context} must be a lowercase SHA-256")
    return value


def _component(value: object, *, context: str) -> str:
    if not isinstance(value, str) or _COMPONENT.fullmatch(value) is None:
        raise V024ScoringError(f"{context} must be a canonical component")
    return value
...
def _issue_prediction_commitment_envelope_v024(
    *,
    prediction_frames: Mapping[str, pd.DataFrame],
    model_state_envelope: V024CommittedModelStateEnvelope,
    prediction_commitment_evidence: V024PredictionCommitmentEvidence,
    formal: bool,
    contract_view: V024ContractView | None = None,
) -> V024PredictionCommitmentEnvelope:
    ...
    try:
        evidence = _require_prediction_commitment_evidence_v024(
...
def _require_prediction_envelope(
    ...
    *,
    ...
    if (
        type(value) is not V024PredictionCommitmentEnvelope
    ...
    if (
        ...
        raise V024ScoringError("Prediction commitment provenance identity changed")
    try:
        evidence = _require_prediction_commitment_evidence_v024(
            envelope._evidence,
            require_ledger_committed=True,
    ...
    if (
        ...
        raise V024ScoringError("Prediction commitment IO evidence changed")
    _require_evidence_model_binding(evidence, model)
    commitment_hash = _sha256(
    ...
    if tuple(name for name, _, _, _ in metadata) != _PREDICTION_FRAME_FILENAMES:
        raise V024ScoringError("Prediction commitment artifact registry changed")
    ...
    if envelope.provenance_sha256 != expected:
        raise V024ScoringError("Prediction commitment envelope digest changed")
    return envelope
...
def _canonical_committed_equal(
    ...
    *,
    ...
    if canonical_csv_bytes(
        ...
        raise V024ScoringError(
            f"{filename} differs byte-for-byte from V2.4 recomputation"
        )


def validate_and_recompute_committed_predictions_v024(
    *,
    prediction_frames: Mapping[str, pd.DataFrame],
    model_state_envelope: V024CommittedModelStateEnvelope,
    ...
    recomputed = recompute_label_free_pipeline_v024(
...
def _policy_comparison_v024(
    ...
    *,
    ...
    if not math.isfinite(random_mean) or random_mean <= 0.0:
        raise V024ScoringError("Random-ranking mean risk is unavailable")
...
def _isotonic_calibration_diagnostics_v024(
...
def _bootstrap_summary_v024(
    ...
    *,
    ...
    if len(replicates) != expected_count:
        raise V024ScoringError("Bootstrap replicate count changed")
...
def canonicalize_score_frame_v024(
    frame: pd.DataFrame,
    filename: str,
    *,
...
def canonical_score_csv_bytes_v024(
    frame: pd.DataFrame,
    filename: str,
    *,
    allow_empty: bool = False,
    ...
    for filename, value in _artifacts_without_manifest(result).items():
        if filename.endswith(".csv"):
            if not isinstance(value, pd.DataFrame):
                raise V024ScoringError(f"{filename} must be a dataframe")
            payloads[filename] = canonical_score_csv_bytes_v024(
                value,
                filename,
                allow_empty=allow_empty,
            )
...
def _finalize_registry(
    ...
    *,
    model: V024CommittedModelStateEnvelope,
    prediction: V024PredictionCommitmentEnvelope,
    view: V024ContractView,
) -> V024ScoringResult:
    ...
    if set(manifest) != _RUN_MANIFEST_KEYS:
        raise V024ScoringError("run_manifest.json keys changed")
...
def required_score_artifact_payloads_v024(
    ...
    if (
        ...
        raise V024ScoringError("run_manifest.json identity or registry changed")
    payloads = _payloads_without_manifest(result)
    artifacts = _artifacts_without_manifest(result)
    expected_records = [
        {
...
def _terminal_result(
    ...
    *,
    ...
    if status_kind not in {"void", "inconclusive_not_success"}:
        raise V024ScoringError("Unknown terminal score status")
    unavailable = (
...
def _score_committed_artifacts_v024(
    *,
    prediction_frames: Mapping[str, pd.DataFrame],
    ...
    decoded, recomputed = validate_and_recompute_committed_predictions_v024(
    ...
    isotonic_diagnostics = _isotonic_calibration_diagnostics_v024(
        trajectories,
        prediction_frames["risk_bundle.csv"],
        decoded,
...
def score_committed_artifacts(
    *,
    ...
    try:
        return _score_committed_artifacts_v024(
            prediction_frames=prediction_frames,
...
score_committed_artifacts_v024 = score_committed_artifacts
required_score_artifact_payloads = required_score_artifact_payloads_v024
canonicalize_score_frame = canonicalize_score_frame_v024
canonical_score_csv_bytes = canonical_score_csv_bytes_v024
canonical_result_summary_bytes = canonical_result_summary_bytes_v024


def _run_stochastic_fixture_analyses_v024(
    trajectories: pd.DataFrame,
    *,
    issue_count: int,
    counts: _AnalysisCounts,
...
__all__ = [
    ...
    "canonical_score_csv_bytes_v024",
    "canonicalize_score_frame",
    "canonicalize_score_frame_v024",
    "required_score_artifact_payloads",
    "required_score_artifact_payloads_v024",
    "score_committed_artifacts",
    "score_committed_artifacts_v024",
    "validate_and_recompute_committed_predictions_v024",
]
