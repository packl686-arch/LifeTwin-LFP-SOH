"""Static, non-generative validator for the preregistered V2.3 amendment."""
...
V023_ONLY_ATTEMPT_ID = "v023-formal-20260810-a1"
V023_ALLOWED_DESIGN_STATUSES = (
    "preregistered_preimplementation",
    "implementation_frozen",
)
V023_SOURCE_CALIBRATION_COUNT = 900
V023_MINIMUM_RISK_ISOTONIC_ELIGIBLE_COUNT = 855
V023_MINIMUM_CALIBRATION_CLASS_COUNT = 60
V023_CONFORMAL_COUNT = 900
V023_CONFORMAL_ORDER_STATISTIC_INDEX = 811
...
V023_EXPECTED_SEED_ROOTS = MappingProxyType(
    {
...
class V023ProtocolError(ValueError):
    """Raised when the preregistered V2.3 bytes or invariants drift."""


@dataclass(frozen=True, slots=True)
class ValidatedV023Design:
    """Validated, non-executable V2.3 amendment identity."""
...
def _object(value: object, *, context: str) -> Mapping[str, Any]:
    if not isinstance(value, Mapping) or any(not isinstance(key, str) for key in value):
        raise V023ProtocolError(f"{context} must be a JSON object")
    return value


def _validate_payload(payload: object, *, path: Path, raw: bytes) -> ValidatedV023Design:
    top = _object(payload, context="V2.3 amendment")
    if set(top) != _EXPECTED_TOP_LEVEL_KEYS:
        raise V023ProtocolError("V2.3 top-level registry changed")
    if hashlib.sha256(raw).hexdigest() != V023_AMENDMENT_BYTE_SHA256:
        raise V023ProtocolError("V2.3 amendment byte commitment changed")
    ...
    if hashlib.sha256(canonical).hexdigest() != V023_AMENDMENT_SEMANTIC_SHA256:
        raise V023ProtocolError("V2.3 amendment semantic commitment changed")
    if top.get("schema_version") != "lifetwin_synthetic_long_horizon_v2_3/1.0.0":
        raise V023ProtocolError("V2.3 schema version changed")
    if top.get("protocol_id") != V023_PROTOCOL_ID:
        raise V023ProtocolError("V2.3 protocol identity changed")
    status = top.get("status")
    if status not in V023_ALLOWED_DESIGN_STATUSES:
        raise V023ProtocolError("V2.3 design status changed")
    if top.get("design_witnesses") != ["Jincheng Liu"]:
        raise V023ProtocolError("V2.3 design witness changed")
    ...
    if (
        ...
        raise V023ProtocolError("V2.3 execution authorization boundary changed")

    attempt = _object(top.get("attempt_registry"), context="attempt_registry")
    if (
        attempt.get("only_attempt_id") != V023_ONLY_ATTEMPT_ID
        ...
        raise V023ProtocolError("V2.3 one-shot attempt registry changed")
    ...
    if (
        ...
        or history.get("v022_reason_code")
        ...
        or history.get("v022_may_be_reclassified_repaired_resumed_or_rerun") is not False
    ):
        raise V023ProtocolError("Immutable predecessor history changed")
    ...
    if (
        ...
        raise V023ProtocolError("V2.3 result-free generation status changed")
    roots = _object(fresh.get("seed_roots"), context="fresh_generation.seed_roots")
    if dict(roots) != dict(V023_EXPECTED_SEED_ROOTS):
        raise V023ProtocolError("V2.3 seed roots changed")
    if set(roots.values()).intersection(
        (*V2_SEED_ROOTS, *V021_SEED_ROOTS, *V022_SEED_ROOTS)
    ):
        raise V023ProtocolError("V2.3 seed roots collide with a predecessor")
    ...
    if (
        ...
        raise V023ProtocolError("Whole-bundle formal contract changed")
    ...
    if (
        ...
        raise V023ProtocolError("V2.3 capability architecture changed")
    ...
    if (
        ...
        raise V023ProtocolError("V2.3 numeric output contract changed")
    ...
    if terminal.get("known_partition_error_may_use_unknown_default") is not False:
        raise V023ProtocolError("Known partition failures may not use unknown_default")
    if tuple(terminal.get("terminal_registry_exact_files", ())) != _EXPECTED_TERMINAL_FILES:
        raise V023ProtocolError("V2.3 terminal registry changed")
    if terminal.get("prediction_and_terminal_registries_mutually_exclusive") is not True:
        raise V023ProtocolError("V2.3 registry exclusivity changed")

    return ValidatedV023Design(
        protocol_id=V023_PROTOCOL_ID,
        status=str(status),
        config_path=path,
        config_byte_sha256=V023_AMENDMENT_BYTE_SHA256,
        seed_roots=MappingProxyType({str(k): int(v) for k, v in roots.items()}),
        raw=_deep_freeze(top),
    )


def load_v023_design(
    path: str | Path = DEFAULT_V023_AMENDMENT_PATH,
) -> ValidatedV023Design:
    """Load and validate V2.3 design bytes without consuming a seed."""
    ...
    try:
        ...
        raise V023ProtocolError("Cannot read V2.3 amendment JSON") from exc
