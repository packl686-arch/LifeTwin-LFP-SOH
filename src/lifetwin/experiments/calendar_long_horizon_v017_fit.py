"""V2.2 identity boundary around the inherited prefix-only optimizer.

The optimizer's numerical declarations are frozen in V2 and contain no truth.
Only its table identity is hard-coded.  This adapter validates V2.2 tables,
maps identity in memory for the inherited computation, then validates and
returns V2.2 tables.  No V2-identified table is exposed to the caller.
"""
...
class V022FitError(RuntimeError):
    """Raised when the inherited fit cannot cross the V2.2 identity boundary."""
...
def _canonical(
    ...
    *,
    ...
    try:
        ...
        raise V022FitError(str(exc)) from exc
...
def _translate(
    ...
    *,
    ...
    if "protocol_id" not in frame.columns:
        raise V022FitError("A fit table lacks protocol_id")
    if set(frame["protocol_id"].astype(str)) != {source}:
        raise V022FitError("A fit table contains an unexpected protocol identity")
...
def _prepare_inputs(
    *,
    ...
    if contract.protocol_id != V022_PROTOCOL_ID:
        raise V022FitError("The artifact contract is not V2.2")
    prefix = _canonical(
        prefix_pack,
...
def _fit_structure_library_formal_from_verified_frames_v022(
    *,
    prefix_pack: pd.DataFrame,
    forecast_coordinates: pd.DataFrame,
    contract: FrozenArtifactContract,
    ...
    try:
        ...
        raise V022FitError("The inherited formal structure fit failed") from exc
