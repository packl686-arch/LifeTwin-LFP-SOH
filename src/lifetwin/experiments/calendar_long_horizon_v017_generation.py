"""One-shot, post-freeze generation for the V2.2 synthetic protocol.

The public lifecycle has two explicit capabilities.  The first performs and
commits the complete RNG-free V2.2/V2 coordinate audit.  The second verifies
that exact commitment before it can consume the first V2.2 seed.  Neither
...
class V022GenerationError(ValueError):
    """Raised when the V2.2 generation lifecycle cannot advance exactly."""


@dataclass(frozen=True)
class WrittenV022GenerationArtifacts:
    """Metadata for one committed V2.2 generation bundle."""
...
def _is_reparse_entry(path: Path) -> bool:
    try:
        ...
        raise V022GenerationError(f"Cannot inspect formal path: {path}") from exc
...
def _physical_root_identity(
    ...
    *,
    ...
    try:
        ...
        raise V022GenerationError(f"Cannot bind {context} root identity") from exc
    if resolved != path:
        raise V022GenerationError(f"{context} root changed during resolution")
...
def _verify_physical_root_identity(
    ...
    *,
    ...
    if observed != identity:
        raise V022GenerationError(f"{context} directory identity changed")
...
def _require_exact_direct_files(
    ...
    *,
    ...
    if not root.is_dir() or _is_reparse_entry(root):
        raise V022GenerationError(f"{context} root is not a physical directory")
...
def _write_prepared_generation_artifacts_resumable(
    ...
    *,
    ...
    if (
        ...
        raise V022GenerationError("Prepared artifact membership or order changed")
...
def _decode_canonical_plan_commitment(raw: bytes) -> Mapping[str, object]:
    ...
    if not isinstance(decoded, Mapping):
        raise V022GenerationError("Generation-plan commitment must be an object")
    canonical = (
        json.dumps(
            decoded,
            ensure_ascii=True,
    ...
    if raw != canonical:
        raise V022GenerationError("Generation-plan commitment bytes are not canonical")
    return decoded
...
def _formal_environment() -> object:
    ...
    if (
        getattr(environment, "protocol_id", None) != V022_PROTOCOL_ID
        or getattr(environment, "git_dirty", None) is not False
    ):
        raise V022GenerationError("Formal V2.2 environment identity is invalid")
    return environment


def _validate_contract_view(view: V022ContractView) -> None:
    if type(view) is not V022ContractView:
        raise V022GenerationError("V2.2 contract view has an invalid type")
    if (
        ...
        raise V022GenerationError("V2.2 protocol and artifact contracts disagree")
    if view.design_status != _IMPLEMENTABLE_STATUS:
        raise V022GenerationError("V2.2 implementation has not been immutably frozen")
...
def _validate_attempt_identities(
    ...
    *,
    ...
    if not states:
        raise V022GenerationError("Formal exposure ledger contains no attempt")
...
def _select_generation_candidate(
    ...
    *,
    ...
    if len(candidates) != 1:
        raise V022GenerationError(
            "Exactly one plan-committed attempt may begin V2.2 generation"
        )
    if any(
        state.identity.attempt_id != candidates[0].identity.attempt_id
        and not state.terminal_failed
...
def commit_frozen_v022_generation_plan(
    *,
    label_free_root: str | Path,
) -> str:
    ...
    if getattr(environment, "config_byte_sha256", None) != (
        ...
        raise V022GenerationError(
            "Formal environment and V2.2 amendment commitments disagree"
        )
    ...
    except (OSError, V022FirewallError) as exc:
        raise V022GenerationError(
    ...
    append_formal_exposure_event(
        ...
        truth_commitments_byte_sha256=None,
        prediction_commitment_byte_sha256=None,
        message="Started the exhaustive RNG-free V2.2/V2 coordinate audit.",
    )
    try:
        commitment = audit_formal_v022_generation_plan(view)
...
def _verify_committed_generation_plan(
    *,
    ...
    except V022CollisionError as exc:
        raise V022GenerationError(
...
def generate_frozen_v022_artifacts(
    *,
    label_free_root: str | Path,
    sealed_truth_root: str | Path,
) -> WrittenV022GenerationArtifacts:
    """Generate fresh V2.2 rows only after the committed collision audit."""
    ...
    try:
        ...
        raise V022GenerationError(
            "Formal exposure ledger failed before V2.2 generation"
        ) from exc
    identity = _identity(progress)
    ...
    try:
        ...
        _append_failure(
            ...
            view=view,
            phase="truth_committed",
            message="V2.2 generation did not reach its truth commitment.",
        )
        raise
    ...
    append_formal_exposure_event(
        ...
        prediction_commitment_byte_sha256=None,
        message="All nine fresh V2.2 sealed files were committed before truth access.",
    )
    return WrittenV022GenerationArtifacts(
