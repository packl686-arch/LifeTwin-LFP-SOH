"""Fixed V2.9 profile over the shared V0.20 lifecycle implementation."""
...
_FORMAL_SCRIPT = _PROJECT_ROOT / "scripts" / "run_calendar_long_horizon_v024.py"


def run_isolated_generation_stage_v029(
    *,
    label_free_root: str | Path,
    sealed_truth_root: str | Path,
) -> None:
    """Run generation with the one authenticated V2.9 identity."""

    run_isolated_generation_stage(
        label_free_root=label_free_root,
        sealed_truth_root=sealed_truth_root,
        _contract_view=load_v029_contract_view(),
        _environment_verifier=verify_formal_environment_v029,
        _implementable_status=V029_DESIGN_STATUS,
    )


def run_isolated_prediction_process_v029(
    *,
    label_free_root: str | Path,
    attempt_id: str,
    repo_root: str | Path,
) -> V024PredictionWriteResult:
    """Run the truth-incapable prediction capsule under the V2.9 attester."""

    if attempt_id != V029_ONLY_ATTEMPT_ID:
        raise ValueError("Prediction attempt differs from the fixed V2.9 identity")
    return run_isolated_prediction_process_v024(
        label_free_root=label_free_root,
        attempt_id=attempt_id,
        repo_root=repo_root,
        _environment_verifier=verify_prediction_environment_v029,
        _input_filenames_by_stage=INPUT_FILENAMES_BY_STAGE,
    )


def run_formal_attempt_v029(
    *,
    attempt_id: str,
    label_free_root: str | Path,
    sealed_truth_root: str | Path,
    ...
    """Execute the sole V2.9 formal profile without a scientific override."""
    ...
    return run_formal_attempt(
        ...
        repo_root=repo_root,
        _contract_view=load_v029_contract_view(),
        _formal_attempt_id=V029_ONLY_ATTEMPT_ID,
...
__all__ = [
    "run_formal_attempt_v029",
    "run_isolated_generation_stage_v029",
    "run_isolated_prediction_process_v029",
]
