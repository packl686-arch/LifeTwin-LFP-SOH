"""Schema-aware numeric contracts for the V2.4 development line.
...
class V024NumericContractError(ValueError):
    """Raised when a V2.4 development output violates numeric semantics."""
...
def _require_columns(
    ...
    *,
    ...
    if missing:
        raise V024NumericContractError(f"{context} columns are missing: {missing}")
...
def _numeric(values: pd.Series, *, context: str) -> np.ndarray:
    ...
    if invalid.any():
        raise V024NumericContractError(f"{context} contains a non-numeric value")
    result = converted.to_numpy(dtype=float)
    if np.isinf(result).any():
        raise V024NumericContractError(f"{context} contains infinity")
    return result
...
def validate_risk_bundle_numeric_contract(
    ...
    if not isinstance(risk_bundle, pd.DataFrame) or not isinstance(
        ...
        raise V024NumericContractError("Risk numeric inputs must be dataframes")
    ...
    if risk_bundle.empty or feature_bundle.empty:
        raise V024NumericContractError("Risk numeric inputs cannot be empty")
    if feature_bundle.duplicated(list(_KEY_COLUMNS)).any():
        raise V024NumericContractError("Feature bundle contains duplicate cluster keys")
    ...
    if aligned.isna().any().any():
        raise V024NumericContractError("Risk rows do not align with feature rows")
    ...
    if not np.array_equal(risk_finite_flags, feature_finite_flags):
        raise V024NumericContractError(
    ...
    if (
        not np.isfinite(raw_scores[risk_finite_flags]).all()
        or not np.isnan(raw_scores[~risk_finite_flags]).all()
    ):
        raise V024NumericContractError(
    ...
    for column in (
        ...
        if not np.isfinite(values).all():
            raise V024NumericContractError(f"{column} must be finite")
...
def validate_feature_bundle_numeric_contract(feature_bundle: pd.DataFrame) -> None:
    ...
    if not isinstance(feature_bundle, pd.DataFrame) or feature_bundle.empty:
        raise V024NumericContractError("Feature bundle must be a nonempty dataframe")
    _require_columns(
        feature_bundle,
        _FEATURE_REQUIRED_COLUMNS,
        context="Feature bundle",
    ...
    if np.isinf(values).any():
        raise V024NumericContractError("Feature bundle contains infinity")
    ...
    if np.any(hard_eligible & ~declared_finite):
        raise V024NumericContractError("A hard-eligible feature row is nonfinite")
...
def validate_prediction_bundle_numeric_contract(
    ...
    if not isinstance(prediction_bundle, pd.DataFrame) or prediction_bundle.empty:
        raise V024NumericContractError("Prediction bundle must be nonempty")
    _require_columns(
        prediction_bundle,
        {*_KEY_COLUMNS, *_PREDICTION_NUMERIC_COLUMNS},
        context="Prediction bundle",
    )
    values = prediction_bundle.loc[:, _PREDICTION_NUMERIC_COLUMNS].to_numpy(dtype=float)
    if not np.isfinite(values).all():
        raise V024NumericContractError("Prediction bundle must be fully finite")
    ...
    if set(map(tuple, keys.to_numpy())) != set(map(tuple, feature_keys.to_numpy())):
        raise V024NumericContractError("Prediction and feature cluster key sets differ")
    for lower, upper in (
        ...
        if np.any(
            ...
            raise V024NumericContractError(f"{lower} exceeds {upper}")
...
def validate_decision_bundle_numeric_contract(
    ...
    *,
    ...
    if not isinstance(decision_bundle, pd.DataFrame) or decision_bundle.empty:
        raise V024NumericContractError("Decision bundle must be nonempty")
    _require_columns(
        decision_bundle,
    ...
    decision_keys = pd.MultiIndex.from_frame(decision_bundle.loc[:, list(_KEY_COLUMNS)])
    aligned = feature_state.reindex(decision_keys)
    if aligned.isna().any().any():
        raise V024NumericContractError("Decision rows do not align with feature rows")
    hard_eligible = decision_bundle["hard_eligible"].to_numpy(dtype=bool)
    if not np.array_equal(
        hard_eligible,
        aligned["hard_eligible"].to_numpy(dtype=bool),
    ):
        raise V024NumericContractError("Decision hard_eligible state differs")
    ...
    if (
        len(merged) != len(decision_bundle)
        or merged["risk_raw"].isna().ne(merged["raw_risk_score"].isna()).any()
    ):
        raise V024NumericContractError("Decision and risk raw-score masks differ")
    finite = ~merged["risk_raw"].isna()
    if not np.array_equal(
        merged.loc[finite, "risk_raw"].to_numpy(float),
        merged.loc[finite, "raw_risk_score"].to_numpy(float),
    ):
        raise V024NumericContractError("Decision and risk raw scores differ")
    ...
    if np.any(issued & ~hard_eligible):
        raise V024NumericContractError("An ineligible decision was issued")
    for (partition, arm), group in decision_bundle.assign(
        ...
        if not np.isnan(group_ranks[~group_eligible]).all():
            raise V024NumericContractError("An ineligible decision has a rank")
        ...
        if not np.array_equal(np.sort(eligible_ranks), expected):
            raise V024NumericContractError("Eligible issuance ranks are not exact")
        ...
        if not np.array_equal(group["issued"].to_numpy(dtype=bool), expected_issued):
            raise V024NumericContractError("Issued flags do not match frozen ranks")
