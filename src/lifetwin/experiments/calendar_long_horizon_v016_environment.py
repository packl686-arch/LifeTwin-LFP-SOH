"""Fail-closed implementation and environment guard for the V2.1 attempt."""
...
class V021EnvironmentError(RuntimeError):
    """Raised when a formal process is not the attested V2.1 implementation."""
...
def _git(repo_root: Path, *arguments: str) -> str:
    try:
        ...
        raise V021EnvironmentError(f"Cannot verify git {' '.join(arguments)}") from exc
...
def _source_hashes(repo_root: Path) -> Mapping[str, str]:
    ...
    if any(not path.is_file() for path in paths):
        raise V021EnvironmentError("A V2.1 implementation source is not a file")
...
def _strict_json_object(path: Path, *, context: str) -> dict[str, object]:
    try:
        ...
        raise V021EnvironmentError(f"{context} is invalid JSON") from exc
    if not isinstance(decoded, dict):
        raise V021EnvironmentError(f"{context} must be a JSON object")
    return decoded
...
def _reject_duplicate_keys(
    ...
    for key, value in pairs:
        if key in result:
            raise V021EnvironmentError(f"Duplicate JSON key: {key}")
        result[key] = value
    return result


def _reject_json_constant(token: str) -> None:
    raise V021EnvironmentError(f"Nonfinite JSON constant: {token}")
...
def verify_formal_environment(
    ...
    if not (root / ".git").exists():
        raise V021EnvironmentError("Formal repository root has no .git directory")
    top_level = Path(_git(root, "rev-parse", "--show-toplevel")).resolve()
    if top_level != root:
        raise V021EnvironmentError("Formal repository root is not the git top level")
    git_commit = _git(root, "rev-parse", "HEAD")
    if _FULL_GIT_HASH.fullmatch(git_commit) is None:
        raise V021EnvironmentError("Git did not return a full commit hash")
    if _git(root, "status", "--porcelain=v1", "--untracked-files=all"):
        raise V021EnvironmentError("Formal implementation worktree is dirty")
    ...
    for path in (amendment_path, prereg_path, freeze_record_path, lock_path):
        if not path.is_file():
            raise V021EnvironmentError(f"Required freeze file is absent: {path}")
    try:
        design = load_v021_design(amendment_path)
    except (OSError, ValueError) as exc:
        raise V021EnvironmentError("V2.1 amendment validation failed") from exc
    ...
    if _sha256_path(prereg_path) != _PREREG_BYTE_SHA256:
        raise V021EnvironmentError("V2.1 preregistration hash changed")
    if _sha256_path(lock_path) != _LOCK_BYTE_SHA256:
        raise V021EnvironmentError("V2.1 environment lock hash changed")
    ...
    if sys.version_info[:3] != _EXPECTED_PYTHON:
        raise V021EnvironmentError("Formal Python runtime differs from CPython 3.12.13")
    ...
    return FormalEnvironmentIdentity(
        ...
        config_canonical_sha256=V021_AMENDMENT_SEMANTIC_SHA256,
        preregistration_byte_sha256=_PREREG_BYTE_SHA256,
        environment_lock_byte_sha256=_LOCK_BYTE_SHA256,
