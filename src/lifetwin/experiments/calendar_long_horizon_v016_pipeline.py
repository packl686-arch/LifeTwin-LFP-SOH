"""V2.1 identity adapter for the frozen V2 label-free numerical pipeline.

The inherited V2 implementation hard-codes its protocol identity.  This module
validates V2.1 inputs, translates identity only in memory, reuses the frozen
numerical core, and then rebuilds every identity-sensitive V2.1 output.  It has
...
from lifetwin.experiments.calendar_long_horizon_v016_provenance import (
    ...
    V021ProvenanceError,
    _extract_label_free_state_for_formal_v021,
)
...
class V021PipelineError(ValueError):
    """Raised when the V2.1 label-free adapter cannot prove its boundaries."""
...
def _canonical_input(
    ...
    *,
    ...
    try:
        ...
        raise V021PipelineError(str(exc)) from exc
...
def _translate_protocol_column(
    ...
    *,
    ...
    if "protocol_id" not in frame.columns:
        raise V021PipelineError("An identity-bearing table lacks protocol_id")
    if set(frame["protocol_id"].astype(str)) != {source}:
        raise V021PipelineError("A table contains an unexpected protocol identity")
    translated = frame.copy(deep=False)
    translated["protocol_id"] = destination
    return translated


def _tie_hash_v021(arm: str, content_hash: str) -> str:
    material = f"{V021_PROTOCOL_ID}|{arm}|{content_hash}".encode("ascii")
    return hashlib.sha256(material).hexdigest()


def rank_primary_arms_v021(
    *,
    ...
    """Apply the inherited ranking rule with the V2.1 tie-hash domain."""
    ...
    if (
        ...
        raise V021PipelineError("Primary ranking inputs have inconsistent lengths")
    if any(not isinstance(value, (bool, np.bool_)) for value in eligible_raw):
        raise V021PipelineError("hard_eligible must contain strict booleans")
    if isinstance(issue_count, bool) or not isinstance(issue_count, int):
        raise V021PipelineError("issue_count must be a strict integer")

    eligible = eligible_raw.astype(bool)
    indices = np.flatnonzero(eligible)
    if issue_count < 0 or issue_count > len(indices):
        raise V021PipelineError(
    ...
    if (
        ...
        raise V021PipelineError("An eligible primary risk score is nonfinite")

    ranking_a = rank_for_issuance(
        prefix_scores[indices],
        tuple(_tie_hash_v021("prefix_only", hashes_a[index]) for index in indices),
        issue_count,
    )
    ranking_b = rank_for_issuance(
        visible_scores[indices],
        tuple(_tie_hash_v021("visible_stress", hashes_b[index]) for index in indices),
        issue_count,
    )
...
def _suppress_ineligible_probabilities(
    ...
    try:
        ...
        raise V021PipelineError("Risk rows do not align with eligibility rows") from exc
    if eligibility.index.has_duplicates or eligibility.reindex(keys).isna().any():
        raise V021PipelineError("Risk rows do not align with eligibility rows")
    ...
    if not np.isfinite(probabilities).all() or np.any(
        ...
        raise V021PipelineError("An eligible primary calibrated probability is invalid")
    return result
    ...
    *,
    ...
    for partition, raw_features in feature_bundle.groupby("partition", sort=True):
        ...
        if set(risks.index.astype(str)) != set(cluster_ids) or set(
            ...
            raise V021PipelineError("Decision inputs contain different cluster sets")
        risks = risks.loc[cluster_ids]
        contents = contents.loc[cluster_ids]
        if partition in PRIMARY_ISSUE_COUNTS:
            ranking = rank_primary_arms_v021(
...
def _recompute_label_free_pipeline_with_state_v021(
    *,
    ...
    if contract.protocol_id != V021_PROTOCOL_ID:
        raise V021PipelineError("The artifact contract is not V2.1")
    supplied = (
        prefix_pack,
    ...
    try:
        ...
        raise V021PipelineError(
            "The inherited label-free numerical pipeline rejected V2.1 inputs"
        ) from exc
...
def recompute_label_free_pipeline_v021(
    *,
    prefix_pack: pd.DataFrame,
    forecast_coordinates: pd.DataFrame,
    operating_pack: pd.DataFrame,
    ...
    """Run V2.1 prediction only from a codec-validated model capability."""

    if contract.protocol_id != V021_PROTOCOL_ID:
        raise V021PipelineError("The artifact contract is not V2.1")
    try:
        state = _extract_label_free_state_for_formal_v021(
...
def _recompute_label_free_pipeline_hand_fixture_v021(
    *,
    prefix_pack: pd.DataFrame,
    forecast_coordinates: pd.DataFrame,
    operating_pack: pd.DataFrame,
    ...
    """Private nonformal channel for small numerical hand fixtures only."""

    if type(state) is not FrozenLabelFreeState:
        raise V021PipelineError("Hand-fixture state must have the exact frozen type")
    return _recompute_label_free_pipeline_with_state_v021(
        prefix_pack=prefix_pack,
        forecast_coordinates=forecast_coordinates,
        operating_pack=operating_pack,
