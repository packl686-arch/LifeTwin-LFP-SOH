"""Capability-minimal V2.1 prediction runtime.
...
class V021PredictionCapsuleError(RuntimeError):
...
def _digest(value: object, *, context: str) -> str:
    if not isinstance(value, str) or _SHA256.fullmatch(value) is None:
        raise V021PredictionCapsuleError(f"{context} must be lowercase SHA256")
    return value


def _utc(value: object, *, context: str) -> str:
    if not isinstance(value, str) or _UTC.fullmatch(value) is None:
        raise V021PredictionCapsuleError(f"{context} must be an RFC3339 UTC timestamp")
    try:
        parsed = datetime.fromisoformat(value[:-1] + "+00:00")
    except ValueError as exc:
        raise V021PredictionCapsuleError(f"{context} is invalid") from exc
    if parsed.utcoffset() is None:
        raise V021PredictionCapsuleError(f"{context} lacks a timezone")
    return value
...
def _strict_object(pairs: list[tuple[str, object]]) -> dict[str, object]:
    ...
    for key, value in pairs:
        if key in result:
            raise V021PredictionCapsuleError(f"Duplicate JSON key: {key}")
        result[key] = value
    return result


def _reject_constant(token: str) -> None:
    raise V021PredictionCapsuleError(f"Nonfinite JSON constant is forbidden: {token}")
...
def _json_ready(value: Any) -> Any:
    ...
    if isinstance(value, float) and not math.isfinite(value):
        raise V021PredictionCapsuleError(
...
def canonical_json_bytes(payload: Mapping[str, Any]) -> bytes:
    try:
        ...
        raise V021PredictionCapsuleError("Payload is not finite JSON") from exc
    return (text + "\n").encode("utf-8")
...
def _strict_json(
    ...
    *,
    ...
    if type(raw) is not bytes:
        raise V021PredictionCapsuleError(f"{filename} must be exact bytes")
    try:
        payload = json.loads(
            raw.decode("utf-8"),
    ...
    if not isinstance(payload, dict):
        raise V021PredictionCapsuleError(f"{filename} must be a JSON object")
    if compact:
        canonical = (
            json.dumps(
                payload,
    ...
    if canonical != raw:
        raise V021PredictionCapsuleError(f"{filename} is not canonical JSON")
    return payload
...
def _exact_mapping(
    ...
    *,
    ...
    if not isinstance(value, Mapping) or set(value) != expected:
        raise V021PredictionCapsuleError(f"{context} keys changed")
    return value


def _real(value: object, *, context: str) -> float:
    if type(value) is not float:
        raise V021PredictionCapsuleError(f"{context} must be a canonical JSON real")
    result = value
    if not math.isfinite(result):
        raise V021PredictionCapsuleError(f"{context} must be finite")
    return result


def _finite_number(value: object, *, context: str) -> float:
    if isinstance(value, (bool, np.bool_)):
        raise V021PredictionCapsuleError(f"{context} must be numeric")
    try:
        result = float(value)
    except (TypeError, ValueError) as exc:
        raise V021PredictionCapsuleError(f"{context} must be numeric") from exc
    if not math.isfinite(result):
        raise V021PredictionCapsuleError(f"{context} must be finite")
    return result
...
def _integer(value: object, *, context: str) -> int:
    if type(value) is not int:
        raise V021PredictionCapsuleError(f"{context} must be an integer")
    return value


def _string(value: object, *, context: str) -> str:
    if not isinstance(value, str) or not value:
        raise V021PredictionCapsuleError(f"{context} must be a nonempty string")
    return value
...
def _real_list(
    ...
    *,
    ...
    if not isinstance(value, list):
        raise V021PredictionCapsuleError(f"{context} must be an array")
    result = tuple(
        _real(item, context=f"{context}[{index}]") for index, item in enumerate(value)
    )
    if length is not None and len(result) != length:
        raise V021PredictionCapsuleError(f"{context} length changed")
    return result
...
def _bool_list(
    ...
    *,
    ...
    if (
        ...
        raise V021PredictionCapsuleError(f"{context} must be a boolean array")
    return tuple(value)
...
def _parse_logistic(
    ...
    *,
    ...
    if not isinstance(names, list) or tuple(names) != expected_names:
        raise V021PredictionCapsuleError(f"{context}.feature_names changed")
    ...
    for index, is_zero in enumerate(zero):
        if is_zero and (scale[index] != 1.0 or abs(coefficients[index]) > 1e-12):
            raise V021PredictionCapsuleError(f"{context} zero-variance state changed")
...
def _parse_isotonic(value: object, *, context: str) -> IsotonicState:
    ...
    if (
        ...
        raise V021PredictionCapsuleError(f"{context} is not an isotonic map")
...
def _validate_prediction_state(state: PredictionState) -> None:
    if not math.isfinite(state.center_beta) or not 0.0 <= state.center_beta <= 1.0:
        raise V021PredictionCapsuleError("center_beta is invalid")
    expected = (
    ...
    if any(risk.feature_names != names for risk, names in expected):
        raise V021PredictionCapsuleError("Prediction feature order changed")
    if (
        state.strongest_single_feature_name not in PREFIX_FEATURE_NAMES
        or state.strongest_single_feature_orientation not in {-1, 1}
    ):
        raise V021PredictionCapsuleError("Strongest single-feature state is invalid")
...
def decode_prediction_state(
    ...
    *,
    ...
    if (
        top.get("protocol_id") != V021_PROTOCOL_ID
        or top.get("config_sha256") != config_sha256
    ):
        raise V021PredictionCapsuleError("model_state.json identity changed")
    ...
    if dict(feature_orders) != expected_orders:
        raise V021PredictionCapsuleError("feature_orders changed")
    ...
    if (
        ...
        raise V021PredictionCapsuleError("center_state changed")
    ...
    if (
        ...
        raise V021PredictionCapsuleError("risk development state changed")
    ...
    if (
        ...
        raise V021PredictionCapsuleError("strongest single-feature state changed")
    ...
    if (
        ...
        raise V021PredictionCapsuleError("calibration counts changed")
    ...
    if not isinstance(baseline_value, Mapping) or set(baseline_value) != set(
        ...
        raise V021PredictionCapsuleError("mean baseline registry changed")
    baseline_iae = {
        model_id: _real(
    ...
    if any(value < 0.0 for value in baseline_iae.values()):
        raise V021PredictionCapsuleError("mean baseline IAE is negative")
    selected = _string(
    ...
    if selected != min(
        ...
        raise V021PredictionCapsuleError("selected mean baseline changed")
    ...
    if not isinstance(raw_hashes, Mapping) or set(raw_hashes) != _MODEL_STATE_PHASES:
        raise V021PredictionCapsuleError("model input phase registry changed")
    ...
    for phase in _MODEL_STATE_PHASES:
        ...
        if len(input_hashes[phase]) != len(phase_hashes):
            raise V021PredictionCapsuleError(f"{phase} input filename is invalid")
    if (
        not isinstance(top["software_versions"], Mapping)
        or dict(top["software_versions"]) != _EXPECTED_SOFTWARE_VERSIONS
    ):
        raise V021PredictionCapsuleError("software_versions changed")
...
def _decode_mask_commitment(raw: bytes) -> _MaskEvidence:
    ...
    if set(payload) != _MASK_TOP_LEVEL_KEYS:
        raise V021PredictionCapsuleError("Calibration mask commitment schema changed")
    if (
        payload["schema_version"] != "1.0.0"
        or payload["protocol_id"] != V021_PROTOCOL_ID
    ):
        raise V021PredictionCapsuleError("Calibration mask commitment identity changed")
    ...
    for index, value in enumerate(rows):
        ...
        if (
            ...
            raise V021PredictionCapsuleError(f"mask rows[{index}] reasons are invalid")
    ...
    if eligible_count < 855 or declared_eligible != eligible_count:
        raise V021PredictionCapsuleError("Calibration mask eligible count is invalid")
...
def _model_substate_hashes(
    ...
    if not isinstance(calibration, Mapping):
        raise V021PredictionCapsuleError("calibration_state must be an object")
    isotonic_payload = {
...
def _verify_training_chain_semantics(
    *,
    ...
    if (
        ...
        raise V021PredictionCapsuleError("Model substates are not objects")
    ...
    if (
        ...
        raise V021PredictionCapsuleError("Center checkpoint semantics changed")
    if (
        ...
        raise V021PredictionCapsuleError("Risk checkpoint semantics changed")
    if (
        ...
        raise V021PredictionCapsuleError("Training manifest semantics changed")
    ...
    if (
        ...
        raise V021PredictionCapsuleError("Calibration manifest semantics changed")
...
def _direct_file(root: Path, filename: str) -> Path:
    if Path(filename).name != filename or filename in {"", ".", ".."}:
        raise V021PredictionCapsuleError(f"Unsafe label-free filename: {filename!r}")
    path = root / filename
    if not path.exists() or _is_reparse(path) or not path.is_file():
        raise V021PredictionCapsuleError(f"{filename} is not a direct physical file")
    return path
...
def _require_membership(
    ...
    *,
    ...
    try:
        ...
        raise V021PredictionCapsuleError(f"Cannot inspect {context} root") from exc
...
def _require_group_grid(
    ...
    *,
    ...
    if (sizes != len(expected)).any():
        raise V021PredictionCapsuleError(f"{context} group membership is incomplete")
...
def canonicalize_frame(
    ...
    *,
    ...
    if type(frame) is not pd.DataFrame:
        raise V021PredictionCapsuleError(f"{filename} must be an exact dataframe")
    if tuple(frame.columns) != schema.columns:
        raise V021PredictionCapsuleError(
            f"{filename} columns differ from the frozen allowlist"
        )
    if frame.empty:
        raise V021PredictionCapsuleError(f"{filename} cannot be empty")
    ...
    if frame.duplicated(list(schema.key)).any():
        raise V021PredictionCapsuleError(f"{filename} contains duplicate key rows")
...
def _identity_json(
    ...
    *,
    ...
    if (
        ...
        raise V021PredictionCapsuleError(f"{filename} identity changed")
...
def _load_prediction_progress(
    ...
    *,
    ...
    if not isinstance(attempt_id, str) or _ATTEMPT_ID.fullmatch(attempt_id) is None:
        raise V021PredictionCapsuleError("attempt_id is invalid")
    if (
        not isinstance(expected_git_commit, str)
        or _GIT_COMMIT.fullmatch(expected_git_commit) is None
    ):
        raise V021PredictionCapsuleError("Expected git commit is invalid")
    ...
    try:
        ...
        raise V021PredictionCapsuleError(str(exc)) from exc
    try:
...
def _verify_truth_commitment(
    ...
    *,
    ...
    if set(payload) != {
        ...
        raise V021PredictionCapsuleError("truth_commitments.json schema changed")
    _identity_json(
        payload,
    ...
    if not isinstance(entries, list) or len(entries) != len(_SEALED_FILENAMES):
        raise V021PredictionCapsuleError("Truth commitment file registry changed")
    result: dict[str, str] = {}
...
def _verify_file_commitment(
    ...
    *,
    ...
    if (
        ...
        raise V021PredictionCapsuleError(f"{filename} semantics changed")
    if require_worker_count and payload.get("worker_count") != 6:
        raise V021PredictionCapsuleError("fit_commitment.json worker count changed")
    _identity_json(
        payload,
        filename=filename,
        config_sha256=config_sha256,
    ...
    if len(entries) != len(expected_files):
        raise V021PredictionCapsuleError(f"{filename} file registry changed")
...
def _require_bundle_unchanged(
    ...
    *,
    ...
    if (
        ...
        raise V021PredictionCapsuleError("Prediction bundle capability is invalid")
    ...
    if set(file_hashes) != set(_PRE_PREDICTION_FILES):
        raise V021PredictionCapsuleError("Committed file-hash registry changed")
    for filename, digest in file_hashes.items():
        if _sha256(_direct_file(value._root, filename).read_bytes()) != digest:
            raise V021PredictionCapsuleError("A committed label-free artifact changed")
    if (
        _direct_file(
            value._root,
            "exposure_log.jsonl",
        ...
        raise V021PredictionCapsuleError("Committed prediction ledger changed")
    decoded = decode_prediction_state(
    ...
    if decoded.state != value._state:
        raise V021PredictionCapsuleError("Sealed in-memory prediction state changed")
    frames = tuple(value._frames)
    if tuple(name for name, _ in frames) != (
        *_LABEL_INPUTS,
        *_FIT_OUTPUTS,
    ):
        raise V021PredictionCapsuleError("Sealed in-memory frame registry changed")
    for filename, frame in frames:
        if type(frame) is not pd.DataFrame:
            raise V021PredictionCapsuleError(f"Sealed frame type changed: {filename}")
...
def _strict_bool(value: object, *, context: str) -> bool:
    if not isinstance(value, (bool, np.bool_)):
        raise V021PredictionCapsuleError(f"{context} must be a strict boolean")
    return bool(value)
...
def _require_exact_grid(
    ...
    *,
    ...
    if (
        ...
        raise V021PredictionCapsuleError(f"{context} does not contain the frozen grid")
...
def _parse_parameter_payload(value: object) -> tuple[dict[str, float], str]:
    ...
    try:
        ...
        raise V021PredictionCapsuleError("parameters_json is invalid JSON") from exc
    if not isinstance(payload, dict):
        raise V021PredictionCapsuleError("parameters_json must encode a JSON object")
    ...
    try:
        ...
        raise V021PredictionCapsuleError("parameters_json is not finite") from exc
    if value != canonical:
        raise V021PredictionCapsuleError(
...
def _evaluate_frozen_variant(
    ...
    raise V021PredictionCapsuleError(f"Unknown candidate structure: {model_id}")
...
def _recomputed_credible(
    ...
    *,
    ...
    if status not in {"succeeded", "failed"}:
        raise V021PredictionCapsuleError("fit_status must be succeeded or failed")
...
def _unique_safe_variants(
    ...
    for variant in variants:
        ...
        if (
            ...
            raise V021PredictionCapsuleError("Credible variant metadata is invalid")
...
def _extract_prefix_features_safe(
    *,
    ...
    if (
        ...
        raise V021PredictionCapsuleError("Prefix feature inputs are invalid")
    ...
    if successful_count == 0 or successful_count > len(DECLARED_STRUCTURE_FAMILIES):
        raise V021PredictionCapsuleError("Successful structure-family count is invalid")
    family_vectors = {
    ...
    if not np.isfinite(values).all():
        raise V021PredictionCapsuleError("Extracted prefix features must all be finite")
    return values
...
def predictor_content_hashes(
    ...
    if set((*REAL_OPERATING_FIELDS, *PLACEBO_FIELDS)).difference(operating):
        raise V021PredictionCapsuleError("Predictor operating content is incomplete")
    base = {
...
def _select_baseline(
    ...
    if not successful:
        raise V021PredictionCapsuleError(f"{model_id} has no finite succeeded forecast")
    first = successful[0]
...
def _stress_index(
    ...
    if not np.isfinite(values).all():
        raise V021PredictionCapsuleError("Stress-index inputs must be finite")
    return float(
...
def _tie_hash(arm: str, content_hash: str) -> str:
    material = f"{V021_PROTOCOL_ID}|{arm}|{content_hash}".encode("ascii")
...
def _rank_primary_arms(
    *,
    ...
    if (
        ...
        raise V021PredictionCapsuleError("Primary ranking inputs are invalid")
    if any(not isinstance(value, (bool, np.bool_)) for value in eligible_raw):
        raise V021PredictionCapsuleError("hard_eligible must contain strict booleans")
    ...
    if (
        ...
        raise V021PredictionCapsuleError("An eligible primary risk score is nonfinite")
...
def _validate_cluster_alignment(
    ...
    if not expected:
        raise V021PredictionCapsuleError("Predictor inputs contain no clusters")
    ...
    for frame in frames[1:]:
        if _cluster_keys(frame) != expected:
            raise V021PredictionCapsuleError("Predictor input cluster sets differ")
...
def _validate_prediction_input_alignment(
    ...
    if set(frames) != set(expected_names):
        raise V021PredictionCapsuleError("Prediction input file registry changed")
...
def recompute_prediction_pipeline(
    *,
    ...
    """Recompute the V2.1 label-free outputs in the minimal capsule."""
    ...
    *,
    ...
    for partition, cluster_id in sorted(_cluster_keys(prefix)):
        ...
        if not np.isfinite(observed).all():
            raise V021PredictionCapsuleError("Prefix observations must be finite")
        ...
        for key in sorted(FROZEN_VARIANT_KEY_SET):
            ...
            if len(diagnostic_matches) != 1:
                raise V021PredictionCapsuleError("Variant diagnostics are not unique")
