"""V2.1 stochastic analysis adapter.
...
move to the V2.1 protocol and seed-root namespace.  It deliberately does not
...
RANDOM_ROOT = 202607260210
BOOTSTRAP_ROOT = 202607260211
STRESS_PERMUTATION_ROOT = 202607260212
...
if any(
    V021_EXPECTED_SEED_ROOTS.get(name) != root
    for name, root in _DECLARED_STOCHASTIC_ROOTS.items()
):
    raise RuntimeError("V2.1 stochastic roots disagree with the protocol amendment")
if tuple(RISK_SCORE_IDS) != ANALYSIS_TIE_ARMS:
    raise RuntimeError("V2.1 analysis tie-arm registry drifted from V2")


class V021AnalysisError(V015AnalysisError):
    """Raised when a V2.1 analysis call drifts from its frozen identity."""
...
def _validate_identity(
    *,
    ...
    if protocol_id != V021_PROTOCOL_ID:
        raise V021AnalysisError(
            f"{operation} requires protocol_id={V021_PROTOCOL_ID!r}"
        )
    if (
        ...
        raise V021AnalysisError(
            f"{operation} requires the frozen V2.1 root {expected_root}"
        )


def _positive_count(value: int, *, context: str) -> int:
    if isinstance(value, bool) or not isinstance(value, int) or value < 1:
        raise V021AnalysisError(f"{context} must be a positive integer")
    return value
...
def _validate_zero_based_indices(
    ...
    *,
    ...
    if series.map(lambda value: isinstance(value, (bool, np.bool_))).any():
        raise V021AnalysisError(f"{context} indices are not exact integers")
...
def _validate_issued_counts_and_rates(
    ...
    *,
    ...
    if (
        ...
        raise V021AnalysisError(f"{context} issued counts or rates are invalid")
    issued_count = int(counts[0])
    if expected_issue_count is not None and issued_count != expected_issue_count:
        raise V021AnalysisError(f"{context} uses the wrong issuance count")
...
def deterministic_random_rankings(
    ...
    *,
    issue_count: int,
    protocol_id: str = V021_PROTOCOL_ID,
    random_root: int = RANDOM_ROOT,
    rankings: int = RANDOM_RANKING_COUNT,
) -> pd.DataFrame:
    """Construct same-count random rankings in the V2.1 random namespace."""
    _validate_identity(
    ...
    if eligible["canonical_prefix_content_sha256"].duplicated().any():
        raise V021AnalysisError("Ordinary random-policy content must be unique")
    hashes = np.asarray(
        [
...
def rank_policy(
    ...
    *,
    ...
    """Return the frozen lowest-danger mask with the V2.1 protocol tie key."""
    _validate_identity(
        protocol_id=protocol_id,
        observed_root=random_root,
        expected_root=RANDOM_ROOT,
    ...
    if not frame.index.is_unique:
        raise V021AnalysisError("Policy ranking requires a unique row index")
    ...
    if len(set(verified_hashes)) != len(verified_hashes):
        raise V021AnalysisError(f"Ordinary predictor content is duplicated for {arm}")
    working["_tie_hash"] = [
        derive_analysis_tie_digest(
            protocol_id,
            arm=arm,
...
def risk_reduction_against_random_rankings(
    ...
    *,
    ...
    """Use the mean V2.1 same-count random-ranking risk as the point baseline."""
    _positive_count(rankings, context="Random-ranking count")
    _require_columns(
        random_rankings,
    ...
    if len(random_rankings) != rankings:
        raise V021AnalysisError(f"All {rankings} random rankings are required")
...
def evaluate_policy_rankings(
    ...
    *,
    ...
    """Report all frozen heads against one V2.1 random-ranking baseline."""
    _validate_identity(
        protocol_id=protocol_id,
        observed_root=random_root,
        expected_root=RANDOM_ROOT,
    ...
    if random_mean <= 0.0:
        raise V021AnalysisError("Policy random baseline is zero")
    eligible = _strict_bool_series(
        trajectories["hard_eligible_visible_stress"],
...
def bootstrap_risk_reductions(
    ...
    *,
    ...
    """Run the frozen family-stratified bootstrap in the V2.1 namespace."""
    _validate_identity(
        protocol_id=protocol_id,
    ...
    if not families or len(set(families)) != len(families):
        raise V021AnalysisError("Bootstrap families must be unique and nonempty")
    required = {
        "truth_family",
        "canonical_prefix_content_sha256",
        "catastrophic",
    ...
    for family in families:
        ...
        if subset.empty:
            raise V021AnalysisError(f"Bootstrap family absent: {family}")
...
def bootstrap_gate_summary(
    ...
    *,
    ...
    if len(replicates) != resamples:
        raise V021AnalysisError(f"Every one of {resamples} replicates is required")
    _validate_zero_based_indices(
        replicates,
    ...
    if not _strict_bool_series(
        ...
        raise V021AnalysisError("An undefined bootstrap makes gates inconclusive")
...
def _permuted_boundary_tie_hashes(
    *,
    ...
    if not operating_index.index.is_unique:
        raise V021AnalysisError("Operating pack contains duplicate cluster keys")
    hashes: list[str] = []
...
def stress_permutation_metrics(
    ...
    *,
    ...
    """Jointly permute each eight-field stress block under V2.1 identity."""
    _validate_identity(
        protocol_id=protocol_id,
    ...
    if frame.empty or frame["cluster_id"].duplicated().any():
        raise V021AnalysisError("Stress permutation test pool is invalid")
    if (
        ...
        raise V021AnalysisError("Stress permutation reference metrics are invalid")
    ...
    if len(set(random_hashes)) != len(random_hashes):
        raise V021AnalysisError("Permutation ordinary content is duplicated")
    eligible = _strict_bool_series(
    ...
    if not np.isfinite(prefix_features).all():
        raise V021AnalysisError("Permutation prefix features are nonfinite")
    ...
    for family in TEST_FAMILIES:
        ...
        if not len(positions):
            raise V021AnalysisError(f"Permutation family is absent: {family}")
    ...
    for permutation_index in range(permutations):
        ...
        if scores.shape != (len(frame),) or not np.isfinite(scores).all():
            raise V021AnalysisError("Permuted visible-stress score is invalid")
        ...
        if needed < 1 or needed > len(boundary):
            raise V021AnalysisError("Permutation cutoff reconstruction failed")
...
def summarize_stress_permutations(
    ...
    *,
    ...
    if len(metrics) != permutations:
        raise V021AnalysisError(f"All {permutations} stress permutations are required")
    _validate_zero_based_indices(
        metrics,
    ...
    if not math.isfinite(observed_visible_minus_prefix_increment):
        raise V021AnalysisError("Observed stress increment is nonfinite")
