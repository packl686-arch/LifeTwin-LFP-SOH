"""Outcome-free construction of the V2.4 actual-analysis hash ledger.
...
class V024ActualLedgerIOError(ValueError):
...
def recompute_generation_plan_commitment_bytes_v024(
    view: V024ContractView,
) -> bytes:
    """Return the exact fully enumerated frozen generation-plan bytes."""

    try:
        return audit_formal_v024_generation_plan(view).canonical_bytes
    except V024CollisionError as exc:
        raise V024ActualLedgerIOError(
            "Formal generation-plan recomputation failed"
        ) from exc


def ordinary_family_lookup_v024(
    view: V024ContractView,
...
def actual_analysis_content_records_v024(
    ...
    *,
    ...
    if set(frames) != set(_LABEL_INPUTS):
        raise V024ActualLedgerIOError("Actual-analysis source table registry changed")
    family_by_member = ordinary_family_lookup_v024(view)
    ...
    if tuple(arm_sources) != ANALYSIS_TIE_ARMS:
        raise V024ActualLedgerIOError("Actual-analysis tie-arm mapping changed")
    ...
    for partition, member_id in sorted(expected):
        ...
        if len(operating_rows) != 1:
            raise V024ActualLedgerIOError("Actual-analysis operating row is not unique")
        try:
        ...
        except (V015ArtifactError, V024CollisionError) as exc:
            raise V024ActualLedgerIOError(
...
def recompute_actual_analysis_hash_ledger_bytes_v024(
    frames: Mapping[str, pd.DataFrame],
    *,
    view: V024ContractView,
) -> bytes:
    ...
    records = actual_analysis_content_records_v024(frames, view=view)
    try:
        return bind_formal_v024_actual_analysis_hash_ledger(
            records,
            view,
        ).canonical_bytes
    except V024CollisionError as exc:
...
def verify_actual_analysis_hash_ledger_payload_v024(
...
__all__ = [
    ...
    "actual_analysis_content_records_v024",
