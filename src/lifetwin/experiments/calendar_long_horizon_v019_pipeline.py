"""V2.4 identity adapter for the frozen V2 label-free numerical pipeline.

The inherited V2 implementation hard-codes its protocol identity.  This module
validates V2.4 inputs, translates identity only in memory, reuses the frozen
numerical core, and then rebuilds every identity-sensitive V2.4 output.  It has
...
from lifetwin.experiments.calendar_long_horizon_v019_provenance import (
    ...
    V024ProvenanceError,
    _extract_label_free_state_for_formal_v024,
)
...
class V024PipelineError(ValueError):
    """Raised when the V2.4 label-free adapter cannot prove its boundaries."""
...
def _translate_protocol_column(
    ...
    *,
    ...
    if "protocol_id" not in frame.columns:
        raise V024PipelineError("An identity-bearing table lacks protocol_id")
    if set(frame["protocol_id"].astype(str)) != {source}:
        raise V024PipelineError("A table contains an unexpected protocol identity")
...
def rank_primary_arms_v024(
    *,
    ...
    if (
        ...
        raise V024PipelineError("Primary ranking inputs have inconsistent lengths")
    if any(not isinstance(value, (bool, np.bool_)) for value in eligible_raw):
        raise V024PipelineError("hard_eligible must contain strict booleans")
    if isinstance(issue_count, bool) or not isinstance(issue_count, int):
        raise V024PipelineError("issue_count must be a strict integer")

    eligible = eligible_raw.astype(bool)
    indices = np.flatnonzero(eligible)
    if issue_count < 0 or issue_count > len(indices):
        raise V024PipelineError(
    ...
    if (
        ...
        raise V024PipelineError("An eligible primary risk score is nonfinite")
...
def _suppress_ineligible_probabilities(
    ...
    try:
        ...
        raise V024PipelineError("Risk rows do not align with eligibility rows") from exc
    if eligibility.index.has_duplicates or eligibility.reindex(keys).isna().any():
        raise V024PipelineError("Risk rows do not align with eligibility rows")
    ...
    if not np.isfinite(probabilities).all() or np.any(
        ...
        raise V024PipelineError("An eligible primary calibrated probability is invalid")
    return result
    ...
    *,
    ...
    for partition, raw_features in feature_bundle.groupby("partition", sort=True):
        ...
        if set(risks.index.astype(str)) != set(cluster_ids) or set(
            ...
            raise V024PipelineError("Decision inputs contain different cluster sets")
        risks = risks.loc[cluster_ids]
        contents = contents.loc[cluster_ids]
        if partition in PRIMARY_ISSUE_COUNTS:
            ranking = rank_primary_arms_v024(
...
def _recompute_label_free_pipeline_with_state_v024(
    *,
    ...
    try:
        ...
        raise V024PipelineError(
            "The inherited label-free numerical pipeline rejected V2.4 inputs"
        ) from exc
...
def recompute_label_free_pipeline_v024(
    *,
    prefix_pack: pd.DataFrame,
    forecast_coordinates: pd.DataFrame,
    operating_pack: pd.DataFrame,
    ...
    try:
        ...
        raise V024PipelineError(str(exc)) from exc
    return _recompute_label_free_pipeline_with_state_v024(
...
def recompute_validated_partition_with_state_v024(
    ...
    *,
    ...
    try:
        ...
        raise V024PipelineError(
            "The inherited numerical core rejected a validated V2.4 partition"
        ) from exc
