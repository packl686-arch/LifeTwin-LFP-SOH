"""Outcome-free construction of the V2.2 actual-analysis hash ledger.
...
class V022ActualLedgerIOError(ValueError):
...
def recompute_generation_plan_commitment_bytes_v022(
    view: V022ContractView,
) -> bytes:
    """Return the exact fully enumerated frozen generation-plan bytes."""

    try:
        return audit_formal_v022_generation_plan(view).canonical_bytes
    except V022CollisionError as exc:
        raise V022ActualLedgerIOError(
            "Formal generation-plan recomputation failed"
        ) from exc


def ordinary_family_lookup_v022(
    view: V022ContractView,
...
def actual_analysis_content_records_v022(
    ...
    *,
    ...
    if set(frames) != set(_LABEL_INPUTS):
        raise V022ActualLedgerIOError("Actual-analysis source table registry changed")
    family_by_member = ordinary_family_lookup_v022(view)
    ...
    if tuple(arm_sources) != ANALYSIS_TIE_ARMS:
        raise V022ActualLedgerIOError("Actual-analysis tie-arm mapping changed")
    ...
    for partition, member_id in sorted(expected):
        ...
        if len(operating_rows) != 1:
            raise V022ActualLedgerIOError("Actual-analysis operating row is not unique")
        try:
        ...
        except (V015ArtifactError, V022CollisionError) as exc:
            raise V022ActualLedgerIOError(
...
def recompute_actual_analysis_hash_ledger_bytes_v022(
    frames: Mapping[str, pd.DataFrame],
    *,
    view: V022ContractView,
) -> bytes:
    ...
    records = actual_analysis_content_records_v022(frames, view=view)
    try:
        return bind_formal_v022_actual_analysis_hash_ledger(
            records,
            view,
        ).canonical_bytes
    except V022CollisionError as exc:
...
def verify_actual_analysis_hash_ledger_payload_v022(
...
__all__ = [
    ...
    "actual_analysis_content_records_v022",
