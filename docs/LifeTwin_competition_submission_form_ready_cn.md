...
4. **可失败、可回退的选择性动态更新**：复杂在线修正必须通过改善率、激活覆盖率与尾部风险门槛才能启用。GP 与无门控状态更新均未晋级；最新 V6.1 只在目标电芯自身出现方向一致、幅度足够的残差趋势时提名 P100 小幅修正，否则精确回退 V5。训练内嵌套审计触发 10/41 个电芯并改善 8/10，但仍只冻结为下一批盲测候选，当前不激活。这说明 LifeTwin 的升级由证据驱动，而不是由模型复杂度驱动。
...
最新 V6.1 训练内 challenger 进一步把更新改为“满足历史趋势证据才激活”。在严格外层留一电芯审计中，P100 门控激活 10/41 个电芯、改善 8/10，并使全体 P100 MAE 从 0.24360 pp 降至 0.22397 pp；P40/P60 均回退或淘汰。该门控由同一训练队列启发，因此只作为新批次盲测候选，不替换当前 V5，也未再次使用 81 个已暴露评估电芯。
