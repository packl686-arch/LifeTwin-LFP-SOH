...
`短期前缀 → 严格未来标签防火墙 → 相似参考检索 → 条件化成对残差轨迹 → V5 中心与共形区间 → 新 landmark 重签发 → V7 未吸收创新量候选 → 回退/拒绝与全链路 SHA-256`

- V5 用目标与参考电芯的早期差异预测未来退化差异，不直接复制最近邻曲线。
- V7 会扣除当前 V5 重发轨迹已经吸收的趋势，避免同一残差被重复修正。
- 动态候选必须同时通过逐电芯尾部风险和批次迁移压力门槛，否则精确回退 V5。
...
| V7 P100 留一电芯激活精度与 MAE | `9/9；0.24360→0.20628 pp` | 同一训练队列的方法开发；双向批次压力通过但尚未盲测 |
| 当前正式 champion / 合格独立长期公开队列数 | `V5 / 0` | V7 未激活，不能包装成海辰或 15–25 年精度 |
...
都不能支持海辰产品或 15–25 年准确度结论。当前下一步已经冻结为两条相互独立的
验证任务：用至少 40 个新物理电芯结果盲测 V7 P100 规则；用许可明确、此前未查看
结果的长期 LFP 队列验证日历老化中心与区间。任一失败都保留 V5 或拒绝签发，不在
同一批真值上改阈值重跑。
