"""Deterministic bootstrap and capability-minimal CLI for V2.1."""
...
_BOOTSTRAP_MARKER = "LIFETWIN_V021_DETERMINISTIC_BOOTSTRAP"
_THREAD_VARIABLES = (
    "OMP_NUM_THREADS",
...
def _parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Execute the one-shot frozen V2.1 lifecycle.",
    )
    parser.add_argument(
        "--internal-stage",
...
def _require_attempt_id(value: str | None) -> str:
    if value is None or _ATTEMPT_ID.fullmatch(value) is None:
        raise SystemExit(
            "attempt ID must match ^v021-[A-Za-z0-9][A-Za-z0-9._:-]{0,95}$"
        )
    return value
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.internal_stage == "generation":
        ...
        from lifetwin.experiments.calendar_long_horizon_v016_runner import (  # noqa: PLC0415
    ...
    if args.internal_stage == "prediction":
        ...
        from lifetwin.experiments.calendar_long_horizon_v016_prediction import (  # noqa: PLC0415
