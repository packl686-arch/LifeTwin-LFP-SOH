...
DEFAULT_CONFIG = Path(
    "configs/experiments/fastcharge_lfp_trajectory_portability_v1.json"
)
DEFAULT_OUTPUT_DIRECTORY = Path("artifacts/fastcharge-trajectory-portability-v1")
...
def _ensure_available(paths: list[Path], *, overwrite: bool) -> None:
    ...
    if existing and not overwrite:
        raise FileExistsError(
            "Refusing to overwrite FastCharge portability artifacts: "
            + ", ".join(existing)
...
def _run(args: argparse.Namespace) -> int:
    ...
    config = load_fastcharge_trajectory_config(args.config)
    ...
    scores, summary = score_fastcharge_trajectory_portability(
        cycles,
        predictions,
        manifest,
        config,
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Run the frozen FastCharge LFP trajectory portability stress"
    )
    subparsers = parser.add_subparsers(dest="command", required=True)

    run = subparsers.add_parser("run")
    run.add_argument("raw_cycles")
    run.add_argument("authoritative_crosswalk")
    _common(run)
