...
def center_frames() -> dict[str, pd.DataFrame]:
    ...
    operating = pd.DataFrame(
        {
            "protocol_id": [V024_PROTOCOL_ID] * len(clusters),
    ...
    for cluster_id in clusters:
        ...
        operating_row = (
            operating.loc[operating["cluster_id"].eq(cluster_id)].iloc[0].copy()
        )
        ...
        for frame in (cluster_diagnostics, cluster_forecasts):
            frame["protocol_id"] = V024_PROTOCOL_ID
...
def test_full_71400_prefix_contract_then_exact_7200_center_slice(
    ...
    center = full.loc[full["partition"].eq("center_development")].reset_index(drop=True)
...
def test_partition_contract_rejects_wrong_count_order_and_nonfinite(
    ...
    with pytest.raises(partition.V024PartitionContractError):
    ...
    with pytest.raises(partition.V024PartitionContractError, match="canonical order"):
...
def test_real_apply_partition_accepts_exact_structural_output_nan(
    ...
    assert (
        result.primary_risk_bundle.loc[
...
def test_capability_constructor_and_mutation_guards(
    ...
    contract = load_v024_contract_view().artifacts
    with pytest.raises(partition.V024PartitionCapabilityError):
    ...
    with pytest.raises(partition.V024PartitionCapabilityError, match="mutated"):
...
def test_runner_binds_one_attempt_and_validates_partition_before_truth() -> None:
    ...
    paths = runner.V024RunPaths.resolve(
        repo_root=tmp_path,
    ...
    with pytest.raises(runner.V024RunnerError, match="must equal"):
        runner.initialize_formal_attempt(paths=paths, attempt_id="v024-formal-a2")
    ...
    assert V024_ONLY_ATTEMPT_ID == "v024-formal-20260810-a1"
