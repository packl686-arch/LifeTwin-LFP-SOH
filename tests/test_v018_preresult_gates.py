...
from lifetwin.experiments import calendar_long_horizon_v018_collision as collision
from lifetwin.experiments import calendar_long_horizon_v018_contract as contract
...
def test_frozen_amendment_identity_and_seed_isolation_without_rng_use() -> None:
    ...
    assert not set(design.seed_roots.values()).intersection(V022_SEED_ROOTS)
    assert view.protocol.seed_roots == tuple(V023_EXPECTED_SEED_ROOTS.items())
...
def test_rng_free_formal_collision_plan_is_exact_and_fresh() -> None:
    before = np.random.get_state()
    commitment = collision.audit_formal_v023_generation_plan()
    after = np.random.get_state()
    assert _rng_equal(before, after)
    assert commitment.byte_sha256 == (
        "c0b343db22fe605a73077ca33cc6a3136c88638b7c81f95f651dc44620b0f7e0"
    )
...
def test_v018_formal_sources_expose_no_formal_false_literal() -> None:
    source_root = Path(inspect.getsourcefile(contract) or "").parent
    sources = sorted(source_root.glob("calendar_long_horizon_v018_*.py"))
    sources.append(source_root.parents[2] / "scripts" / "run_calendar_long_horizon_v018.py")
    assert sources
    for path in sources:
