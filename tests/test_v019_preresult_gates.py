...
def test_frozen_amendment_identity_and_seed_isolation_without_rng_use() -> None:
    ...
    assert view.protocol.seed_roots == tuple(V024_EXPECTED_SEED_ROOTS.items())
...
def test_rng_free_formal_collision_plan_is_exact_and_fresh() -> None:
    before = np.random.get_state()
    commitment = collision.audit_formal_v024_generation_plan()
    after = np.random.get_state()
    assert _rng_equal(before, after)
...
def test_v019_formal_sources_expose_no_formal_false_literal() -> None:
    source_root = Path(inspect.getsourcefile(contract) or "").parent
    sources = sorted(source_root.glob("calendar_long_horizon_v019_*.py"))
    sources.append(
        source_root.parents[2] / "scripts" / "run_calendar_long_horizon_v019.py"
    )
    assert sources
    for path in sources:
